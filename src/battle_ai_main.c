...
{
  bool32 isSecondTrainer = (GetBattlerPosition(battler) == B_POSITION_OPPONENT_RIGHT) &&
      (gBattleTypeFlags & BATTLE_TYPE_TWO_OPPONENTS) && !BATTLE_TWO_VS_ONE_OPPONENT;
  u16 trainerId = isSecondTrainer ? gTrainerBattleOpponent_B : gTrainerBattleOpponent_A;
  aiData->shouldDynamax[battler] = (CanDynamax(battler) &&
                                    Rogue_ShouldDynamaxMon(trainerId, gBattlerPartyIndexes[battler],
                                                           CountAIAliveNonEggMonsExcept(
                                                               gBattlerPartyIndexes[battler])));
  aiData->shouldTerastal[battler] = (CanTerastallize(battler) &&
                                     Rogue_ShouldTerastallizeMon(trainerId, gBattlerPartyIndexes[battler],
                                                                 CountAIAliveNonEggMonsExcept(
                                                                     gBattlerPartyIndexes[battler])));
}
...
static u32 Ai_SetMoveAccuracy(struct AiLogicData* aiData, u32 battlerAtk, u32 battlerDef, u32 move)
{
  ...
  if (abilityAtk == ABILITY_NO_GUARD || abilityDef == ABILITY_NO_GUARD ||
      gBattleMoves[move].accuracy == 0)  // Moves with accuracy 0 or no guard ability always hit.
  {
    accuracy = 100;
  } else
  {
    accuracy = GetTotalAccuracy(battlerAtk, battlerDef, move, abilityAtk, abilityDef, aiData->holdEffects[battlerAtk],
...
static bool32 AI_ShouldSwitchIfBadMoves(u32 battler, bool32 doubleBattle)
{
  u32 i, j;
  // If can switch.
  if (CountUsablePartyMons(battler) > 0 && !IsBattlerTrapped(battler, TRUE) &&
      !(gBattleTypeFlags & (BATTLE_TYPE_ARENA | BATTLE_TYPE_PALACE)) &&
      AI_THINKING_STRUCT->aiFlags &
          (AI_FLAG_CHECK_VIABILITY | AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_TRY_TO_FAINT | AI_FLAG_PREFER_BATON_PASS))
  {
    ...
    if (GetBattlerAbility(battler) == ABILITY_TRUANT && IsTruantMonVulnerable(battler, gBattlerTarget) &&
        gDisableStructs[battler].truantCounter && gBattleMons[battler].hp >= gBattleMons[battler].maxHP / 2 &&
...
static inline void BattleAI_DoAIProcessing(struct AI_ThinkingStruct* aiThink, u32 battlerAi, u32 battlerDef)
{
  do
  {
    ...
    if (aiThink->moveConsidered != MOVE_NONE && aiThink->score[aiThink->movesetIndex] > 0 &&
        ShouldConsiderMoveForBattler(battlerAi, battlerDef, aiThink->moveConsidered))
    {
      if (aiThink->aiLogicId < ARRAY_COUNT(sBattleAiFuncTable) && sBattleAiFuncTable[aiThink->aiLogicId] != NULL)
      {
        // Call AI function
        aiThink
            ->score[aiThink->movesetIndex] = sBattleAiFuncTable[aiThink->aiLogicId](battlerAi, battlerDef,
                                                                                    aiThink->moveConsidered,
                                                                                    aiThink
                                                                                        ->score[aiThink->movesetIndex]);
      }
    } else
    {
...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
  ...
  if (!(moveTarget & MOVE_TARGET_USER))
  {
    ...
    // check powder moves
    if ((gBattleMoves[move].flags & FLAG_POWDER_BASED) &&
    ...
    // check off screen
    if (IsSemiInvulnerable(battlerDef, move) && moveEffect != EFFECT_SEMI_INVULNERABLE &&
    ...
    if (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move))
    {
      switch (aiData->abilities[battlerDef])
      {
        case ABILITY_MAGIC_GUARD:
          switch (moveEffect)
          {
            ...
            case EFFECT_LEECH_SEED:
              ...
              break;
            case EFFECT_CURSE:
              if (IS_BATTLER_OF_TYPE(battlerAtk, TYPE_GHOST))  // Don't use Curse if you're a ghost type vs a Magic
                                                               // Guard user, they'll take no damage.
              {
                ADJUST_SCORE(-5);
              }
              break;
        ...
        case ABILITY_HYPER_CUTTER:
          if ((moveEffect == EFFECT_ATTACK_DOWN || moveEffect == EFFECT_ATTACK_DOWN_2) && move != MOVE_PLAY_NICE &&
              move != MOVE_NOBLE_ROAR && move != MOVE_TEARFUL_LOOK && move != MOVE_VENOM_DRENCH)
          {
            RETURN_SCORE_MINUS(10);
          }
          break;
  ...
  switch (moveEffect)
  {
      ...
      if (isDoubleBattle)
      {
        if (!(IS_BATTLER_OF_TYPE(battlerAtk, TYPE_GRASS) && AI_IsBattlerGrounded(battlerAtk) &&
            ...
            !(IS_BATTLER_OF_TYPE(BATTLE_PARTNER(battlerAtk), TYPE_GRASS) &&
              ...
              (BattlerStatCanRise(BATTLE_PARTNER(battlerAtk), aiData->abilities[BATTLE_PARTNER(battlerAtk)],
                                  STAT_ATK) ||
               BattlerStatCanRise(BATTLE_PARTNER(battlerAtk), aiData->abilities[BATTLE_PARTNER(battlerAtk)],
                                  STAT_SPATK))))
        {
    ...
    case EFFECT_GEAR_UP:
      ...
      if (isDoubleBattle)
      {
        if (aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_PLUS ||
            aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_MINUS)
        {
          if ((!BattlerStatCanRise(BATTLE_PARTNER(battlerAtk), aiData->abilities[BATTLE_PARTNER(battlerAtk)],
                                   STAT_ATK) ||
               !HasMoveWithSplit(battlerAtk, SPLIT_PHYSICAL)) &&
              (!BattlerStatCanRise(BATTLE_PARTNER(battlerAtk), aiData->abilities[BATTLE_PARTNER(battlerAtk)],
                                   STAT_SPATK) ||
    ...
    case EFFECT_MAGNETIC_FLUX:
      ...
      if (isDoubleBattle)
      {
            ...
            aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_MINUS)
        {
          ...
          {
            ADJUST_SCORE(-10);
          } else if (!BattlerStatCanRise(BATTLE_PARTNER(battlerAtk), aiData->abilities[BATTLE_PARTNER(battlerAtk)],
                                         STAT_SPDEF))
          {
            ADJUST_SCORE(-8);
          }
    ...
    case EFFECT_ATTACK_DOWN_2:
      if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef],
                           STAT_ATK))  //|| !HasMoveWithSplit(battlerDef, SPLIT_PHYSICAL))
      {
        ADJUST_SCORE(-10);
    ...
    case EFFECT_SPECIAL_ATTACK_DOWN_2:
      if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef],
                           STAT_SPATK))  //|| !HasMoveWithSplit(battlerDef, SPLIT_SPECIAL))
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_SWAGGER:
    case EFFECT_FLATTER:
      if (!AI_CanConfuse(battlerAtk, battlerDef, aiData->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move,
                         aiData->partnerMove))
      {
        ADJUST_SCORE(-10);
      }
    ...
    case EFFECT_STEALTH_ROCK:
      if (gSideTimers[GetBattlerSide(battlerDef)].stealthRockAmount > 0 ||
          PartnerMoveIsSameNoTarget(BATTLE_PARTNER(battlerAtk), move,
                                    aiData->partnerMove))  // Only one mon needs to set up Stealth Rocks
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_PERISH_SONG:
            ...
            CountUsablePartyMons(FOE(battlerAtk)) >= 1)
        {
          ADJUST_SCORE(-10);  // Don't wipe your team if you're going to lose
                   ...
                   (!IsBattlerAlive(BATTLE_PARTNER(FOE(battlerAtk))) ||
                    aiData->abilities[BATTLE_PARTNER(FOE(battlerAtk))] == ABILITY_SOUNDPROOF ||
      ...
      } else
      {
        ...
        if (gStatuses3[FOE(battlerAtk)] & STATUS3_PERISH_SONG ||
            aiData->abilities[FOE(battlerAtk)] == ABILITY_SOUNDPROOF)
        {
          ADJUST_SCORE(-10);
        }
      }
    ...
    case EFFECT_BATON_PASS:
      if (CountUsablePartyMons(battlerAtk) == 0)
      {
        ADJUST_SCORE(-10);
                 ...
                 (gStatuses3[battlerAtk] &
                  (STATUS3_ROOTED | STATUS3_AQUA_RING | STATUS3_MAGNET_RISE | STATUS3_POWER_TRICK)) ||
    ...
    case EFFECT_TORMENT:
      ...
      break;
    case EFFECT_WILL_O_WISP:
      if (!AI_CanBurn(battlerAtk, battlerDef, aiData->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move,
                      aiData->partnerMove))
      {
        ADJUST_SCORE(-10);
      }
    ...
    case EFFECT_MEMENTO:
          ...
          DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
      {
        ADJUST_SCORE(-10);
                 ...
                 gBattleMons[battlerDef].statStages[STAT_SPATK] == MIN_STAT_STAGE)
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_PSYCHO_SHIFT:
          ...
          !AI_CanPoison(battlerAtk, battlerDef, aiData->abilities[battlerDef], move, aiData->partnerMove))
      {
        ADJUST_SCORE(-10);
                 ...
                 !AI_CanBurn(battlerAtk, battlerDef, aiData->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move,
                             aiData->partnerMove))
      {
        ADJUST_SCORE(-10);
      } else if (gBattleMons[battlerAtk].status1 & STATUS1_FROSTBITE &&
                 !AI_CanGiveFrostbite(battlerAtk, battlerDef, aiData->abilities[battlerDef], BATTLE_PARTNER(battlerAtk),
    ...
    case EFFECT_AROMATIC_MIST:
      if (!isDoubleBattle || gBattleMons[BATTLE_PARTNER(battlerAtk)].hp == 0 ||
          !BattlerStatCanRise(BATTLE_PARTNER(battlerAtk), aiData->abilities[BATTLE_PARTNER(battlerAtk)], STAT_SPDEF))
      {
        ADJUST_SCORE(-10);
      }
      break;
    case EFFECT_BIDE:
      if (!HasDamagingMove(battlerDef) || aiData->hpPercents[battlerAtk] < 30  // Close to death
          || gBattleMons[battlerDef].status1 &
              (STATUS1_SLEEP | STATUS1_FREEZE))  // No point in biding if can't take damage
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_TEETER_DANCE:
      if (((gBattleMons[battlerDef].status2 & STATUS2_CONFUSION) ||
           (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move) &&
            aiData->abilities[battlerDef] == ABILITY_OWN_TEMPO) ||
          ...
          ((gBattleMons[BATTLE_PARTNER(battlerDef)].status2 & STATUS2_CONFUSION) ||
           (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move) &&
            aiData->abilities[BATTLE_PARTNER(battlerDef)] == ABILITY_OWN_TEMPO) ||
           (IsBattlerGrounded(BATTLE_PARTNER(battlerDef)) &&
            AI_IsTerrainAffected(BATTLE_PARTNER(battlerDef), STATUS_FIELD_MISTY_TERRAIN)) ||
    ...
    case EFFECT_TRANSFORM:
      if (gBattleMons[battlerAtk].status2 & STATUS2_TRANSFORMED ||
          (gBattleMons[battlerDef].status2 &
           (STATUS2_TRANSFORMED | STATUS2_SUBSTITUTE)))  // Leave out Illusion b/c AI is supposed to be fooled
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_CONVERSION_2:
      ...
      break;
    case EFFECT_LOCK_ON:
      if (gStatuses3[battlerDef] & STATUS3_ALWAYS_HITS || aiData->abilities[battlerAtk] == ABILITY_NO_GUARD ||
    ...
    case EFFECT_HEAL_BELL:
      if (!AnyPartyMemberStatused(battlerAtk, (gBattleMoves[move].flags & FLAG_SOUND_BASED)) ||
          PartnerHasSameMoveEffectWithoutTarget(BATTLE_PARTNER(battlerAtk), move, aiData->partnerMove))
      {
        ADJUST_SCORE(-10);
      }
    ...
    case EFFECT_HIT_PREVENT_ESCAPE:
      break;
    case EFFECT_ENDURE:
      if (gBattleMons[battlerAtk].hp == 1 ||
          GetBattlerSecondaryDamage(battlerAtk))  // Don't use Endure if you'll die after using it
      {
        ADJUST_SCORE(-10);
      }
      break;
    case EFFECT_PROTECT: {
      ...
      if (move != MOVE_QUICK_GUARD && move != MOVE_WIDE_GUARD &&
          move != MOVE_CRAFTY_SHIELD)  // These moves have infinite usage
      {
        if (GetBattlerSecondaryDamage(battlerAtk) >= gBattleMons[battlerAtk].hp &&
            aiData->abilities[battlerDef] != ABILITY_MOXIE && aiData->abilities[battlerDef] != ABILITY_BEAST_BOOST)
        {
    ...
    case EFFECT_MIRACLE_EYE:
      ...
      if (gBattleMons[battlerDef].statStages[STAT_EVASION] <= 4 || !(IS_BATTLER_OF_TYPE(battlerDef, TYPE_DARK)) ||
          DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
      {
        ADJUST_SCORE(-9);
      }
    ...
    case EFFECT_DEFOG:
      if (gSideStatuses[GetBattlerSide(battlerDef)] &
              (SIDE_STATUS_REFLECT | SIDE_STATUS_LIGHTSCREEN | SIDE_STATUS_AURORA_VEIL | SIDE_STATUS_SAFEGUARD |
               SIDE_STATUS_MIST) ||
      ...
      if (isDoubleBattle)
      {
        if (IsHazardMoveEffect(gBattleMoves[aiData->partnerMove].effect)  // partner is going to set up hazards
            && AI_WhoStrikesFirst(BATTLE_PARTNER(battlerAtk), battlerAtk, aiData->partnerMove) ==
                AI_IS_FASTER)  // partner is going to set up before the potential Defog
        {
          ADJUST_SCORE(-10);
      ...
      if (gBattleMons[battlerDef].statStages[STAT_EVASION] == MIN_STAT_STAGE ||
          ((aiData->abilities[battlerDef] == ABILITY_CONTRARY) &&
           !IS_TARGETING_PARTNER(battlerAtk, battlerDef)))  // don't want to raise target stats unless its your partner
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_ROLE_PLAY:
      if (aiData->abilities[battlerAtk] == aiData->abilities[battlerDef] ||
          aiData->abilities[battlerDef] == ABILITY_NONE || IsRolePlayBannedAbilityAtk(aiData->abilities[battlerAtk]) ||
    ...
    case EFFECT_SKILL_SWAP:
      if (aiData->abilities[battlerAtk] == ABILITY_NONE || aiData->abilities[battlerDef] == ABILITY_NONE ||
          IsSkillSwapBannedAbility(aiData->abilities[battlerAtk]) ||
          IsSkillSwapBannedAbility(aiData->abilities[battlerDef]) ||
    ...
    case EFFECT_POWER_TRICK:
      ...
      {
        ADJUST_SCORE(-10);
      } else if (gBattleMons[battlerAtk].defense >= gBattleMons[battlerAtk].attack &&
                 !HasMoveWithSplit(battlerAtk, SPLIT_PHYSICAL))
      {
        ADJUST_SCORE(-10);
      }
    ...
    case EFFECT_POWER_SPLIT:
      ...
      } else
      {
        ...
        if (atkAttack + atkSpAttack >=
            defAttack + defSpAttack)  // Combined attacker stats are > than combined target stats
        {
          ADJUST_SCORE(-10);
        }
        break;
    ...
    case EFFECT_GUARD_SPLIT:
      ...
      } else
      {
        ...
        if (atkDefense + atkSpDefense >=
            defDefense + defSpDefense)  // Combined attacker stats are > than combined target stats
        {
          ADJUST_SCORE(-10);
        }
        break;
    ...
    case EFFECT_PLEDGE:
      if (isDoubleBattle && gBattleMons[BATTLE_PARTNER(battlerAtk)].hp > 0)
      {
        if (aiData->partnerMove != MOVE_NONE && gBattleMoves[aiData->partnerMove].effect == EFFECT_PLEDGE &&
    ...
    case EFFECT_TRICK_ROOM:
      ...
      {
        if (GetBattlerSideSpeedAverage(battlerAtk) <
            GetBattlerSideSpeedAverage(battlerDef))  // Attacker side slower than target side
        {
      ...
      {
        if (GetBattlerSideSpeedAverage(battlerAtk) >=
            GetBattlerSideSpeedAverage(battlerDef))  // Attacker side faster than target side
        {
    ...
    case EFFECT_WONDER_ROOM:
      ...
      break;
    case EFFECT_GRAVITY:
      if ((gFieldStatuses & STATUS_FIELD_GRAVITY && !IS_BATTLER_OF_TYPE(battlerAtk, TYPE_FLYING) &&
    ...
    case EFFECT_FLING:
      ...
      break;
    case EFFECT_EMBARGO:
      if (aiData->abilities[battlerDef] == ABILITY_KLUTZ || gFieldStatuses & STATUS_FIELD_MAGIC_ROOM ||
    ...
    case EFFECT_TELEKINESIS:
      if (gStatuses3[battlerDef] & (STATUS3_TELEKINESIS | STATUS3_ROOTED | STATUS3_SMACKED_DOWN) ||
          gFieldStatuses & STATUS_FIELD_GRAVITY || aiData->holdEffects[battlerDef] == HOLD_EFFECT_IRON_BALL ||
    ...
    case EFFECT_SOAK:
      if (PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove) ||
          (GetBattlerType(battlerDef, 0, FALSE) == TYPE_WATER && GetBattlerType(battlerDef, 1, FALSE) == TYPE_WATER &&
    ...
    case EFFECT_INSTRUCT: {
      ...
      if (instructedMove == MOVE_NONE || (gBattleMoves[instructedMove].banMask & BAN_INSTRUCT) ||
          (gBattleMoves[instructedMove].flags2 & FLAG_TWO_TURN) ||
          gBattleMoves[instructedMove].effect == EFFECT_RECHARGE || IsZMove(instructedMove) ||
      ...
      {
        if (AI_GetBattlerMoveTargetType(battlerDef, instructedMove) &
                (MOVE_TARGET_SELECTED | MOVE_TARGET_DEPENDS | MOVE_TARGET_RANDOM | MOVE_TARGET_BOTH |
    ...
    break;
    case EFFECT_QUASH:
      if (!isDoubleBattle || AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_SLOWER ||
          PartnerMoveIsSameAsAttacker(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
      {
        ADJUST_SCORE(-10);
      }
    ...
    case EFFECT_AFTER_YOU:
      if (!IS_TARGETING_PARTNER(battlerAtk, battlerDef) || !isDoubleBattle ||
    ...
    case EFFECT_SUCKER_PUNCH:
      if (predictedMove != MOVE_NONE)
      {
        if (IS_MOVE_STATUS(predictedMove) ||
            AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_SLOWER)  // Opponent going first
        {
          ADJUST_SCORE(-10);
        }
      }
    ...
    case EFFECT_TAILWIND:
      if (gSideTimers[GetBattlerSide(battlerAtk)].tailwindTimer != 0 ||
          ...
          (gFieldStatuses & STATUS_FIELD_TRICK_ROOM &&
           gFieldTimers.trickRoomTimer > 1))  // Trick Room active and not ending this turn
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_MAGNET_RISE:
      if (gFieldStatuses & STATUS_FIELD_GRAVITY || gDisableStructs[battlerAtk].magnetRiseTimer != 0 ||
    ...
    case EFFECT_SKY_DROP:
      ...
      if (BattlerWillFaintFromWeather(battlerAtk, aiData->abilities[battlerAtk]) ||
          DoesSubstituteBlockMove(battlerAtk, battlerDef, move) || GetBattlerWeight(battlerDef) >= 2000)  // 200.0 kg
      {
        ADJUST_SCORE(-10);
      }
      break;
    ...
    case EFFECT_UPPER_HAND:
      if (predictedMove == MOVE_NONE || IS_MOVE_STATUS(predictedMove) ||
          AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_SLOWER || GetMovePriority(battlerDef, move) < 1 ||
          GetMovePriority(battlerDef, move) > 3)  // Opponent going first or not using priority move
      {
        ADJUST_SCORE(-10);
      }
      break;
...
static s32 AI_DoubleBattle(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
  ...
  if (aiData->partnerMove != 0)
  {
    ...
    if ((gBattleMoves[aiData->partnerMove].flags & FLAG_ALWAYS_CRIT) &&
        aiData->abilities[battlerAtk] == ABILITY_ANGER_POINT)
    {
  ...
  switch (effect)
  {
    ...
    case EFFECT_MAGNET_RISE:
      if (IsBattlerGrounded(battlerAtk) &&
          ...
          (AI_GetMoveEffectiveness(MOVE_EARTHQUAKE, battlerAtk, battlerAtkPartner) !=
           AI_EFFECTIVENESS_x0))  // Doesn't resist ground move
      {
  ...
  switch (effect)
  {
    ...
    case EFFECT_ACID_RAIN:
      if (IsBattlerAlive(battlerAtkPartner) &&
          ShouldSetAcidRain(battlerAtkPartner, atkPartnerAbility, atkPartnerHoldEffect))
      {
        RETURN_SCORE_PLUS(2);  // our partner benefits from snow
      }
      break;
  ...
  if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
  {
    ...
    if (!partnerProtecting && moveTarget != MOVE_TARGET_BOTH &&
        !DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move))
    {
      switch (atkPartnerAbility)
      {
        ...
        case ABILITY_WATER_COMPACTION:
          ...
          {
            RETURN_SCORE_PLUS(
                1);  // only mon with this ability is weak to water so only make it okay if we do very little damage
          }
          RETURN_SCORE_MINUS(10);
          break;
        case ABILITY_FLASH_FIRE:
          ...
          break;
        case ABILITY_AERODYNAMICS:
          if (moveType == TYPE_FLYING && HasMoveWithType(battlerAtkPartner, TYPE_FLYING) &&
      ...
      switch (effect)
      {
        ...
        case EFFECT_SWAGGER:
          if (gBattleMons[battlerAtkPartner].statStages[STAT_ATK] < MAX_STAT_STAGE &&
              ...
              (!AI_CanBeConfused(battlerAtkPartner, TRUE) || atkPartnerHoldEffect == HOLD_EFFECT_CURE_CONFUSION ||
        ...
        case EFFECT_FLATTER:
          ...
          break;
        case EFFECT_BEAT_UP:
          if (atkPartnerAbility == ABILITY_JUSTIFIED && moveType == TYPE_DARK && !IS_MOVE_STATUS(move) &&
              HasMoveWithSplit(battlerAtkPartner, SPLIT_PHYSICAL) &&
              BattlerStatCanRise(battlerAtkPartner, atkPartnerAbility, STAT_ATK) &&
            ...
            if (aiData->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_TRUANT)
            {
              RETURN_SCORE_PLUS(10);
                       ...
                       HasMoveWithLowAccuracy(battlerAtkPartner, FOE(battlerAtkPartner), 90, TRUE, atkPartnerAbility,
                                              aiData->abilities[FOE(battlerAtkPartner)], atkPartnerHoldEffect,
                                              aiData->holdEffects[FOE(battlerAtkPartner)]))
            {
              RETURN_SCORE_PLUS(3);
            }
          }
        ...
        case EFFECT_ENTRAINMENT:
          ...
          break;
        case EFFECT_SOAK:
          if (atkPartnerAbility == ABILITY_WONDER_GUARD && IS_BATTLER_OF_TYPE(battlerAtkPartner, TYPE_WATER) &&
        ...
        case EFFECT_INSTRUCT: {
          ...
          if (instructedMove != MOVE_NONE && !IS_MOVE_STATUS(instructedMove) &&
              (AI_GetBattlerMoveTargetType(battlerAtkPartner, instructedMove) &
               (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY)))  // Use instruct on multi-target moves
          {
            RETURN_SCORE_PLUS(1);
          }
        }
        break;
        case EFFECT_AFTER_YOU:
          if (AI_WhoStrikesFirst(battlerAtkPartner, FOE(battlerAtkPartner), aiData->partnerMove) ==
                  AI_IS_SLOWER  // Opponent mon 1 goes before partner
              || AI_WhoStrikesFirst(battlerAtkPartner, BATTLE_PARTNER(FOE(battlerAtkPartner)), aiData->partnerMove) ==
                  AI_IS_SLOWER)  // Opponent mon 2 goes before partner
          {
            if (gBattleMoves[aiData->partnerMove].effect == EFFECT_COUNTER ||
                gBattleMoves[aiData->partnerMove].effect == EFFECT_MIRROR_COAT)
            {
  ...
  } else  // checking opponent
  {
    ...
    switch (effect)
    {
      case EFFECT_SKILL_SWAP:
        ...
        {
          ADJUST_SCORE(5);
        } else if (IsAbilityOfRating(aiData->abilities[battlerAtk], 0) ||
                   IsAbilityOfRating(aiData->abilities[battlerDef], 10))
        {
...
static s32 AI_CompareDamagingMoves(u32 battlerAtk, u32 battlerDef, u32 currId)
{
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
  {
    ...
    /*
        MgbaPrintf_("%S: required hits: %d Dmg: %d", gMoveNames[moves[i]], noOfHits[i],
       AI_DATA->simulatedDmg[battlerAtk][battlerDef][i]);
    */
  }
...
static s32 AI_CheckViability(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
  ...
  if (!IS_MOVE_STATUS(move) && gBattleMoves[move].accuracy == 0)
  {
    ...
    if (AI_RandLessThan(100) &&
        (gBattleMons[battlerDef].statStages[STAT_EVASION] >= 8 || gBattleMons[battlerAtk].statStages[STAT_ACC] <= 4))
    {
      ADJUST_SCORE(1);
    }
  }
  ...
  // check already dead
  if (!IsBattlerIncapacitated(battlerDef, aiData->abilities[battlerDef]) && CanTargetFaintAi(battlerAtk, battlerDef) &&
  ...
  // check thawing moves
  if ((gBattleMons[battlerAtk].status1 & (STATUS1_FREEZE | STATUS1_FROSTBITE)) &&
      (gBattleMoves[move].flags & FLAG_THAW_USER))
  {
    ADJUST_SCORE(10);
  }
  ...
  switch (moveEffect)
  {
    ...
    case EFFECT_EVASION_UP_2:
      ...
      if ((gBattleMons[battlerDef].status1 & STATUS1_PSN_ANY) && aiData->hpPercents[battlerAtk] >= 50 &&
          !AI_RandLessThan(80))
      {
        ADJUST_SCORE(3);
      }
    ...
    case EFFECT_EVASION_DOWN_2:
      ...
      if ((aiData->hpPercents[battlerAtk] < 70 || gBattleMons[battlerDef].statStages[STAT_EVASION] <= 3) &&
          !AI_RandLessThan(50))
      {
        ADJUST_SCORE(-2);
      }
    ...
    case EFFECT_FLINCH_HIT:
      score += ShouldTryToFlinch(battlerAtk, battlerDef, aiData->abilities[battlerAtk], aiData->abilities[battlerDef],
                                 move);
      break;
    case EFFECT_SWALLOW:
      if (gDisableStructs[battlerAtk].stockpileCounter == 0)
    ...
    case EFFECT_FOCUS_ENERGY:
    case EFFECT_LASER_FOCUS:
      if (aiData->abilities[battlerAtk] == ABILITY_SUPER_LUCK || aiData->abilities[battlerAtk] == ABILITY_SNIPER ||
          aiData->holdEffects[battlerAtk] == HOLD_EFFECT_SCOPE_LENS || HasHighCritRatioMove(battlerAtk))
      {
        ADJUST_SCORE(2);
      }
      break;
    ...
    case EFFECT_SUBSTITUTE:
      ...
      if (HasMoveEffect(battlerDef, EFFECT_SLEEP) || HasMoveEffect(battlerDef, EFFECT_TOXIC) ||
          HasMoveEffect(battlerDef, EFFECT_POISON) || HasMoveEffect(battlerDef, EFFECT_PARALYZE) ||
          HasMoveEffect(battlerDef, EFFECT_WILL_O_WISP) || HasMoveEffect(battlerDef, EFFECT_CONFUSE) ||
          HasMoveEffect(battlerDef, EFFECT_LEECH_SEED))
      {
        ADJUST_SCORE(2);
      }
      if (!gBattleMons[battlerDef].status2 &
          (STATUS2_WRAPPED | STATUS2_ESCAPE_PREVENTION && aiData->hpPercents[battlerAtk] > 70))
      {
        ADJUST_SCORE(1);
      }
      break;
    ...
    case EFFECT_LEECH_SEED:
      if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS) || gStatuses3[battlerDef] & STATUS3_LEECHSEED ||
          HasMoveEffect(battlerDef, EFFECT_RAPID_SPIN) || aiData->abilities[battlerDef] == ABILITY_LIQUID_OOZE ||
    ...
    case EFFECT_PARTING_SHOT:
      if (!IsDoubleBattle())
      {
        switch (ShouldPivot(battlerAtk, battlerDef, aiData->abilities[battlerDef], move, movesetIndex))
        {
          case 0:  // no
            ADJUST_SCORE(
                -10);  // technically should go in CheckBadMove, but this is easier/less computationally demanding
            break;
          case 1:  // maybe
            break;
          case 2:  // yes
    ...
    case EFFECT_PAIN_SPLIT: {
      ...
      if (newHp > healthBenchmark &&
          ShouldAbsorb(battlerAtk, battlerDef, move, aiData->simulatedDmg[battlerAtk][battlerDef][movesetIndex]))
      {
        ADJUST_SCORE(2);
      }
    }
    ...
    case EFFECT_LOCK_ON:
      if (HasMoveEffect(battlerAtk, EFFECT_OHKO))
      {
        ADJUST_SCORE(3);
                 ...
                 HasMoveWithLowAccuracy(battlerAtk, battlerDef, 80, TRUE, aiData->abilities[battlerAtk],
                                        aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk],
                                        aiData->holdEffects[battlerDef]))
      {
        ADJUST_SCORE(3);
      } else if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 85, TRUE, aiData->abilities[battlerAtk],
                                        aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk],
                                        aiData->holdEffects[battlerDef]))
      {
        ADJUST_SCORE(3);
      } else if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, TRUE, aiData->abilities[battlerAtk],
                                        aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk],
                                        aiData->holdEffects[battlerDef]))
      {
        ADJUST_SCORE(1);
    ...
    case EFFECT_SPEED_UP_HIT:
      if (secondaryEffectChance >= 100 && aiData->abilities[battlerDef] != ABILITY_CONTRARY &&
          !AI_STRIKES_FIRST(battlerAtk, battlerDef, move))
      {
        ADJUST_SCORE(3);
      }
      break;
    case EFFECT_DESTINY_BOND:
      ...
      {
        break;
      } else if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER &&
    ...
    case EFFECT_THIEF: {
      ...
      if (canSteal && aiData->items[battlerAtk] == ITEM_NONE && aiData->items[battlerDef] != ITEM_NONE &&
          CanBattlerGetOrLoseItem(battlerDef, aiData->items[battlerDef]) &&
          CanBattlerGetOrLoseItem(battlerAtk, aiData->items[battlerDef]) &&
          !HasMoveEffect(battlerAtk, EFFECT_ACROBATICS) && aiData->abilities[battlerDef] != ABILITY_STICKY_HOLD)
      {
    ...
    case EFFECT_NIGHTMARE:
      if (aiData->abilities[battlerDef] != ABILITY_MAGIC_GUARD &&
          !(gBattleMons[battlerDef].status2 & STATUS2_NIGHTMARE) && AI_IsBattlerAsleepOrComatose(battlerDef))
      {
        ADJUST_SCORE(5);
        if (IsBattlerTrapped(battlerDef, TRUE))
        {
    ...
    case EFFECT_PROTECT:
      ...
      switch (move)
      {
        ...
        case MOVE_WIDE_GUARD:
          if (predictedMove != MOVE_NONE &&
              AI_GetBattlerMoveTargetType(battlerDef, predictedMove) & (MOVE_TARGET_FOES_AND_ALLY | MOVE_TARGET_BOTH))
          {
            ProtectChecks(battlerAtk, battlerDef, move, predictedMove, &score);
          } else if (isDoubleBattle &&
                     AI_GetBattlerMoveTargetType(BATTLE_PARTNER(battlerAtk), aiData->partnerMove) &
        ...
        case MOVE_CRAFTY_SHIELD:
          if (predictedMove != MOVE_NONE && IS_MOVE_STATUS(predictedMove) &&
              !(AI_GetBattlerMoveTargetType(battlerDef, predictedMove) & MOVE_TARGET_USER))
          {
            ProtectChecks(battlerAtk, battlerDef, move, predictedMove, &score);
          }
          break;
        ...
        case MOVE_MAT_BLOCK:
          if (gDisableStructs[battlerAtk].isFirstTurn && predictedMove != MOVE_NONE && !IS_MOVE_STATUS(predictedMove) &&
              !(AI_GetBattlerMoveTargetType(battlerDef, predictedMove) & MOVE_TARGET_USER))
          {
            ProtectChecks(battlerAtk, battlerDef, move, predictedMove, &score);
          }
        ...
        case MOVE_KINGS_SHIELD:
          if (aiData->abilities[battlerAtk] == ABILITY_STANCE_CHANGE  // Special logic for Aegislash
              && (gBattleMons[battlerAtk].species == SPECIES_AEGISLASH_BLADE ||
                  gBattleMons[battlerAtk].species == SPECIES_WOBBUFFET_PUNCHING) &&
    ...
    case EFFECT_FORESIGHT:
      if (aiData->abilities[battlerAtk] == ABILITY_SCRAPPY || aiData->abilities[battlerAtk] == ABILITY_MINDS_EYE)
      {
        break;
                 ...
                 (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GHOST) &&
                  (HasMoveWithType(battlerAtk, TYPE_NORMAL) || HasMoveWithType(battlerAtk, TYPE_FIGHTING))))
      {
        ADJUST_SCORE(2);
      }
      break;
    ...
    case EFFECT_SANDSTORM:
      if (ShouldSetSandstorm(battlerAtk, aiData->holdEffects[battlerAtk], aiData->holdEffects[battlerAtk]))
      {
        ...
        if (HasMoveEffect(battlerDef, EFFECT_MORNING_SUN) || HasMoveEffect(battlerDef, EFFECT_SYNTHESIS) ||
    ...
    case EFFECT_HAIL:
      ...
      {
        if ((HasMoveEffect(battlerAtk, EFFECT_AURORA_VEIL) ||
             HasMoveEffect(BATTLE_PARTNER(battlerAtk), EFFECT_AURORA_VEIL)) &&
    ...
    case EFFECT_RAIN_DANCE:
      if (ShouldSetRain(battlerAtk, aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk]))
      {
        ...
        if (HasMoveEffect(battlerDef, EFFECT_MORNING_SUN) || HasMoveEffect(battlerDef, EFFECT_SYNTHESIS) ||
            HasMoveEffect(battlerDef, EFFECT_SOLAR_BEAM) || HasMoveEffect(battlerDef, EFFECT_MOONLIGHT))
        {
          ADJUST_SCORE(2);
        }
    ...
    case EFFECT_STOCKPILE:
      ...
      break;
    case EFFECT_SWAGGER:
      if (HasMoveEffect(battlerAtk, EFFECT_FOUL_PLAY) || HasMoveEffect(battlerAtk, EFFECT_PSYCH_UP) ||
    ...
    case EFFECT_ATTRACT:
      ...
      if (gBattleMons[battlerDef].status1 & STATUS1_ANY || (gBattleMons[battlerDef].status2 & STATUS2_CONFUSION) ||
    ...
    case EFFECT_PURSUIT:
      ...
      if (IsPredictedToSwitch(battlerDef, battlerAtk))
          ADJUST_SCORE(3);
      else if (IsPredictedToUsePursuitableMove(battlerDef, battlerAtk) && !MoveWouldHitFirst(move, battlerAtk,
      battlerDef)) //Pursuit against fast U-Turn ADJUST_SCORE(3);*/
      break;
    case EFFECT_RAPID_SPIN:
    ...
    case EFFECT_DEFOG:
      ...
      switch (move)
      {
        case MOVE_DEFOG:
          ...
          {
            ADJUST_SCORE(3);
          } else if (!(gSideStatuses[GetBattlerSide(battlerDef)] &
                       SIDE_STATUS_SPIKES))  // Don't blow away hazards if you set them up
          {
            if (isDoubleBattle)
            {
              if (IsHazardMoveEffect(gBattleMoves[aiData->partnerMove].effect)  // Partner is going to set up hazards
                  && AI_WhoStrikesFirst(battlerAtk, BATTLE_PARTNER(battlerAtk), move) ==
                      AI_IS_SLOWER)  // Partner going first
              {
                break;  // Don't use Defog if partner is going to set up hazards
              }
            }
            ...
            if (ShouldLowerEvasion(battlerAtk, battlerDef, aiData->abilities[battlerDef]))
            {
              if (gBattleMons[battlerDef].statStages[STAT_EVASION] > 7 ||
                  HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, TRUE, aiData->abilities[battlerAtk],
                                         aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk],
                                         aiData->holdEffects[battlerDef]))
              {
    ...
    case EFFECT_WILL_O_WISP:
      ...
      break;
    case EFFECT_FOLLOW_ME:
      if (isDoubleBattle && move != MOVE_SPOTLIGHT &&
          !IsBattlerIncapacitated(battlerDef, aiData->abilities[battlerDef]) &&
          (move != MOVE_RAGE_POWDER ||
           IsAffectedByPowder(battlerDef, aiData->abilities[battlerDef],
                              aiData->holdEffects[battlerDef]))  // Rage Powder doesn't affect powder immunities
    ...
    case EFFECT_BESTOW:
      switch (aiData->holdEffects[battlerAtk])
      {
        ...
        case HOLD_EFFECT_FLAME_ORB:
          if (!ShouldBurnSelf(battlerAtk, aiData->abilities[battlerAtk]) &&
              AI_CanBeBurned(battlerAtk, aiData->abilities[battlerDef]))
          {
            ADJUST_SCORE(2);
          }
          break;
        ...
        case HOLD_EFFECT_EJECT_BUTTON:
          ...
          if (HasDamagingMove(battlerAtk) ||
              (isDoubleBattle && IsBattlerAlive(BATTLE_PARTNER(battlerAtk)) &&
               HasDamagingMove(BATTLE_PARTNER(battlerAtk))))
          {
    ...
    case EFFECT_ROLE_PLAY:
      if (!IsRolePlayBannedAbilityAtk(aiData->abilities[battlerAtk]) &&
          !IsRolePlayBannedAbility(aiData->abilities[battlerDef]) &&
          !IsAbilityOfRating(aiData->abilities[battlerAtk], 5) && IsAbilityOfRating(aiData->abilities[battlerDef], 5))
      {
        ADJUST_SCORE(2);
      }
      break;
    ...
    case EFFECT_MAGIC_COAT:
      if (IS_MOVE_STATUS(predictedMove) &&
          AI_GetBattlerMoveTargetType(battlerDef, predictedMove) &
              (MOVE_TARGET_SELECTED | MOVE_TARGET_OPPONENTS_FIELD | MOVE_TARGET_BOTH))
      {
        ADJUST_SCORE(3);
      }
      break;
    case EFFECT_RECYCLE:
      ...
      if (aiData->abilities[battlerAtk] == ABILITY_RIPEN)
      {
        ...
        u32 toHeal = (ItemId_GetHoldEffectParam(item) == 10)
            ? 10
            : gBattleMons[battlerAtk].maxHP / ItemId_GetHoldEffectParam(item);
        ...
        if (IsStatBoostingBerry(item) && aiData->hpPercents[battlerAtk] > 60)
        {
          ADJUST_SCORE(1);
                   ...
                   ((GetWhichBattlerFaster(battlerAtk, battlerDef, TRUE) == 0 &&
                     CanTargetFaintAiWithMod(battlerDef, battlerAtk, 0, 0)) ||
                    !CanTargetFaintAiWithMod(battlerDef, battlerAtk, toHeal, 0)))
        {
          ADJUST_SCORE(1);  // Recycle healing berry if we can't otherwise faint the target and the target wont kill us
                            // after we activate the berry
        }
      }
      break;
    ...
    case EFFECT_ENTRAINMENT:
      ...
      {
        break;
      } else if (IsAbilityOfRating(aiData->abilities[battlerDef], 5) ||
                 GetAbilityRating(aiData->abilities[battlerAtk]) <= 0)
      {
        if (aiData->abilities[battlerDef] != aiData->abilities[battlerAtk] &&
            !(gStatuses3[battlerDef] & STATUS3_GASTRO_ACID))
        {
          ADJUST_SCORE(2);
        }
      }
    ...
    case EFFECT_POWER_TRICK:
      ...
      {
        if (gBattleMons[battlerAtk].defense > gBattleMons[battlerAtk].attack &&
            HasMoveWithSplit(battlerAtk, SPLIT_PHYSICAL))
        {
          ADJUST_SCORE(2);
        }
    ...
    case EFFECT_INCINERATE:
      if (gBattleMons[battlerDef].status2 & STATUS2_SUBSTITUTE || aiData->abilities[battlerDef] == ABILITY_STICKY_HOLD)
      {
        break;
                 ...
                 aiData->holdEffects[battlerDef] == HOLD_EFFECT_GEMS)
      {
        ADJUST_SCORE(3);
      }
      break;
    ...
    case EFFECT_RELIC_SONG:
      ...
      {
        if (gBattleMons[battlerAtk].species == SPECIES_MELOETTA_ARIA &&
            gBattleMons[battlerDef].defense < gBattleMons[battlerDef].spDefense)
        {
          ADJUST_SCORE(3);  // Change to Pirouette if can do more damage
        } else if (gBattleMons[battlerAtk].species == SPECIES_MELOETTA_PIROUETTE &&
                   gBattleMons[battlerDef].spDefense < gBattleMons[battlerDef].defense)
        {
    ...
    case EFFECT_TRICK_ROOM:
      if (!(gFieldStatuses & STATUS_FIELD_TRICK_ROOM) &&
          GetBattlerSideSpeedAverage(battlerAtk) < GetBattlerSideSpeedAverage(battlerDef))
      {
        ADJUST_SCORE(3);
    ...
    case EFFECT_MAGIC_ROOM:
      ...
      if (isDoubleBattle && aiData->holdEffects[BATTLE_PARTNER(battlerAtk)] == HOLD_EFFECT_NONE &&
          aiData->holdEffects[BATTLE_PARTNER(battlerDef)] != HOLD_EFFECT_NONE)
      {
        ADJUST_SCORE(1);
      }
      break;
    case EFFECT_WONDER_ROOM:
      if ((HasMoveWithSplit(battlerDef, SPLIT_PHYSICAL) &&
          ...
          (HasMoveWithSplit(battlerDef, SPLIT_SPECIAL) &&
           gBattleMons[battlerAtk].spDefense < gBattleMons[battlerAtk].defense))
      {
        ADJUST_SCORE(2);
      }
      break;
    case EFFECT_GRAVITY:
      if (!(gFieldStatuses & STATUS_FIELD_GRAVITY))
      {
        ...
        {
          IncreaseSleepScore(battlerAtk, battlerDef, move, &score);
        } else if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, FALSE, aiData->abilities[battlerAtk],
                                          aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk],
                                          aiData->holdEffects[battlerDef]))
        {
          ADJUST_SCORE(2);
    ...
    case EFFECT_FLING:
      ...
      case MOVE_EFFECT_FLINCH:
          score += ShouldTryToFlinch(battlerAtk, battlerDef, aiData->abilities[battlerAtk],
      aiData->abilities[battlerDef], move); break; case MOVE_EFFECT_PARALYSIS: IncreaseParalyzeScore(battlerAtk,
      battlerDef, move, &score); break; case MOVE_EFFECT_POISON: case MOVE_EFFECT_TOXIC: IncreasePoisonScore(battlerAtk,
      battlerDef, move, &score); break; case MOVE_EFFECT_FREEZE: if (AI_CanFreeze(battlerAtk, battlerDef))
              ADJUST_SCORE(3);
          break;
      }*/
      break;
    ...
    case EFFECT_POWDER:
      ...
      break;
    case EFFECT_TELEKINESIS:
      if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, FALSE, aiData->abilities[battlerAtk],
                                 aiData->abilities[battlerDef], aiData->holdEffects[battlerAtk],
                                 aiData->holdEffects[battlerDef]) ||
      ...
      break;
    case EFFECT_THROAT_CHOP:
      if (predictedMove != MOVE_NONE && (gBattleMoves[predictedMove].flags & FLAG_SOUND_BASED) &&
          AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER)
      {
      ...
      break;
    case EFFECT_HEAL_BLOCK:
      if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER && predictedMove != MOVE_NONE &&
          IsHealingMove(predictedMove))
      {
        ADJUST_SCORE(3);  // Try to cancel healing move
                 ...
                 (aiData->holdEffects[battlerDef] == HOLD_EFFECT_BLACK_SLUDGE &&
    ...
    case EFFECT_SOAK:
      if (HasMoveWithType(battlerAtk, TYPE_ELECTRIC) || HasMoveWithType(battlerAtk, TYPE_GRASS) ||
          HasMoveEffect(battlerAtk, EFFECT_FREEZE_DRY))
      {
    ...
    case EFFECT_QUASH:
      if (isDoubleBattle &&
          AI_WhoStrikesFirst(BATTLE_PARTNER(battlerAtk), battlerDef, aiData->partnerMove) ==
              AI_IS_SLOWER)  // Attacker partner wouldn't go before target
      {
        ADJUST_SCORE(1);
      }
      break;
    ...
    case EFFECT_MAGNET_RISE:
      if (IsBattlerGrounded(battlerAtk) && HasDamagingMoveOfType(battlerDef, TYPE_ELECTRIC) &&
          !(AI_GetTypeEffectiveness(MOVE_EARTHQUAKE, battlerDef, battlerAtk) ==
            AI_EFFECTIVENESS_x0))  // Doesn't resist ground move
      {
        if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER)  // Attacker goes first
        {
    ...
    case EFFECT_CAMOUFLAGE:
      if (predictedMove != MOVE_NONE &&
          AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER  // Attacker goes first
          && !IS_MOVE_STATUS(move) &&
    ...
    case EFFECT_FLAME_BURST:
      if (isDoubleBattle)
      {
        if (IsBattlerAlive(BATTLE_PARTNER(battlerDef)) && aiData->hpPercents[BATTLE_PARTNER(battlerDef)] < 12 &&
    ...
    case EFFECT_MIRROR_COAT:
      if (!IsBattlerIncapacitated(battlerDef, aiData->abilities[battlerDef]) && predictedMove != MOVE_NONE)
      {
        ...
        if (GetNoOfHitsToKOBattler(battlerDef, battlerAtk, predictedMoveSlot) >= 2 &&
            GetBattleMoveSplit(predictedMove) == SPLIT_SPECIAL)
        {
          ADJUST_SCORE(3);
        }
      }
    ...
    case EFFECT_SALT_CURE:
      ...
      break;
    case EFFECT_RECHARGE:
      if ((aiData->abilities[battlerAtk] == ABILITY_RAMPAGE) &&
...
static s32 AI_SetupFirstTurn(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
  ...
  if (AI_THINKING_STRUCT->aiFlags & AI_FLAG_SMART_SWITCHING &&
      AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_SLOWER && CanTargetFaintAi(battlerDef, battlerAtk) &&
      GetMovePriority(battlerAtk, move) == 0)
  {
...
static s32 AI_PreferBatonPass(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
  ...
  if (IS_TARGETING_PARTNER(battlerAtk, battlerDef) || CountUsablePartyMons(battlerAtk) == 0 ||
      gBattleMoves[move].power != 0 || !HasMoveEffect(battlerAtk, EFFECT_BATON_PASS) ||
...
static s32 AI_HPAware(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
  ...
  if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
  {
    if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY) ||
        ...
        (moveType == TYPE_WATER &&
         (AI_DATA->abilities[BATTLE_PARTNER(battlerAtk)] == ABILITY_DRY_SKIN ||
