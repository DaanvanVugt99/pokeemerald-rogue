...
static void SetBattlerAiData(u32 battler, struct AiLogicData *aiData)
{
    ...
    aiData->uniqueAbilities[battler] = AI_DecideKnownUniqueAbilityForTurn(battler);
...
// the following checks apply to any target (including user)
    ...
    // other
        ...
        case EFFECT_KNOCK_OFF:
            if (AI_HasAbility(battlerDef, ABILITY_STICKY_HOLD))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_INGRAIN:
        ...
        case EFFECT_TEETER_DANCE:
            if (((gBattleMons[battlerDef].status2 & STATUS2_CONFUSION)
              || (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move) && AI_HasAbility(battlerDef, ABILITY_OWN_TEMPO))
             ...
             && ((gBattleMons[BATTLE_PARTNER(battlerDef)].status2 & STATUS2_CONFUSION)
              || (!DoesBattlerIgnoreAbilityChecks(aiData->abilities[battlerAtk], move) && AI_HasAbility(BATTLE_PARTNER(battlerDef), ABILITY_OWN_TEMPO))
        ...
        case EFFECT_TAUNT:
            if (gDisableStructs[battlerDef].tauntTimer > 0
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove)
              || aiData->abilities[battlerDef] == ABILITY_OBLIVIOUS)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_BESTOW:
        ...
        case EFFECT_POWER_TRICK:
            if (HasBattlerAbility(battlerAtk, ABILITY_ANCIENT_IDOL))
                ADJUST_SCORE(-10);
        ...
        case EFFECT_INFESTED_TERRAIN:
            if (PartnerMoveEffectIsTerrain(BATTLE_PARTNER(battlerAtk), aiData->partnerMove) || gFieldStatuses & STATUS_FIELD_INFESTED_TERRAIN)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_PLAIN_TERRAIN:
            if (PartnerMoveEffectIsTerrain(BATTLE_PARTNER(battlerAtk), aiData->partnerMove) || gFieldStatuses & STATUS_FIELD_PLAIN_TERRAIN)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_PLEDGE:
...
// stat lowering effects
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_JESTER_SWITCH:
    ...
    case EFFECT_BUG_BITE:   // And pluck
        if (gBattleMons[battlerDef].status2 & STATUS2_SUBSTITUTE || AI_HasAbility(battlerDef, ABILITY_STICKY_HOLD))
            break;
