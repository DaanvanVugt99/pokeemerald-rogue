...
// Check if Dynamax battle flag is set. This needs to be defined in include/config/battle.h
    ...
    if ((GetBattlerPosition(battlerId) == B_POSITION_PLAYER_LEFT || (!(gBattleTypeFlags & BATTLE_TYPE_MULTI) && GetBattlerPosition(battlerId) == B_POSITION_PLAYER_RIGHT)) && !CheckBagHasItem(ITEM_DYNAMAX_BAND, 1))
        return FALSE;

    // Check if species isn't allowed to Dynamax.
    if (GET_BASE_SPECIES_ID(species) == SPECIES_ZACIAN || GET_BASE_SPECIES_ID(species) == SPECIES_ZAMAZENTA || GET_BASE_SPECIES_ID(species) == SPECIES_ETERNATUS || species == SPECIES_RAYQUAZA_MEGA || species == SPECIES_KYOGRE_PRIMAL || species == SPECIES_GROUDON_PRIMAL || species == SPECIES_PIKIN_MEGA)
        return FALSE;
    ...
    if (gBattleStruct->dynamax.alreadyDynamaxed[GetBattlerSide(battlerId)] || gBattleStruct->dynamax.dynamaxed[BATTLE_PARTNER(battlerId)] || gBattleStruct->dynamax.toDynamax & gBitTable[BATTLE_PARTNER(battlerId)])
        return FALSE;
...
u8 GetMaxMovePower(u16 move)
{
    ...
    if (gBattleMoves[move].type == TYPE_FIGHTING || gBattleMoves[move].type == TYPE_POISON || move == MOVE_MULTI_ATTACK)
    {
        switch (tier)
        {
        default:
...
void BS_SetMaxMoveEffect(void)
{
    ...
    case MAX_EFFECT_YAWN_FOE:
    {
        ...
        if (!(gStatuses3[gBattlerTarget] & STATUS3_YAWN) && CanSleep(gBattlerTarget) && RandomElement(RNG_G_MAX_SNOOZE, sSnoozeEffects)) // 50% chance of success
        {
...
void BS_TrySetStatus2(void)
{
    ...
    case STATUS2_INFATUATION:
    {
        ...
        if (!(gBattleMons[gBattlerTarget].status2 & STATUS2_INFATUATION) && gBattleMons[gBattlerTarget].ability != ABILITY_OBLIVIOUS && !IsAbilityOnSide(gBattlerTarget, ABILITY_AROMA_VEIL) && atkGender != defGender && atkGender != MON_GENDERLESS && defGender != MON_GENDERLESS)
        {
