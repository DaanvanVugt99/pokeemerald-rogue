...
static const u16 sSkillSwapBannedAbilities[] = {
    ABILITY_WONDER_GUARD,    ABILITY_MULTITYPE,        ABILITY_ILLUSION, ABILITY_STANCE_CHANGE, ABILITY_SCHOOLING,
    ABILITY_COMATOSE,        ABILITY_SHIELDS_DOWN,     ABILITY_DISGUISE, ABILITY_RKS_SYSTEM,    ABILITY_BATTLE_BOND,
    ABILITY_POWER_CONSTRUCT, ABILITY_NEUTRALIZING_GAS, ABILITY_ICE_FACE, ABILITY_HUNGER_SWITCH, ABILITY_GULP_MISSILE,
...
static const u16 sRolePlayBannedAttackerAbilities[] = {
    ABILITY_MULTITYPE,    ABILITY_ZEN_MODE,     ABILITY_STANCE_CHANGE, ABILITY_SCHOOLING,   ABILITY_COMATOSE,
    ABILITY_SHIELDS_DOWN, ABILITY_DISGUISE,     ABILITY_RKS_SYSTEM,    ABILITY_BATTLE_BOND, ABILITY_POWER_CONSTRUCT,
    ABILITY_ICE_FACE,     ABILITY_GULP_MISSILE, ABILITY_ZERO_TO_HERO,  ABILITY_TERA_SHIFT,
};

static const u16 sWorrySeedBannedAbilities[] = {
    ABILITY_MULTITYPE, ABILITY_STANCE_CHANGE, ABILITY_SCHOOLING,    ABILITY_COMATOSE,        ABILITY_SHIELDS_DOWN,
    ABILITY_DISGUISE,  ABILITY_RKS_SYSTEM,    ABILITY_BATTLE_BOND,  ABILITY_POWER_CONSTRUCT, ABILITY_TRUANT,
    ABILITY_ICE_FACE,  ABILITY_GULP_MISSILE,  ABILITY_ZERO_TO_HERO, ABILITY_TERA_SHIFT,
};

static const u16 sGastroAcidBannedAbilities[] = {
    ABILITY_AS_ONE_ICE_RIDER, ABILITY_AS_ONE_SHADOW_RIDER,
    ABILITY_BATTLE_BOND,      ABILITY_COMATOSE,
    ABILITY_DISGUISE,         ABILITY_GULP_MISSILE,
    ABILITY_ICE_FACE,         ABILITY_MULTITYPE,
    ABILITY_POWER_CONSTRUCT,  ABILITY_RKS_SYSTEM,
    ABILITY_SCHOOLING,        ABILITY_SHIELDS_DOWN,
    ABILITY_STANCE_CHANGE,    ABILITY_ZEN_MODE,
    ABILITY_ZERO_TO_HERO,     ABILITY_TERA_SHIFT,
};
...
static const u16 sEntrainmentTargetSimpleBeamBannedAbilities[] = {
    ABILITY_TRUANT,       ABILITY_MULTITYPE,    ABILITY_STANCE_CHANGE, ABILITY_SCHOOLING,   ABILITY_COMATOSE,
    ABILITY_SHIELDS_DOWN, ABILITY_DISGUISE,     ABILITY_RKS_SYSTEM,    ABILITY_BATTLE_BOND, ABILITY_ICE_FACE,
    ABILITY_GULP_MISSILE, ABILITY_ZERO_TO_HERO, ABILITY_TERA_SHIFT,
};
...
bool32 IsAffectedByFollowMe(u32 battlerAtk, u32 defSide, u32 move)
{
  ...
  if (gSideTimers[defSide].followmeTimer == 0 || gBattleMons[gSideTimers[defSide].followmeTarget].hp == 0 ||
      gBattleMoves[move].effect == EFFECT_SNIPE_SHOT || gBattleMoves[move].effect == EFFECT_SKY_DROP ||
      ability == ABILITY_PROPELLER_TAIL || ability == ABILITY_STALWART)
  {
    return FALSE;
  }

  if (gSideTimers[defSide].followmePowder &&
      !IsAffectedByPowder(battlerAtk, ability, GetBattlerHoldEffect(battlerAtk, TRUE)))
  {
    return FALSE;
  }
...
void HandleAction_UseMove(void)
{
  ...
  if (IsAffectedByFollowMe(gBattlerAttacker, side, gCurrentMove) && moveTarget == MOVE_TARGET_SELECTED &&
      GetBattlerSide(gBattlerAttacker) != GetBattlerSide(gSideTimers[side].followmeTarget))
  {
    gBattleStruct->moveTarget[gBattlerAttacker] = gBattlerTarget = gSideTimers[side]
                                                                       .followmeTarget;  // follow me moxie fix
             ...
             ((GetBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker)) != ABILITY_LIGHTNING_ROD &&
              ...
              (GetBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker)) != ABILITY_STORM_DRAIN &&
               moveType == TYPE_WATER)))
  {
    side = GetBattlerSide(gBattlerAttacker);
    for (battler = 0; battler < gBattlersCount; battler++)
    {
      if (side != GetBattlerSide(battler) && *(gBattleStruct->moveTarget + gBattlerAttacker) != battler &&
          ...
          GetBattlerTurnOrderNum(battler) < var && gBattleMoves[gCurrentMove].effect != EFFECT_SNIPE_SHOT &&
...
void HandleAction_UseItem(void)
{
  ...
  gLastUsedItem = gBattleResources->bufferB[gBattlerAttacker][1] |
...
bool32 TryRunFromBattle(u32 battler)
{
  ...
  } else if (GetBattlerAbility(battler) == ABILITY_RUN_AWAY)
  {
    if (InBattlePyramid())
    {
      ...
      speedVar = (gBattleMons[battler].speed * pyramidMultiplier) / (gBattleMons[BATTLE_OPPOSITE(battler)].speed) +
  ...
  } else
  {
    ...
    {
      pyramidMultiplier = GetPyramidRunMultiplier();
      speedVar = (gBattleMons[battler].speed * pyramidMultiplier) / (gBattleMons[runningFromBattler].speed) +
          (gBattleStruct->runTries * 30);
      if (speedVar > (Random() & 0xFF))
      {
        effect = TRUE;
    ...
    {
      speedVar = (gBattleMons[battler].speed * 128) / (gBattleMons[runningFromBattler].speed) +
          (gBattleStruct->runTries * 30);
      if (speedVar > (Random() & 0xFF))
      {
        effect = TRUE;
...
// BUG: safariEscapeFactor can become 0 below. This causes the pokeblock throw glitch.
    ...
    {
      gBattleStruct->safariEscapeFactor -= sPkblToEscapeFactor[gBattleStruct->safariPkblThrowCounter]
...
void HandleAction_NothingIsFainted(void)
{
  ...
  gHitMarker &= ~(HITMARKER_DESTINYBOND | HITMARKER_IGNORE_SUBSTITUTE | HITMARKER_ATTACKSTRING_PRINTED |
                  HITMARKER_NO_PPDEDUCT | HITMARKER_STATUS_ABILITY_EFFECT | HITMARKER_PASSIVE_DAMAGE | HITMARKER_OBEYS |
                  HITMARKER_WAKE_UP_CLEAR | HITMARKER_SYNCHRONISE_EFFECT | HITMARKER_CHARGING | HITMARKER_NEVER_SET);
}
...
void HandleAction_ActionFinished(void)
{
  ...
  *(gBattleStruct->monToSwitchIntoId +
    gBattlerByTurnOrder[gCurrentTurnActionNumber]) = gSelectedMonPartyId = PARTY_SIZE;
  gCurrentTurnActionNumber++;
  gCurrentActionFuncId = gActionsByTurnOrder[gCurrentTurnActionNumber];
  ...
  if (IsTerastallized(gBattlerAttacker) && GetBattlerTeraType(gBattlerAttacker) == TYPE_STELLAR &&
      gBattleMoves[gCurrentMove].split != SPLIT_STATUS && IsTypeStellarBoosted(gBattlerAttacker, moveType))
  {
  ...
  if (B_RECALC_TURN_AFTER_ACTIONS >= GEN_8 && !afterYouActive && !gBattleStruct->pledgeMove)
  {
    ...
    // taken action. It's been previously increased, which we want in order to not recalculate the turn of the mon that
    // just finished its action
    for (i = gCurrentTurnActionNumber; i < gBattlersCount - 1; i++)
    {
      for (j = i + 1; j < gBattlersCount; j++)
      {
        ...
        if (gProtectStructs[battler1].quash || gProtectStructs[battler2].quash || gProtectStructs[battler1].shellTrap ||
            gProtectStructs[battler2].shellTrap)
        {
          continue;
        }

        // We recalculate order only for action of the same priority. If any action other than switch/move has been
        // taken, they should have been executed before. The only recalculation needed is for moves/switch. Mega
        // evolution is handled in src/battle_main.c/TryChangeOrder
        ...
        {
          if (GetWhichBattlerFaster(battler1, battler2, TRUE))  // If the actions chosen are switching, we recalc order
                                                                // but ignoring the moves
          {
            SwapTurnOrder(i, j);
          }
        }
...
static const u8 sAbilitiesAffectedByMoldBreaker[] = {
    [ABILITY_BATTLE_ARMOR] = 1,    [ABILITY_CLEAR_BODY] = 1,      [ABILITY_DAMP] = 1,
    [ABILITY_DRY_SKIN] = 1,        [ABILITY_FILTER] = 1,          [ABILITY_FLASH_FIRE] = 1,
    [ABILITY_FLOWER_GIFT] = 1,     [ABILITY_HEATPROOF] = 1,       [ABILITY_HYPER_CUTTER] = 1,
    [ABILITY_IMMUNITY] = 1,        [ABILITY_INNER_FOCUS] = 1,     [ABILITY_INSOMNIA] = 1,
    [ABILITY_KEEN_EYE] = 1,        [ABILITY_LEAF_GUARD] = 1,      [ABILITY_LEVITATE] = 1,
    [ABILITY_LIGHTNING_ROD] = 1,   [ABILITY_LIMBER] = 1,          [ABILITY_MAGMA_ARMOR] = 1,
    [ABILITY_MARVEL_SCALE] = 1,    [ABILITY_MOTOR_DRIVE] = 1,     [ABILITY_OBLIVIOUS] = 1,
    [ABILITY_OWN_TEMPO] = 1,       [ABILITY_SAND_VEIL] = 1,       [ABILITY_SHELL_ARMOR] = 1,
    [ABILITY_SHIELD_DUST] = 1,     [ABILITY_SIMPLE] = 1,          [ABILITY_SNOW_CLOAK] = 1,
    [ABILITY_SOLID_ROCK] = 1,      [ABILITY_SOUNDPROOF] = 1,      [ABILITY_STICKY_HOLD] = 1,
    [ABILITY_STORM_DRAIN] = 1,     [ABILITY_STURDY] = 1,          [ABILITY_SUCTION_CUPS] = 1,
    [ABILITY_TANGLED_FEET] = 1,    [ABILITY_THICK_FAT] = 1,       [ABILITY_UNAWARE] = 1,
    [ABILITY_VITAL_SPIRIT] = 1,    [ABILITY_VOLT_ABSORB] = 1,     [ABILITY_WATER_ABSORB] = 1,
    [ABILITY_WATER_VEIL] = 1,      [ABILITY_WHITE_SMOKE] = 1,     [ABILITY_WONDER_GUARD] = 1,
    [ABILITY_BIG_PECKS] = 1,       [ABILITY_CONTRARY] = 1,        [ABILITY_FRIEND_GUARD] = 1,
    [ABILITY_HEAVY_METAL] = 1,     [ABILITY_LIGHT_METAL] = 1,     [ABILITY_MAGIC_BOUNCE] = 1,
    [ABILITY_MULTISCALE] = 1,      [ABILITY_SAP_SIPPER] = 1,      [ABILITY_TELEPATHY] = 1,
    [ABILITY_WONDER_SKIN] = 1,     [ABILITY_AROMA_VEIL] = 1,      [ABILITY_BULLETPROOF] = 1,
    [ABILITY_FLOWER_VEIL] = 1,     [ABILITY_FUR_COAT] = 1,        [ABILITY_OVERCOAT] = 1,
    [ABILITY_SWEET_VEIL] = 1,      [ABILITY_DAZZLING] = 1,        [ABILITY_DISGUISE] = 1,
    [ABILITY_FLUFFY] = 1,          [ABILITY_QUEENLY_MAJESTY] = 1, [ABILITY_WATER_BUBBLE] = 1,
    [ABILITY_MIRROR_ARMOR] = 1,    [ABILITY_PUNK_ROCK] = 1,       [ABILITY_ICE_SCALES] = 1,
    [ABILITY_ICE_FACE] = 1,        [ABILITY_PASTEL_VEIL] = 1,     [ABILITY_ARMOR_TAIL] = 1,
    [ABILITY_EARTH_EATER] = 1,     [ABILITY_GOOD_AS_GOLD] = 1,    [ABILITY_PURIFYING_SALT] = 1,
...
static const u8 sHoldEffectToType[][2] = {
    ...
    {HOLD_EFFECT_NORMAL_POWER, TYPE_NORMAL},     {HOLD_EFFECT_FAIRY_POWER, TYPE_FAIRY},
};
...
static const uq4_12_t sTypeEffectivenessTable[NUMBER_OF_MON_TYPES][NUMBER_OF_MON_TYPES] = {
    //                   Defender -->
    //  Attacker         Normal  Fighting Flying  Poison  Ground   Rock    Bug     Ghost   Steel  Mystery  Fire   Water
    //  Grass  Electric Psychic   Ice   Dragon   Dark   Fairy   Stellar
    [TYPE_NORMAL] = {______, ______, ______, ______, ______, X(0.5), ______, X(0.0), X(0.5), ______,
                     ______, ______, ______, ______, ______, ______, ______, ______, ______, ______},
    [TYPE_FIGHTING] = {X(2.0), ______, X(0.5), X(0.5), ______, X(2.0), X(0.5), X(0.0), X(2.0), ______,
                       ______, ______, ______, ______, X(0.5), X(2.0), ______, X(2.0), X(0.5), ______},
    [TYPE_FLYING] = {______, X(2.0), ______, ______, ______, X(0.5), X(2.0), ______, X(0.5), ______,
                     ______, ______, X(2.0), X(0.5), ______, ______, ______, ______, ______, ______},
    [TYPE_POISON] = {______, ______, ______, X(0.5), X(0.5), X(0.5), ______, X(0.5), X(0.0), ______,
                     ______, ______, X(2.0), ______, ______, ______, ______, ______, X(2.0), ______},
    [TYPE_GROUND] = {______, ______, X(0.0), X(2.0), ______, X(2.0), X(0.5), ______, X(2.0), ______,
                     X(2.0), ______, X(0.5), X(2.0), ______, ______, ______, ______, ______, ______},
    [TYPE_ROCK] = {______, X(0.5), X(2.0), ______, X(0.5), ______, X(2.0), ______, X(0.5), ______,
                   X(2.0), ______, ______, ______, ______, X(2.0), ______, ______, ______, ______},
    [TYPE_BUG] = {______, X(0.5), X(0.5), X(0.5), ______, ______, ______, X(0.5), X(0.5), ______,
                  X(0.5), ______, X(2.0), ______, X(2.0), ______, ______, X(2.0), X(0.5), ______},
#if B_STEEL_RESISTANCES >= GEN_6
    [TYPE_GHOST] = {X(0.0), ______, ______, ______, ______, ______, ______, X(2.0), ______, ______,
                    ______, ______, ______, ______, X(2.0), ______, ______, X(0.5), ______, ______},
#else
    [TYPE_GHOST] = {X(0.0), ______, ______, ______, ______, ______, ______, X(2.0), X(0.5), ______,
                    ______, ______, ______, ______, X(2.0), ______, ______, X(0.5), ______, ______},
#endif
    [TYPE_STEEL] = {______, ______, ______, ______, ______, X(2.0), ______, ______, X(0.5), ______,
                    X(0.5), X(0.5), ______, X(0.5), ______, X(2.0), ______, ______, X(2.0), ______},
    [TYPE_MYSTERY] = {______, ______, ______, ______, ______, ______, ______, ______, ______, ______,
                      ______, ______, ______, ______, ______, ______, ______, ______, ______, ______},
    [TYPE_FIRE] = {______, ______, ______, ______, ______, X(0.5), X(2.0), ______, X(2.0), ______,
                   X(0.5), X(0.5), X(2.0), ______, ______, X(2.0), X(0.5), ______, ______, ______},
    [TYPE_WATER] = {______, ______, ______, ______, X(2.0), X(2.0), ______, ______, ______, ______,
                    X(2.0), X(0.5), X(0.5), ______, ______, ______, X(0.5), ______, ______, ______},
    [TYPE_GRASS] = {______, ______, X(0.5), X(0.5), X(2.0), X(2.0), X(0.5), ______, X(0.5), ______,
                    X(0.5), X(2.0), X(0.5), ______, ______, ______, X(0.5), ______, ______, ______},
    [TYPE_ELECTRIC] = {______, ______, X(2.0), ______, X(0.0), ______, ______, ______, ______, ______,
                       ______, X(2.0), X(0.5), X(0.5), ______, ______, X(0.5), ______, ______, ______},
    [TYPE_PSYCHIC] = {______, X(2.0), ______, X(2.0), ______, ______, ______, ______, X(0.5), ______,
                      ______, ______, ______, ______, X(0.5), ______, ______, X(0.0), ______, ______},
    [TYPE_ICE] = {______, ______, X(2.0), ______, X(2.0), ______, ______, ______, X(0.5), ______,
                  X(0.5), X(0.5), X(2.0), ______, ______, X(0.5), X(2.0), ______, ______, ______},
    [TYPE_DRAGON] = {______, ______, ______, ______, ______, ______, ______, ______, X(0.5), ______,
                     ______, ______, ______, ______, ______, ______, X(2.0), ______, X(0.0), ______},
#if B_STEEL_RESISTANCES >= GEN_6
    [TYPE_DARK] = {______, X(0.5), ______, ______, ______, ______, ______, X(2.0), ______, ______,
                   ______, ______, ______, ______, X(2.0), ______, ______, X(0.5), X(0.5), ______},
#else
    [TYPE_DARK] = {______, X(0.5), ______, ______, ______, ______, ______, X(2.0), X(0.5), ______,
                   ______, ______, ______, ______, X(2.0), ______, ______, X(0.5), X(0.5), ______},
#endif
    [TYPE_FAIRY] = {______, X(2.0), ______, X(0.5), ______, ______, ______, ______, X(0.5), ______,
                    X(0.5), ______, ______, ______, ______, ______, X(2.0), X(2.0), ______, ______},
    [TYPE_STELLAR] = {______, ______, ______, ______, ______, ______, ______, ______, ______, ______,
                      ______, ______, ______, ______, ______, ______, ______, ______, ______, ______},
};
...
const u8* CancelMultiTurnMoves(u32 battler)
{
  ...
  if (gBattleStruct->skyDropTargets[battler] != 0xFF && !(gStatuses3[battler] & STATUS3_SKY_DROPPED))
  {
    ...
    // Clear skyDropTargets data, unless this CancelMultiTurnMoves is caused by Yawn, attackcanceler, or
...
{
  if (gProtectStructs[battler].prlzImmobility || gProtectStructs[battler].usedImprisonedMove ||
      gProtectStructs[battler].loveImmobility || gProtectStructs[battler].usedDisabledMove ||
      gProtectStructs[battler].usedTauntedMove || gProtectStructs[battler].usedGravityPreventedMove ||
      gProtectStructs[battler].usedHealBlockedMove || gProtectStructs[battler].flag2Unknown ||
      gProtectStructs[battler].flinchImmobility || gProtectStructs[battler].confusionSelfDmg ||
      gProtectStructs[battler].powderSelfDmg || gProtectStructs[battler].usedThroatChopPreventedMove)
  {
    return TRUE;
  } else
  {
...
void PrepareStringBattle(u16 stringId, u32 battler)
{
  ...
  if ((stringId == STRINGID_ITDOESNTAFFECT || stringId == STRINGID_PKMNWASNTAFFECTED ||
       stringId == STRINGID_PKMNUNAFFECTED) &&
      GetBattlerSide(gBattlerTarget) == B_SIDE_OPPONENT && gBattleStruct->trainerSlidePlayerMonUnaffectedMsgState != 2)
  {
...
static bool32 IsGravityPreventingMove(u32 move)
{
  ...
  return gBattleMoves[move].banMask & BAN_GRAVITY;
}
...
u32 TrySetCantSelectMoveBattleScript(u32 battler)
{
  ...
  u8 moveId = gBattleResources->bufferB[battler][2] &
      ~(RET_MEGA_EVOLUTION | RET_ULTRA_BURST | RET_DYNAMAX | RET_TERASTAL);
  u32 move = gBattleMons[battler].moves[moveId];
  u32 holdEffect = GetBattlerHoldEffect(battler, TRUE);
  u16* choicedMove = &gBattleStruct->choicedMove[battler];

  if (DYNAMAX_BYPASS_CHECK && gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE &&
      gDisableStructs[battler].disabledMove == move && move != MOVE_NONE)
  {
  ...
  if (DYNAMAX_BYPASS_CHECK && gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE &&
      move == gLastMoves[battler] && move != MOVE_STRUGGLE && (gBattleMons[battler].status2 & STATUS2_TORMENT))
  {
  ...
  if (gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE && gDisableStructs[battler].tauntTimer != 0 &&
      IS_MOVE_STATUS(move))
  {
  ...
  if (DYNAMAX_BYPASS_CHECK && gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE &&
      gDisableStructs[battler].throatChopTimer != 0 && (gBattleMoves[move].flags & FLAG_SOUND_BASED))
  {
    gCurrentMove = move;
    if (gBattleTypeFlags & BATTLE_TYPE_PALACE)
    {
  ...
  if (DYNAMAX_BYPASS_CHECK && gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE &&
      IsBelchPreventingMove(battler, move))
  {
    gCurrentMove = move;
    if (gBattleTypeFlags & BATTLE_TYPE_PALACE)
    {
  ...
  if (DYNAMAX_BYPASS_CHECK && move == MOVE_STUFF_CHEEKS &&
  ...
  if ((gBattleMoves[move].flags2 & FLAG_CANT_USE_TWICE) && move == gLastResultingMoves[battler])
  {
    gCurrentMove = move;
  ...
  gPotentialItemEffectBattler = battler;
  if (DYNAMAX_BYPASS_CHECK && HOLD_EFFECT_CHOICE(holdEffect) && *choicedMove != MOVE_NONE &&
...
u8 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
  {
    ...
    // Placeholder
    else if (check & MOVE_LIMITATION_PLACEHOLDER &&
             gBattleMoves[gBattleMons[battler].moves[i]].effect == EFFECT_PLACEHOLDER)
    {
      unusableMoves |= gBitTable[i];
    }
    // Disable
    ...
    // Torment
    else if (check & MOVE_LIMITATION_TORMENTED && gBattleMons[battler].moves[i] == gLastMoves[battler] &&
    ...
    // Encore
    else if (check & MOVE_LIMITATION_ENCORE && gDisableStructs[battler].encoreTimer &&
             gDisableStructs[battler].encoredMove != gBattleMons[battler].moves[i])
    {
      unusableMoves |= gBitTable[i];
    }
    // Choice Items
    else if (check & MOVE_LIMITATION_CHOICE_ITEM && HOLD_EFFECT_CHOICE(holdEffect) && *choicedMove != MOVE_NONE &&
             *choicedMove != MOVE_UNAVAILABLE && *choicedMove != gBattleMons[battler].moves[i])
    {
      unusableMoves |= gBitTable[i];
    }
    // Assault Vest
    else if (check & MOVE_LIMITATION_ASSAULT_VEST && holdEffect == HOLD_EFFECT_ASSAULT_VEST &&
             IS_MOVE_STATUS(gBattleMons[battler].moves[i]) && gBattleMons[battler].moves[i] != MOVE_ME_FIRST)
    {
      unusableMoves |= gBitTable[i];
    }
    // Gravity
    ...
    // Throat Chop
    else if (check & MOVE_LIMITATION_THROAT_CHOP && gDisableStructs[battler].throatChopTimer &&
             (gBattleMoves[gBattleMons[battler].moves[i]].flags & FLAG_SOUND_BASED))
    {
      unusableMoves |= gBitTable[i];
    }
    // Stuff Cheeks
    else if (check & MOVE_LIMITATION_STUFF_CHEEKS && gBattleMons[battler].moves[i] == MOVE_STUFF_CHEEKS &&
             ItemId_GetPocket(gBattleMons[battler].item) != POCKET_BERRIES)
    {
      unusableMoves |= gBitTable[i];
    }
    // Gorilla Tactics
    else if (check & MOVE_LIMITATION_CHOICE_ITEM && GetBattlerAbility(battler) == ABILITY_GORILLA_TACTICS &&
    ...
    // Can't Use Twice flag
             ...
             (gBattleMoves[gBattleMons[battler].moves[i]].flags2 & FLAG_CANT_USE_TWICE) &&
             gBattleMons[battler].moves[i] == gLastResultingMoves[battler])
    {
      unusableMoves |= gBitTable[i];
    }
  }
  ...
  do
  {
    ...
    switch (gBattleStruct->turnCountersTracker)
    {
      ...
      case ENDTURN_SUN:
        ...
        {
          if (!(gBattleWeather & B_WEATHER_SUN_PERMANENT) && !(gBattleWeather & B_WEATHER_SUN_PRIMAL) &&
      ...
      case ENDTURN_STATUS_HEAL:
        ...
        {
          if (B_AFFECTION_MECHANICS == TRUE && GetBattlerSide(gBattlerAttacker) == B_SIDE_PLAYER &&
         ...
         gBattleStruct->turnEffectsTracker <= ENDTURN_BATTLER_COUNT)
  {
    ...
    switch (gBattleStruct->turnEffectsTracker)
    {
      case ENDTURN_INGRAIN:  // ingrain
        if ((gStatuses3[battler] & STATUS3_ROOTED) && !BATTLER_MAX_HP(battler) &&
            !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && gBattleMons[battler].hp != 0)
        {
      ...
      case ENDTURN_AQUA_RING:  // aqua ring
        if ((gStatuses3[battler] & STATUS3_AQUA_RING) && !BATTLER_MAX_HP(battler) &&
            !(gStatuses3[battler] & STATUS3_HEAL_BLOCK) && gBattleMons[battler].hp != 0)
        {
          gBattleMoveDamage = GetDrainedBigRootHp(battler, GetNonDynamaxMaxHP(battler) / 16);
      ...
      case ENDTURN_LEECH_SEED:  // leech seed
        if ((gStatuses3[battler] & STATUS3_LEECHSEED) &&
            gBattleMons[gStatuses3[battler] & STATUS3_LEECHSEED_BATTLER].hp != 0 && gBattleMons[battler].hp != 0)
        {
          MAGIC_GUARD_CHECK;

          gBattlerTarget = gStatuses3[battler] &
        ...
        if ((gBattleMons[battler].status1 & STATUS1_BURN) && gBattleMons[battler].hp != 0)
        {
          ...
          if (ability == ABILITY_HEATPROOF)
          {
            if (gBattleMoveDamage >
                (gBattleMoveDamage / 2) + 1)  // Record ability if the burn takes less damage than it normally would.
            {
        ...
        if (gBattleMons[battler].status2 & STATUS2_UPROAR)
        {
          for (gBattlerAttacker = 0; gBattlerAttacker < gBattlersCount; gBattlerAttacker++)
          {
                ...
                GetBattlerAbility(gBattlerAttacker) != ABILITY_SOUNDPROOF)
            {
              ...
              BtlController_EmitSetMonData(gBattlerAttacker, BUFFER_A, REQUEST_STATUS_BATTLE, 0, 4,
      ...
      case ENDTURN_ENCORE:  // encore
        ...
        {
          if (gBattleMons[battler].moves[gDisableStructs[battler].encoredMovePos] !=
              gDisableStructs[battler].encoredMove)  // pokemon does not have the encored move anymore
          {
      ...
      case ENDTURN_YAWN:  // yawn
        if (gStatuses3[battler] & STATUS3_YAWN)
        {
          ...
          if (!(gStatuses3[battler] & STATUS3_YAWN) && !(gBattleMons[battler].status1 & STATUS1_ANY) &&
              battlerAbility != ABILITY_VITAL_SPIRIT && battlerAbility != ABILITY_INSOMNIA &&
              !UproarWakeUpCheck(battler) && !IsLeafGuardProtected(battler))
          {
            CancelMultiTurnMoves(battler);
            ...
            } else
            {
              ...
              BtlController_EmitSetMonData(battler, BUFFER_A, REQUEST_STATUS_BATTLE, 0, 4,
      ...
      case ENDTURN_CUD_CHEW:
        if (GetBattlerAbility(battler) == ABILITY_CUD_CHEW && !gDisableStructs[battler].cudChew &&
...
bool32 HandleWishPerishSongOnTurnEnd(void)
{
  ...
  switch (gBattleStruct->wishPerishSongState)
  {
    ...
    case 1:
      ...
      // fall through
    case 2:
      if ((gBattleTypeFlags & BATTLE_TYPE_ARENA) && gBattleStruct->arenaTurnCounter == 2 && gBattleMons[0].hp != 0 &&
          gBattleMons[1].hp != 0)
      {
        s32 i;
  ...
  do
  {
    ...
    switch (gBattleStruct->faintedActionsState)
    {
      ...
      case 2:
        ...
        if (B_FAINT_SWITCH_IN >= GEN_4 && gBattleOutcome == 0 && !NoAliveMonsForEitherParty() &&
            gCurrentTurnActionNumber != gBattlersCount)
        {
    ...
    switch (gBattleStruct->atkCancellerTracker)
    {
      ...
      case CANCELLER_FROZEN:  // check being frozen
        if (gBattleMons[gBattlerAttacker].status1 & STATUS1_FREEZE &&
            !(gBattleMoves[gCurrentMove].flags & FLAG_THAW_USER))
        {
      ...
      case CANCELLER_TAUNTED:  // taunt
        if (gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE &&
            gDisableStructs[gBattlerAttacker].tauntTimer && IS_MOVE_STATUS(gCurrentMove))
        {
      ...
      case CANCELLER_IMPRISONED:  // imprisoned
        if (gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE &&
            GetImprisonedMovesCount(gBattlerAttacker, gCurrentMove))
        {
        ...
        if (!gBattleStruct->isAtkCancelerForCalledMove && gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION)
        {
          ...
          if (gBattleMons[gBattlerAttacker].status2 & STATUS2_CONFUSION)
          {
            ...
            if (RandomWeighted(RNG_CONFUSION, (B_CONFUSION_SELF_DMG_CHANCE >= GEN_7 ? 2 : 1), 1))
            {
              ...
              gBattlerTarget = gBattlerAttacker;
              gBattleMoveDamage = CalculateMoveDamage(MOVE_NONE, gBattlerAttacker, gBattlerAttacker, TYPE_MYSTERY, 40,
                                                      FALSE, FALSE, TRUE);
      ...
      case CANCELLER_PARALYSED:  // paralysis
        if (!gBattleStruct->isAtkCancelerForCalledMove && (gBattleMons[gBattlerAttacker].status1 & STATUS1_PARALYSIS) &&
      ...
      case CANCELLER_POWDER_MOVE:
        if ((gBattleMoves[gCurrentMove].flags & FLAG_POWDER_BASED) && (gBattlerAttacker != gBattlerTarget))
        {
        ...
        if (gProtectStructs[gBattlerAttacker]
                .usesBouncedMove)  // Edge case for bouncing a powder move against a grass type pokemon.
        {
      ...
      case CANCELLER_THROAT_CHOP:
        if (gBattleStruct->zmove.toBeUsed[gBattlerAttacker] == MOVE_NONE &&
            gDisableStructs[gBattlerAttacker].throatChopTimer && (gBattleMoves[gCurrentMove].flags & FLAG_SOUND_BASED))
        {
      ...
      case CANCELLER_Z_MOVES:
        if (gBattleStruct->zmove.toBeUsed[gBattlerAttacker] != MOVE_NONE)
        {
          ...
          {
            gBattleStruct->zmove
                .used[BATTLE_PARTNER(gBattlerAttacker)] = TRUE;  // if 1v1 double, set partner used flag as well
          }
      ...
      case CANCELLER_MULTIHIT_MOVES: {
        if (gBattleMoves[gCurrentMove].effect == EFFECT_MULTI_HIT)
        {
          ...
          {
            gMultiHitCounter = 5;
          } else if (ability == ABILITY_BATTLE_BOND && gCurrentMove == MOVE_WATER_SHURIKEN &&
        ...
        } else if (IsManuallyScriptedMultiHit(gCurrentMove))
        {
          if (gBattleMoves[gCurrentMove].effect == EFFECT_POPULATION_BOMB &&
  ...
  do
  {
    switch (gBattleStruct->atkCancellerTracker)
    {
      ...
      case CANCELLER_PSYCHIC_TERRAIN:
        if (gFieldStatuses & STATUS_FIELD_PSYCHIC_TERRAIN && IsBattlerGrounded(gBattlerTarget) &&
...
bool32 HasNoMonsToSwitch(u32 battler, u8 partyIdBattlerOn1, u8 partyIdBattlerOn2)
{
  ...
  if (BATTLE_TWO_VS_ONE_OPPONENT && side == B_SIDE_OPPONENT)
  {
    ...
    for (i = 0; i < PARTY_SIZE; i++)
    {
      if (IsValidForBattle(&party[i]) && i != partyIdBattlerOn1 && i != partyIdBattlerOn2 &&
...
{
  u16 battlerAbility = GetBattlerAbility(battler);
  if (gBattleWeather & B_WEATHER_PRIMAL_ANY && battlerAbility != ABILITY_DESOLATE_LAND &&
      battlerAbility != ABILITY_PRIMORDIAL_SEA && battlerAbility != ABILITY_DELTA_STREAM)
  {
    return FALSE;
...
static bool32 TryChangeBattleTerrain(u32 battler, u32 statusFlag, u8* timer)
{
  ...
  {
    gFieldStatuses &= ~(STATUS_FIELD_MISTY_TERRAIN | STATUS_FIELD_GRASSY_TERRAIN | STATUS_FIELD_ELECTRIC_TERRAIN |
                        STATUS_FIELD_PSYCHIC_TERRAIN | STATUS_FIELD_TERRAIN_PERMANENT);
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
  ...
  switch (caseID)
  {
    ...
    case ABILITYEFFECT_ON_SWITCHIN:  // 0
      ...
      switch (gLastUsedAbility)
      {
            ...
            for (i = 0; i < MAX_BATTLERS_COUNT; i++)
            {
              if (IsBattlerAlive(i) && side != GetBattlerSide(i))
              {
                for (j = 0; j < MAX_MON_MOVES; j++)
                {
                  ...
                  GET_MOVE_TYPE(move, moveType);
                  if (CalcTypeEffectivenessMultiplier(move, moveType, i, battler, ABILITY_ANTICIPATION, FALSE) >=
                      UQ_4_12(2.0))
                  {
                    effect++;
                    break;
        ...
        case ABILITY_COIL_UP:
          if (!gSpecialStatuses[battler].switchInAbilityDone && !(gStatuses4[battler] & STATUS4_COILED))
          {
            gBattlerAttacker = battler;
            gSpecialStatuses[battler].switchInAbilityDone = TRUE;
        ...
        case ABILITY_DAUNTLESS_SHIELD:
          if (!gSpecialStatuses[battler].switchInAbilityDone &&
              ...
              !(gBattleStruct->dauntlessShieldBoost[GetBattlerSide(battler)] &
        ...
        case ABILITY_COSTAR:
          if (!gSpecialStatuses[battler].switchInAbilityDone && IsDoubleBattle() &&
              IsBattlerAlive(BATTLE_PARTNER(battler)) && CountBattlerStatIncreases(BATTLE_PARTNER(battler), FALSE))
          {
            gSpecialStatuses[battler].switchInAbilityDone = TRUE;
      ...
      if (gBattleMons[battler].hp != 0)
      {
        ...
        switch (gLastUsedAbility)
        {
          ...
          case ABILITY_HEALER:
            ...
            if (IsBattlerAlive(gBattleScripting.battler) && gBattleMons[gBattleScripting.battler].status1 & STATUS1_ANY)
            {
          ...
          case ABILITY_BALL_FETCH:
            if (gBattleMons[battler].item == ITEM_NONE &&
                gBattleResults.catchAttempts[gLastUsedBall - ITEM_ULTRA_BALL] >= 1 && !gHasFetchedBall)
            {
              gBattleScripting.battler = battler;
    ...
    case ABILITYEFFECT_MOVES_BLOCK:  // 2
    {
      ...
      if ((gLastUsedAbility == ABILITY_SOUNDPROOF && (gBattleMoves[move].flags & FLAG_SOUND_BASED) &&
           !(moveTarget & MOVE_TARGET_USER)) ||
          (gLastUsedAbility == ABILITY_BULLETPROOF && (gBattleMoves[move].flags & FLAG_BALLISTIC_BASED)))
      {
        ...
        effect = 1;
                  ...
                  gLastUsedAbility == ABILITY_ARMOR_TAIL || IsBattlerAlive(battler ^= BIT_FLANK)) &&
                 (battlerAbility == ABILITY_DAZZLING || battlerAbility == ABILITY_QUEENLY_MAJESTY ||
                  battlerAbility == ABILITY_ARMOR_TAIL) &&
                 GetChosenMovePriority(gBattlerAttacker) > 0 &&
                 GetBattlerSide(gBattlerAttacker) != GetBattlerSide(battler))
      {
        ...
        effect = 1;
                 ...
                 !(IS_MOVE_STATUS(move) &&
                   (targetAbility == ABILITY_MAGIC_BOUNCE || gProtectStructs[gBattlerTarget].bounceMove)))
      {
        if (!(gBattleTypeFlags & BATTLE_TYPE_DOUBLE) || !(moveTarget & (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY)))
        {
          CancelMultiTurnMoves(
              gBattlerAttacker);  // Don't cancel moves that can hit two targets bc one target might not be protected
        }
        gBattleScripting.battler = gBattlerAbility = gBattlerTarget;
        gBattlescriptCurrInstr = BattleScript_DarkTypePreventsPrankster;
        effect = 1;
      } else if (GetBattlerAbility(gBattlerTarget) == ABILITY_GOOD_AS_GOLD && IS_MOVE_STATUS(gCurrentMove) &&
                 !(moveTarget & MOVE_TARGET_USER) && !(moveTarget & MOVE_TARGET_OPPONENTS_FIELD) &&
    ...
    case ABILITYEFFECT_MOVE_END:  // Think contact abilities.
      switch (gLastUsedAbility)
      {
        ...
        case ABILITY_CURSED_BODY:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && TARGET_TURN_DAMAGED &&
              gDisableStructs[gBattlerAttacker].disabledMove == MOVE_NONE && IsBattlerAlive(gBattlerAttacker) &&
            ...
            switch (gBattleMons[gBattlerAttacker].ability)
            {
              ...
              default:
                ...
                gLastUsedAbility = gBattleMons[gBattlerAttacker].ability =
                    gBattleStruct->overwrittenAbilities[gBattlerAttacker] = gBattleMons[gBattlerTarget].ability;
                BattleScriptPushCursor();
        ...
        case ABILITY_WANDERING_SPIRIT:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && IsBattlerAlive(gBattlerAttacker) && TARGET_TURN_DAMAGED &&
              IsMoveMakingContact(move, gBattlerAttacker) && !IsDynamaxed(gBattlerTarget))
          {
            switch (gBattleMons[gBattlerAttacker].ability)
            {
              case ABILITY_DISGUISE:
              ...
              default:
                ...
                gBattleMons[gBattlerAttacker]
                    .ability = gBattleStruct->overwrittenAbilities[gBattlerAttacker] = gBattleMons[gBattlerTarget]
                                                                                           .ability;
                gBattleMons[gBattlerTarget].ability = gBattleStruct
                                                          ->overwrittenAbilities[gBattlerTarget] = gLastUsedAbility;
                BattleScriptPushCursor();
        ...
        case ABILITY_ANGER_POINT:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && gIsCriticalHit && TARGET_TURN_DAMAGED &&
              IsBattlerAlive(battler) && CompareStat(battler, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
          {
            SET_STATCHANGER(STAT_ATK, MAX_STAT_STAGE - gBattleMons[battler].statStages[STAT_ATK], FALSE);
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_TargetsStatWasMaxedOut;
            effect++;
          } else if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && TARGET_TURN_DAMAGED && IsBattlerAlive(battler) &&
                     IS_MOVE_PHYSICAL(gCurrentMove) && CompareStat(battler, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN))
          {
        ...
        case ABILITY_AFTERMATH:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && gBattleMons[gBattlerTarget].hp == 0 &&
              IsBattlerAlive(gBattlerAttacker) && IsMoveMakingContact(move, gBattlerAttacker))
          {
            u32 battler;
        ...
        case ABILITY_EFFECT_SPORE:
              ...
              GetBattlerHoldEffect(gBattlerAttacker, TRUE) != HOLD_EFFECT_SAFETY_GOGGLES)
          {
            ...
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && gBattleMons[gBattlerAttacker].hp != 0 &&
                !gProtectStructs[gBattlerAttacker].confusionSelfDmg && TARGET_TURN_DAMAGED &&
                CanSleep(gBattlerAttacker) && IsMoveMakingContact(move, gBattlerAttacker) && (Random() % 3) == 0)
            {
        ...
        case ABILITY_CUTE_CHARM:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && gBattleMons[gBattlerAttacker].hp != 0 &&
              !gProtectStructs[gBattlerAttacker].confusionSelfDmg && TARGET_TURN_DAMAGED &&
              gBattleMons[gBattlerTarget].hp != 0 && RandomWeighted(RNG_CUTE_CHARM, 2, 1) &&
              !(gBattleMons[gBattlerAttacker].status2 & STATUS2_INFATUATION) &&
              AreBattlersOfOppositeGender(gBattlerAttacker, gBattlerTarget) &&
              GetBattlerAbility(gBattlerAttacker) != ABILITY_OBLIVIOUS && IsMoveMakingContact(move, gBattlerAttacker) &&
              !IsAbilityOnSide(gBattlerAttacker, ABILITY_AROMA_VEIL))
          {
          ...
          break;
        case ABILITY_ILLUSION:
          if (gBattleStruct->illusion[gBattlerTarget].on && !gBattleStruct->illusion[gBattlerTarget].broken &&
              TARGET_TURN_DAMAGED)
          {
            BattleScriptPushCursor();
        ...
        case ABILITY_SAND_SPIT:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && !gProtectStructs[gBattlerAttacker].confusionSelfDmg &&
              TARGET_TURN_DAMAGED && !(gBattleWeather & B_WEATHER_SANDSTORM && WEATHER_HAS_EFFECT))
          {
        ...
        case ABILITY_PERISH_BODY:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && !gProtectStructs[gBattlerAttacker].confusionSelfDmg &&
              TARGET_TURN_DAMAGED && IsBattlerAlive(battler) && (IsMoveMakingContact(move, gBattlerAttacker)) &&
        ...
        case ABILITY_SEED_SOWER:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && !gProtectStructs[gBattlerAttacker].confusionSelfDmg &&
              TARGET_TURN_DAMAGED && IsBattlerAlive(gBattlerTarget) &&
        ...
        case ABILITY_THERMAL_EXCHANGE:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && TARGET_TURN_DAMAGED && IsBattlerAlive(gBattlerTarget) &&
              CompareStat(gBattlerTarget, STAT_ATK, MAX_STAT_STAGE, CMP_LESS_THAN) && moveType == TYPE_FIRE)
          {
            gEffectBattler = gBattlerTarget;
            SET_STATCHANGER(STAT_ATK, 1, FALSE);
            BattleScriptPushCursor();
        ...
        case ABILITY_ANGER_SHELL:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && !gProtectStructs[gBattlerAttacker].confusionSelfDmg &&
              ...
              && IsBattlerAlive(gBattlerTarget) && HadMoreThanHalfHpNowHasLess(gBattlerTarget) &&
              !(TestSheerForceFlag(gBattlerAttacker, gCurrentMove)))
          {
        ...
        case ABILITY_TOXIC_DEBRIS:
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && (!gBattleStruct->isSkyBattle) &&
              !gProtectStructs[gBattlerAttacker].confusionSelfDmg && IS_MOVE_PHYSICAL(gCurrentMove) &&
              TARGET_TURN_DAMAGED && (gSideTimers[gBattlerAttacker].toxicSpikesAmount != 2))
          {
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER:  // Same as above, but for attacker
      switch (gLastUsedAbility)
      {
        ...
        case ABILITY_ELECTROCYTES:
          if (moveType == TYPE_WATER && TARGET_TURN_DAMAGED && IsBattlerAlive(gBattlerTarget) &&
        ...
        case ABILITY_MOLTEN_DOWN:
          if (moveType == TYPE_FIRE && TARGET_TURN_DAMAGED && IsBattlerAlive(gBattlerTarget) &&
    ...
    case ABILITYEFFECT_MOVE_END_OTHER:  // Abilities that activate on *another* battler's moveend: Dancer,
      ...
      {
        case ABILITY_DANCER:
          if (IsBattlerAlive(battler) && (gBattleMoves[gCurrentMove].flags & FLAG_DANCE_BASED) &&
              !gSpecialStatuses[battler].dancerUsedMove && gBattlerAttacker != battler)
          {
...
bool32 CanSleep(u32 battler)
{
  ...
  if (ability == ABILITY_INSOMNIA || ability == ABILITY_VITAL_SPIRIT || ability == ABILITY_COMATOSE ||
      ability == ABILITY_PURIFYING_SALT || gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SAFEGUARD ||
      gBattleMons[battler].status1 & STATUS1_ANY || IsAbilityOnSide(battler, ABILITY_SWEET_VEIL) ||
...
bool32 CanBePoisoned(u32 battlerAttacker, u32 battlerTarget)
{
  ...
  if (!(CanPoisonType(battlerAttacker, battlerTarget)) ||
      ...
      gBattleMons[battlerTarget].status1 & STATUS1_ANY || ability == ABILITY_IMMUNITY || ability == ABILITY_COMATOSE ||
      ability == ABILITY_PURIFYING_SALT || IsAbilityOnSide(battlerTarget, ABILITY_PASTEL_VEIL) ||
      IsAbilityStatusProtected(battlerTarget) || IsBattlerTerrainAffected(battlerTarget, STATUS_FIELD_MISTY_TERRAIN))
  {
    return FALSE;
  }
  return TRUE;
...
bool32 CanBeParalyzed(u32 battler)
{
  ...
  if ((B_PARALYZE_ELECTRIC >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_ELECTRIC)) ||
      gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SAFEGUARD || ability == ABILITY_LIMBER ||
...
{
  if (CompareStat(battler, statId, MAX_STAT_STAGE, CMP_LESS_THAN) &&
      HasEnoughHpToEatBerry(battler, GetBattlerItemHoldEffectParam(battler, itemId), itemId))
  {
...
{
  if (IsBattlerAlive(battler) && !DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove) &&
...
{
  if (IsBattlerAlive(battler) && CompareStat(battler, statId, MAX_STAT_STAGE, CMP_LESS_THAN) &&
      (gBattleScripting.overrideBerryRequirements ||
       (!DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove) &&
        GetBattleMoveSplit(gCurrentMove) == split && battler != gBattlerAttacker && TARGET_TURN_DAMAGED)))
  {
    BufferStatChange(battler, statId, STRINGID_STATROSE);
...
static u8 ItemEffectMoveEnd(u32 battler, u16 holdEffect)
{
  ...
  switch (holdEffect)
  {
    ...
    case HOLD_EFFECT_CRITICAL_UP:  // lansat berry
      if (B_BERRIES_INSTANT >= GEN_4 && !(gBattleMons[battler].status2 & STATUS2_FOCUS_ENERGY) &&
  ...
  switch (caseID)
  {
      ...
      if (!gSpecialStatuses[battler].switchInItemDone)
      {
        switch (battlerHoldEffect)
        {
          ...
          case HOLD_EFFECT_RANDOM_STAT_UP:
            ...
            break;
          case HOLD_EFFECT_CURE_PAR:
            if (B_BERRIES_INSTANT >= GEN_4 && gBattleMons[battler].status1 & STATUS1_PARALYSIS &&
                !UnnerveOn(battler, gLastUsedItem))
            {
              gBattleMons[battler].status1 &= ~STATUS1_PARALYSIS;
            ...
            break;
          case HOLD_EFFECT_CURE_PSN:
            if (B_BERRIES_INSTANT >= GEN_4 && (gBattleMons[battler].status1 & STATUS1_PSN_ANY) &&
                !UnnerveOn(battler, gLastUsedItem))
            {
              gBattleMons[battler].status1 &= ~(STATUS1_PSN_ANY | STATUS1_TOXIC_COUNTER);
            ...
            break;
          case HOLD_EFFECT_CURE_BRN:
            if (B_BERRIES_INSTANT >= GEN_4 && (gBattleMons[battler].status1 & STATUS1_BURN) &&
                !UnnerveOn(battler, gLastUsedItem))
            {
              gBattleMons[battler].status1 &= ~STATUS1_BURN;
            ...
            break;
          case HOLD_EFFECT_CURE_FRZ:
            if (B_BERRIES_INSTANT >= GEN_4 && (gBattleMons[battler].status1 & STATUS1_FREEZE) &&
                !UnnerveOn(battler, gLastUsedItem))
            {
              gBattleMons[battler].status1 &= ~STATUS1_FREEZE;
            ...
            if (B_BERRIES_INSTANT >= GEN_4 && (gBattleMons[battler].status1 & STATUS1_FROSTBITE) &&
                !UnnerveOn(battler, gLastUsedItem))
            {
              gBattleMons[battler].status1 &= ~STATUS1_FROSTBITE;
            ...
            break;
          case HOLD_EFFECT_CURE_SLP:
            if (B_BERRIES_INSTANT >= GEN_4 && (gBattleMons[battler].status1 & STATUS1_SLEEP) &&
                !UnnerveOn(battler, gLastUsedItem))
            {
              gBattleMons[battler].status1 &= ~STATUS1_SLEEP;
              gBattleMons[battler].status2 &= ~STATUS2_NIGHTMARE;
          ...
          case HOLD_EFFECT_EJECT_PACK:
            if (gProtectStructs[battler].statFell && gProtectStructs[battler].disableEjectPack == 0 &&
                CountUsablePartyMons(battler) > 0 &&
                !(gCurrentMove == MOVE_PARTING_SHOT &&
                  CanBattlerSwitch(
                      gBattlerAttacker)))  // Does not activate if attacker used Parting Shot and can switch out
            {
    ...
    case ITEMEFFECT_KINGSROCK: {
      ...
      if (gBattleMoveDamage != 0)
      {
        ...
        {
          if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT) && TARGET_TURN_DAMAGED &&
              RandomPercentage(RNG_ROGUE_FLINCH_CHARM, extraFlinchChance) && gBattleMons[gBattlerTarget].hp)
          {
            shouldFlinch = TRUE;
          }
        }
      ...
      switch (atkHoldEffect)
      {
        case HOLD_EFFECT_FLINCH: {
          ...
          if (gBattleMoveDamage != 0  // Need to have done damage
              ...
              !(gBattleMoves[gCurrentMove].banMask & BAN_KINGS_ROCK) && gBattleMons[gBattlerTarget].hp &&
        ...
        case HOLD_EFFECT_BLUNDER_POLICY:
          if (gBattleStruct->blunderPolicy && gBattleMons[gBattlerAttacker].hp != 0 &&
    ...
    case ITEMEFFECT_LIFEORB_SHELLBELL:
      ...
      switch (atkHoldEffect)
      {
        ...
        case HOLD_EFFECT_LIFE_ORB:
          if (IsBattlerAlive(gBattlerAttacker) && !(TestSheerForceFlag(gBattlerAttacker, gCurrentMove)) &&
        ...
        case HOLD_EFFECT_THROAT_SPRAY:  // Does NOT need to be a damaging move
          if (gProtectStructs[gBattlerAttacker].targetAffected && gBattleMons[gBattlerAttacker].hp != 0 &&
              (gBattleMoves[gCurrentMove].flags & FLAG_SOUND_BASED) &&
      ...
      if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT))
      {
        ...
        switch (battlerHoldEffect)
        {
          ...
          case HOLD_EFFECT_ROCKY_HELMET:
            if (TARGET_TURN_DAMAGED && IsMoveMakingContact(gCurrentMove, gBattlerAttacker) &&
          ...
          case HOLD_EFFECT_JABOCA_BERRY:  // consume and damage attacker if used physical move
            if (IsBattlerAlive(battler) && TARGET_TURN_DAMAGED &&
                !DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove) && IS_MOVE_PHYSICAL(gCurrentMove) &&
                GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD)
            {
              gBattleMoveDamage = GetNonDynamaxMaxHP(gBattlerAttacker) / 8;
              if (gBattleMoveDamage == 0)
          ...
          case HOLD_EFFECT_ROWAP_BERRY:  // consume and damage attacker if used special move
            if (IsBattlerAlive(battler) && TARGET_TURN_DAMAGED &&
                !DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove) && IS_MOVE_SPECIAL(gCurrentMove) &&
                GetBattlerAbility(gBattlerAttacker) != ABILITY_MAGIC_GUARD)
            {
              gBattleMoveDamage = GetNonDynamaxMaxHP(gBattlerAttacker) / 8;
              if (gBattleMoveDamage == 0)
          ...
          case HOLD_EFFECT_STICKY_BARB:
            if (TARGET_TURN_DAMAGED && (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)) &&
                IsMoveMakingContact(gCurrentMove, gBattlerAttacker) &&
                !DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove) && IsBattlerAlive(gBattlerAttacker) &&
    ...
    case ITEMEFFECT_ORBS: {
      ...
      switch (battlerHoldEffect)
      {
        ...
        case HOLD_EFFECT_FLAME_ORB:
          ...
          break;
        case HOLD_EFFECT_STICKY_BARB:  // Not an orb per se, but similar effect, and needs to NOT activate with
                                       // pickpocket
...
u8 IsMonDisobedient(void)
{
  ...
  // if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP && (gCurrentMove == MOVE_SNORE || gCurrentMove ==
  // MOVE_SLEEP_TALK))
  //{
  ...
  //    gBattleMons[gBattlerAttacker].ability != ABILITY_VITAL_SPIRIT && gBattleMons[gBattlerAttacker].ability !=
  //    ABILITY_INSOMNIA)
  //    {
  //        // try putting asleep
  //        int i;
  ...
  //        MOVE_POUND, 0, 40, 0, gBattlerAttacker, gBattlerAttacker); gBattlerTarget = gBattlerAttacker;
...
bool32 IsMoveMakingContact(u32 move, u32 battlerAtk)
{
  ...
  } else if ((atkHoldEffect == HOLD_EFFECT_PUNCHING_GLOVE && gBattleMoves[move].flags & FLAG_PUNCHING_BASED) ||
             atkHoldEffect == HOLD_EFFECT_PROTECTIVE_PADS || GetBattlerAbility(battlerAtk) == ABILITY_LONG_REACH)
  {
    return FALSE;
  } else
  {
...
bool32 IsBattlerProtected(u32 battler, u32 move)
{
  ...
  if ((IsMaxMove(move) || gBattleStruct->zmove.active) &&
      (!gProtectStructs[battler].maxGuarded || gBattleMoves[move].argument == MAX_EFFECT_BYPASS_PROTECT))
  {
    return FALSE;
  }
  ...
  {
    return FALSE;
  } else if (gBattleMoves[move].flags & FLAG_IGNORES_PROTECT)
  {
    return FALSE;
  } else if (gProtectStructs[battler].protected)
  {
...
static inline u32 CalcMoveBasePower(u32 move, u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 weather)
{
  ...
  switch (gBattleMoves[move].effect)
  {
    ...
    case EFFECT_SPIT_UP:
      ...
      break;
    case EFFECT_REVENGE:
      if ((gProtectStructs[battlerAtk].physicalDmg && gProtectStructs[battlerAtk].physicalBattlerId == battlerDef) ||
          (gProtectStructs[battlerAtk].specialDmg && gProtectStructs[battlerAtk].specialBattlerId == battlerDef))
      {
        basePower *= 2;
      }
      break;
    ...
    case EFFECT_ASSURANCE:
      if (gProtectStructs[battlerDef].physicalDmg != 0 || gProtectStructs[battlerDef].specialDmg != 0 ||
          gProtectStructs[battlerDef].confusionSelfDmg)
      {
        basePower *= 2;
      }
      break;
    ...
    case EFFECT_HIDDEN_POWER: {
      ...
      {
        u8 powerBits = ((gBattleMons[battlerAtk].hpIV & 2) >> 1) | ((gBattleMons[battlerAtk].attackIV & 2) << 0) |
            ((gBattleMons[battlerAtk].defenseIV & 2) << 1) | ((gBattleMons[battlerAtk].speedIV & 2) << 2) |
            ((gBattleMons[battlerAtk].spAttackIV & 2) << 3) | ((gBattleMons[battlerAtk].spDefenseIV & 2) << 4);
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move,
                                                  u32 battlerAtk,
                                                  u32 battlerDef,
                                                  u32 moveType,
                                                  bool32 updateFlags,
                                                  ...
                                                  u32 weather)
{
  ...
  switch (gBattleMoves[move].effect)
  {
    ...
    case EFFECT_SOLAR_BEAM:
      if (IsBattlerWeatherAffected(battlerAtk,
                                   (B_WEATHER_HAIL | B_WEATHER_SANDSTORM | B_WEATHER_RAIN | B_WEATHER_SNOW)))
      {
    ...
    case EFFECT_KNOCK_OFF:
      if (B_KNOCK_OFF_DMG >= GEN_6 && gBattleMons[battlerDef].item != ITEM_NONE &&
  ...
  switch (atkAbility)
  {
    ...
    case ABILITY_STRONG_JAW:
      if (gBattleMoves[move].flags & FLAG_BITING_BASED)
      {
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
      }
      break;
    case ABILITY_MEGA_LAUNCHER:
      if (gBattleMoves[move].flags & FLAG_LAUNCHER_BASED)
      {
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
      }
      break;
    ...
    case ABILITY_PROTOSYNTHESIS: {
      ...
      if ((IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN)) &&
          ((IS_MOVE_PHYSICAL(move) && atkHighestStat == STAT_ATK) ||
           (IS_MOVE_SPECIAL(move) && atkHighestStat == STAT_SPATK)))
      {
        modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
      }
      break;
  ...
  switch (defAbility)
  {
    ...
    case ABILITY_PROTOSYNTHESIS: {
      ...
      if (IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN) &&
          ((IS_MOVE_PHYSICAL(move) && defHighestStat == STAT_DEF) ||
           (IS_MOVE_SPECIAL(move) && defHighestStat == STAT_SPDEF)))
      {
  ...
  switch (holdEffectAtk)
  {
    ...
    case HOLD_EFFECT_LUSTROUS_ORB:
      if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_PALKIA &&
    ...
    case HOLD_EFFECT_ADAMANT_ORB:
      if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_DIALGA &&
    ...
    case HOLD_EFFECT_GRISEOUS_ORB:
      if (GET_BASE_SPECIES_ID(gBattleMons[battlerAtk].species) == SPECIES_GIRATINA &&
  ...
  if (IsTerastallized(battlerAtk) &&
      ...
      uq4_12_multiply_by_int_half_down(modifier, basePower) < 60 && !IsManuallyScriptedMultiHit(move) &&
...
static inline u32 CalcAttackStat(u32 move,
                                 u32 battlerAtk,
                                 u32 battlerDef,
                                 u32 moveType,
                                 bool32 isCrit,
                                 ...
                                 u32 holdEffectAtk)
{
  ...
  } else if (GetBattlerAbility(battlerAtk) == ABILITY_POWER_CORE)
  {
    if (IS_MOVE_PHYSICAL(move))
    {
      atkStat = gBattleMons[battlerAtk].attack +
          ((gBattleMons[battlerAtk].defense * gStatStageRatios[gBattleMons[battlerAtk].statStages[STAT_DEF]][0] /
    ...
    } else
    {
      atkStat = gBattleMons[battlerAtk].spAttack +
          ((gBattleMons[battlerAtk].spDefense * gStatStageRatios[gBattleMons[battlerAtk].statStages[STAT_SPDEF]][0] /
  ...
  switch (atkAbility)
  {
    ...
    case ABILITY_PURE_POWER: {
      u16 highestStat = (gBattleMons[battlerAtk].attack > gBattleMons[battlerAtk].spAttack)
          ? gBattleMons[battlerAtk].attack
          : gBattleMons[battlerAtk].spAttack;

      if ((IS_MOVE_PHYSICAL(move) && highestStat == gBattleMons[battlerAtk].attack) ||
          (IS_MOVE_SPECIAL(move) && highestStat == gBattleMons[battlerAtk].spAttack))
      {
    ...
    case ABILITY_FLOWER_GIFT:
      if (gBattleMons[battlerAtk].species == SPECIES_CHERRIM_SUNSHINE &&
          IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN) && IS_MOVE_PHYSICAL(move))
      {
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
      }
      break;
  ...
  if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
  {
    ...
    {
      case ABILITY_FLOWER_GIFT:
        if (gBattleMons[BATTLE_PARTNER(battlerAtk)].species == SPECIES_CHERRIM_SUNSHINE &&
            IsBattlerWeatherAffected(BATTLE_PARTNER(battlerAtk), B_WEATHER_SUN) && IS_MOVE_PHYSICAL(move))
        {
          modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        }
        break;
...
static inline u32 CalcDefenseStat(u32 move,
                                  u32 battlerAtk,
                                  u32 battlerDef,
                                  u32 moveType,
                                  bool32 isCrit,
                                  ...
                                  u32 weather)
{
  ...
  if ((gBattleMons[battlerAtk].ability == ABILITY_SPELL_FIST) && gBattleMoves[move].flags & FLAG_PUNCHING_BASED)
  {
    defStat = spDef;
    defStage = gBattleMons[battlerDef].statStages[STAT_SPDEF];
    usesDefStat = FALSE;
  ...
  if ((gBattleMons[battlerAtk].ability == ABILITY_ROUNDHOUSE) && gBattleMoves[move].flags & FLAG_KICKING_BASED)
  {
  ...
  switch (defAbility)
  {
    ...
    case ABILITY_GRASS_PELT:
      ...
      break;
    case ABILITY_FLOWER_GIFT:
      if (gBattleMons[battlerDef].species == SPECIES_CHERRIM_SUNSHINE &&
    ...
    switch (GetBattlerAbility(BATTLE_PARTNER(battlerDef)))
    {
      case ABILITY_FLOWER_GIFT:
        if (gBattleMons[BATTLE_PARTNER(battlerDef)].species == SPECIES_CHERRIM_SUNSHINE &&
            IsBattlerWeatherAffected(BATTLE_PARTNER(battlerDef), B_WEATHER_SUN) && !usesDefStat)
        {
          modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
        }
        break;
  ...
  switch (holdEffectDef)
  {
    ...
    case HOLD_EFFECT_SOUL_DEW:
      if (B_SOUL_DEW_BOOST < GEN_7 &&
          ...
          !(gBattleTypeFlags & BATTLE_TYPE_FRONTIER) && !usesDefStat)
      {
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
      }
      break;
  ...
  if (B_SANDSTORM_SPDEF_BOOST >= GEN_4 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_ROCK) &&
...
static uq4_12_t GetWeatherDamageModifier(u32 battlerAtk,
                                         u32 move,
                                         u32 moveType,
                                         u32 holdEffectAtk,
                                         u32 holdEffectDef,
                                         u32 weather)
{
  if (weather == B_WEATHER_NONE && gBattleMons[battlerAtk].ability != ABILITY_CHLOROPLAST)
  {
    return UQ_4_12(1.0);
  }

  if (gBattleMoves[move].effect == EFFECT_HYDRO_STEAM && IsBattlerWeatherAffected(battlerAtk, B_WEATHER_SUN))
  {
    return UQ_4_12(1.5);
  }
...
static inline uq4_12_t GetBurnOrFrostBiteModifier(u32 battlerAtk, u32 move, u32 abilityAtk)
{
  if (gBattleMons[battlerAtk].status1 & STATUS1_BURN && IS_MOVE_PHYSICAL(move) &&
      (B_BURN_FACADE_DMG < GEN_6 || gBattleMoves[move].effect != EFFECT_FACADE) && abilityAtk != ABILITY_GUTS)
  {
    return UQ_4_12(0.5);
  }
...
{
  if ((gBattleMoves[move].flags2 & FLAG_MINIMIZE_DOUBLE) && gStatuses3[battlerDef] & STATUS3_MINIMIZED)
  {
    return UQ_4_12(2.0);
  }
  return UQ_4_12(1.0);
...
static inline uq4_12_t GetAttackerAbilitiesModifier(u32 battlerAtk,
...
static inline uq4_12_t GetDefenderAbilitiesModifier(u32 move,
                                                    u32 moveType,
                                                    u32 battlerAtk,
                                                    u32 battlerDef,
                                                    uq4_12_t typeEffectivenessModifier,
...
static inline uq4_12_t GetDefenderItemsModifier(u32 moveType,
                                                u32 battlerDef,
                                                uq4_12_t typeEffectivenessModifier,
                                                bool32 updateFlags,
                                                u32 abilityDef,
...
static inline uq4_12_t GetOtherModifiers(u32 move,
                                         u32 moveType,
                                         u32 battlerAtk,
                                         u32 battlerDef,
                                         bool32 isCrit,
                                         ...
                                         u32 holdEffectDef)
{
  ...
  if (unmodifiedAttackerSpeed >= unmodifiedDefenderSpeed)
  {
    ...
    DAMAGE_MULTIPLY_MODIFIER(
        GetDefenderAbilitiesModifier(move, moveType, battlerAtk, battlerDef, typeEffectivenessModifier, abilityDef));
...
static inline s32 DoMoveDamageCalcVars(u32 move,
                                       u32 battlerAtk,
                                       u32 battlerDef,
                                       u32 moveType,
                                       s32 fixedBasePower,
                                       ...
                                       u32 abilityDef)
{
  ...
  {
    gBattleMovePower = CalcMoveBasePowerAfterModifiers(move, battlerAtk, battlerDef, moveType, updateFlags, abilityAtk,
                                                       abilityDef, holdEffectAtk, weather);
  }

  userFinalAttack = CalcAttackStat(move, battlerAtk, battlerDef, moveType, isCrit, updateFlags, abilityAtk, abilityDef,
                                   holdEffectAtk);
  targetFinalDefense = CalcDefenseStat(move, battlerAtk, battlerDef, moveType, isCrit, updateFlags, abilityAtk,
                                       abilityDef, holdEffectDef, weather);
  ...
  DAMAGE_APPLY_MODIFIER(GetOtherModifiers(move, moveType, battlerAtk, battlerDef, isCrit, typeEffectivenessModifier,
                                          updateFlags, abilityAtk, abilityDef, holdEffectAtk, holdEffectDef));
...
static inline s32 DoMoveDamageCalc(u32 move,
                                   u32 battlerAtk,
                                   u32 battlerDef,
                                   u32 moveType,
                                   s32 fixedBasePower,
                                   ...
                                   u32 weather)
{
  ...
  return DoMoveDamageCalcVars(move, battlerAtk, battlerDef, moveType, fixedBasePower, isCrit, randomFactor, updateFlags,
...
s32 CalculateMoveDamage(u32 move,
                        u32 battlerAtk,
                        u32 battlerDef,
                        u32 moveType,
                        s32 fixedBasePower,
                        ...
                        bool32 updateFlags)
{
  return DoMoveDamageCalc(move, battlerAtk, battlerDef, moveType, fixedBasePower, isCrit, randomFactor, updateFlags,
                          CalcTypeEffectivenessMultiplier(move, moveType, battlerAtk, battlerDef,
                                                          GetBattlerAbility(battlerDef), updateFlags),
                          GetWeather());
}

// for AI so that typeEffectivenessModifier, weather, abilities and holdEffects are calculated only once
s32 CalculateMoveDamageVars(u32 move,
                            u32 battlerAtk,
                            u32 battlerDef,
                            u32 moveType,
                            s32 fixedBasePower,
...
static inline void MulByTypeEffectiveness(uq4_12_t* modifier,
                                          u32 move,
                                          u32 moveType,
                                          u32 battlerDef,
                                          u32 defType,
                                          u32 battlerAtk,
                                          bool32 recordAbilities)
{
  ...
  if (mod == UQ_4_12(0.0) && GetBattlerHoldEffect(battlerDef, TRUE) == HOLD_EFFECT_RING_TARGET)
  {
    ...
    if (recordAbilities)
    {
      RecordItemEffectBattle(battlerDef, HOLD_EFFECT_RING_TARGET);
             ...
             gBattleMons[battlerDef].status2 & STATUS2_FORESIGHT && mod == UQ_4_12(0.0))
  {
    mod = UQ_4_12(1.0);
  } else if ((moveType == TYPE_FIGHTING || moveType == TYPE_NORMAL) && defType == TYPE_GHOST &&
             (abilityAtk == ABILITY_SCRAPPY || abilityAtk == ABILITY_MINDS_EYE) && mod == UQ_4_12(0.0))
  {
    mod = UQ_4_12(1.0);
    if (recordAbilities)
    {
...
static inline void TryNoticeIllusionInTypeEffectiveness(u32 move,
                                                        u32 moveType,
                                                        u32 battlerAtk,
                                                        u32 battlerDef,
                                                        uq4_12_t resultingModifier,
                                                        u32 illusionSpecies)
{
  ...
  MulByTypeEffectiveness(&presumedModifier, move, moveType, battlerDef, gSpeciesInfo[illusionSpecies].types[0],
                         battlerAtk, FALSE);
  if (gSpeciesInfo[illusionSpecies].types[1] != gSpeciesInfo[illusionSpecies].types[0])
  {
    MulByTypeEffectiveness(&presumedModifier, move, moveType, battlerDef, gSpeciesInfo[illusionSpecies].types[1],
                           battlerAtk, FALSE);
  }
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(u32 move,
                                                               u32 moveType,
                                                               u32 battlerAtk,
                                                               u32 battlerDef,
                                                               bool32 recordAbilities,
                                                               ...
                                                               u32 defAbility)
{
  ...
  MulByTypeEffectiveness(&modifier, move, moveType, battlerDef, GetBattlerType(battlerDef, 0, FALSE), battlerAtk,
                         recordAbilities);
  if (GetBattlerType(battlerDef, 1, FALSE) != GetBattlerType(battlerDef, 0, FALSE))
  {
    MulByTypeEffectiveness(&modifier, move, moveType, battlerDef, GetBattlerType(battlerDef, 1, FALSE), battlerAtk,
                           recordAbilities);
  }
  if (GetBattlerType(battlerDef, 2, FALSE) != TYPE_MYSTERY &&
      GetBattlerType(battlerDef, 2, FALSE) != GetBattlerType(battlerDef, 1, FALSE) &&
      GetBattlerType(battlerDef, 2, FALSE) != GetBattlerType(battlerDef, 0, FALSE))
  {
    MulByTypeEffectiveness(&modifier, move, moveType, battlerDef, GetBattlerType(battlerDef, 2, FALSE), battlerAtk,
                           recordAbilities);
  }
  ...
  if (gBattleMoves[move].split == SPLIT_STATUS && move != MOVE_THUNDER_WAVE)
  {
    ...
    if (B_GLARE_GHOST < GEN_4 && move == MOVE_GLARE && IS_BATTLER_OF_TYPE(battlerDef, TYPE_GHOST))
    {
      modifier = UQ_4_12(0.0);
             ...
             !(gBattleMoves[move].flags2 & FLAG_DMG_UNGROUNDED_IGNORE_TYPE_IF_FLYING))
  {
    modifier = UQ_4_12(0.0);
  ...
  if (!IsBattlerGrounded(battlerDef) && (gBattleMoves[move].flags2 & FLAG_DMG_UNGROUNDED_IGNORE_TYPE_IF_FLYING) &&
      (gBattleMons[battlerDef].type1 == TYPE_FLYING || gBattleMons[battlerDef].type2 == TYPE_FLYING ||
...
uq4_12_t CalcTypeEffectivenessMultiplier(u32 move,
                                         u32 moveType,
                                         u32 battlerAtk,
                                         u32 battlerDef,
                                         u32 defAbility,
                                         bool32 recordAbilities)
{
  ...
  {
    modifier = CalcTypeEffectivenessMultiplierInternal(move, moveType, battlerAtk, battlerDef, recordAbilities,
                                                       modifier, defAbility);
    if (gBattleMoves[move].effect == EFFECT_TWO_TYPED_MOVE)
    {
      modifier = CalcTypeEffectivenessMultiplierInternal(move, gBattleMoves[move].argument, battlerAtk, battlerDef,
                                                         recordAbilities, modifier, defAbility);
    }
  }
...
bool32 CanUltraBurst(u32 battler)
{
  ...
  if (gBattleTypeFlags & BATTLE_TYPE_DOUBLE && IsPartnerMonFromSameTrainer(battler) &&
      (gBattleStruct->burst.alreadyBursted[partnerPosition] ||
       (gBattleStruct->burst.toBurst & gBitTable[BATTLE_PARTNER(battler)])))
  {
    return FALSE;
  }
        ...
        switch (method)
        {
          ...
          case FORM_CHANGE_BATTLE_WEATHER:
            ...
            if (formChange.param2 &&
                (IS_FORECAST_ABILITY(GetBattlerAbility(battler)) ? ABILITY_FORECAST : GetBattlerAbility(battler)) !=
                    formChange.param2 &&
...
bool32 CanBattlerFormChange(u32 battler, u16 method)
{
  ...
  if ((IsBattlerMegaEvolved(battler) || IsBattlerUltraBursted(battler) || IsBattlerInTeraForm(battler)) &&
  ...
  else if (IsGigantamaxed(battler) &&
           (method == FORM_CHANGE_FAINT || method == FORM_CHANGE_BATTLE_SWITCH || method == FORM_CHANGE_END_BATTLE))
  {
    return TRUE;
  }
...
bool32 TryBattleFormChange(u32 battler, u16 method)
{
  ...
  } else if (gBattleStruct->changedSpecies[side][monId] != SPECIES_NONE)
  {
    ...
    // Mega Evolved and Ultra Bursted Pokémon should always revert to normal upon fainting or ending the battle, so no
    // need to add it to the form change tables.
    if ((IsBattlerMegaEvolved(battler) || IsBattlerUltraBursted(battler) || IsBattlerInTeraForm(battler)) &&
        (method == FORM_CHANGE_FAINT || method == FORM_CHANGE_END_BATTLE))
    {
      restoreSpecies = TRUE;
...
{
  s32 i;
  u8 types1[3] = {GetBattlerType(battler1, 0, FALSE), GetBattlerType(battler1, 1, FALSE),
                  GetBattlerType(battler1, 2, FALSE)};
  u8 types2[3] = {GetBattlerType(battler2, 0, FALSE), GetBattlerType(battler2, 1, FALSE),
                  GetBattlerType(battler2, 2, FALSE)};
...
bool32 SetIllusionMon(struct Pokemon* mon, u32 battler)
{
  ...
  {
    id = i;
    if (GetMonData(&party[id], MON_DATA_SANITY_HAS_SPECIES) && GetMonData(&party[id], MON_DATA_HP) &&
...
u8 GetBattleMoveSplit(u32 moveId)
{
  ...
  if (gBattleStruct != NULL &&
      gBattleStruct->swapDamageCategory)  // Photon Geyser, Shell Side Arm, Light That Burns the Sky
  {
...
static bool32 IsUnnerveAbilityOnOpposingSide(u32 battler)
{
  if (IsAbilityOnOpposingSide(battler, ABILITY_UNNERVE) || IsAbilityOnOpposingSide(battler, ABILITY_AS_ONE_ICE_RIDER) ||
...
bool32 CanFling(u32 battler)
{
  ...
  if (item == ITEM_NONE || (B_KLUTZ_FLING_INTERACTION >= GEN_5 && GetBattlerAbility(battler) == ABILITY_KLUTZ) ||
      gFieldStatuses & STATUS_FIELD_MAGIC_ROOM || gDisableStructs[battler].embargoTimer != 0 ||
      GetFlingPowerFromItemId(item) == 0 || !CanBattlerGetOrLoseItem(battler, item))
  {
    return FALSE;
  }
...
bool32 CanStealItem(u32 battlerStealing, u32 battlerItem, u16 item)
{
  ...
  if (stealerSide == B_SIDE_OPPONENT &&
      !(gBattleTypeFlags &
        (BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_FRONTIER | BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK |
         BATTLE_TYPE_SECRET_BASE | (B_TRAINERS_KNOCK_OFF_ITEMS == TRUE ? BATTLE_TYPE_TRAINER : 0))))
  {
    return FALSE;
...
{
  // Because BtlController_EmitSetMonData does SetMonData, we need to save the stolen item only if it matches the
  // battler's original So, if the player steals an item during battle and has it stolen from it, it will not end the
  // battle with it (naturally)
  if (B_TRAINERS_KNOCK_OFF_ITEMS == FALSE)
  {
    return;
  }
  // If regular trainer battle and mon's original item matches what is being stolen, save it to be restored at end of
  // battle
...
bool32 BlocksPrankster(u16 move, u32 battlerPrankster, u32 battlerDef, bool32 checkTarget)
{
  ...
  if (checkTarget &&
      (GetBattlerMoveTargetType(battlerPrankster, move) & (MOVE_TARGET_OPPONENTS_FIELD | MOVE_TARGET_DEPENDS)))
  {
    return FALSE;
  }
...
u32 GetBattlerMoveTargetType(u32 battler, u32 move)
{
  ...
  else if (GetBattlerAbility(battler) == ABILITY_ARTILLERY && (gBattleMoves[move].flags & FLAG_LAUNCHER_BASED))
  {
    return MOVE_TARGET_BOTH;
  }
...
bool32 CanTargetBattler(u32 battlerAtk, u32 battlerDef, u16 move)
{
  if (gBattleMoves[move].effect == EFFECT_HIT_ENEMY_HEAL_ALLY &&
      GetBattlerSide(battlerAtk) == GetBattlerSide(battlerDef) && gStatuses3[battlerAtk] & STATUS3_HEAL_BLOCK)
  {
...
static void SetRandomMultiHitCounter()
{
  ...
  } else
  {
    gMultiHitCounter = RandomWeighted(RNG_HITS, 0, 0, 3, 3, 1,
                                      1);  // 37.5%: 2 hits, 37.5%: 3 hits, 12.5% 4 hits, 12.5% 5 hits.
  }
}
...
bool8 canUseExtraMove(u32 battlerAtk, u32 battlerDef)
{
  if (IsBattlerAlive(battlerAtk) && IsBattlerAlive(battlerDef) && battlerAtk != battlerDef &&
      !gProtectStructs[battlerAtk].confusionSelfDmg && !gProtectStructs[battlerAtk].extraMoveUsed &&
      !(gBattleMons[battlerAtk].status1 & STATUS1_SLEEP) && !(gBattleMons[battlerAtk].status1 & STATUS1_FREEZE))
  {
    return TRUE;
  } else
  {
