...
{
    UNIQUE_VERSE_NONE,
    UNIQUE_VERSE_OPENING, // Next contact move raises Speed.
    UNIQUE_VERSE_FINAL,   // Next sound move can't miss and always crits.
};
...
static bool32 TryUseBasaltShellCalledMove(u32 battler, u32 target);
static bool32 TryUseBalloonBurstCalledMove(u32 battler, u32 target);
static bool32 TryUseKeyringCalledMove(u32 battler);
...
static u32 GetMeloettaVerseState(u32 battler)
{
    if (gDisableStructs[battler].uniquePersistentStateActive)
        return UNIQUE_VERSE_OPENING;
    if (gDisableStructs[battler].uniqueOncePerSwitchInUsed)
        return UNIQUE_VERSE_FINAL;
    return UNIQUE_VERSE_NONE;
}
...
static void SetMeloettaVerseState(u32 battler, u32 state)
{
    ...
    if (state == UNIQUE_VERSE_OPENING)
        gDisableStructs[battler].uniquePersistentStateActive = TRUE;
    else if (state == UNIQUE_VERSE_FINAL)
...
void HandleAction_UseMove(void)
{
        ...
        && GetBattlerSide(gBattlerAttacker) != GetBattlerSide(gSideTimers[side].followmeTarget))
    {
        gBattleStruct->moveTarget[gBattlerAttacker] = gBattlerTarget = gSideTimers[side].followmeTarget; // follow me moxie fix
           ...
           && ((!HasBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker), ABILITY_LIGHTNING_ROD) && moveType == TYPE_ELECTRIC)
            || (!HasBattlerAbility(*(gBattleStruct->moveTarget + gBattlerAttacker), ABILITY_STORM_DRAIN) && moveType == TYPE_WATER)))
    {
        ...
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            if (side != GetBattlerSide(battler)
                ...
                && ((HasBattlerAbility(battler, ABILITY_LIGHTNING_ROD) && moveType == TYPE_ELECTRIC)
                 || (HasBattlerAbility(battler, ABILITY_STORM_DRAIN) && moveType == TYPE_WATER))
                ...
                && !HasBattlerAbility(gBattlerAttacker, ABILITY_PROPELLER_TAIL)
                && !HasBattlerAbility(gBattlerAttacker, ABILITY_STALWART))
            {
        ...
        else
        {
            ...
            if (HasBattlerAbility(battler, ABILITY_LIGHTNING_ROD) && gCurrentMove != MOVE_TEATIME)
            {
...
const u8* CancelMultiTurnMoves(u32 battler)
{
    ...
    if (gBattleStruct->skyDropTargets[battler] != 0xFF && !(gStatuses3[battler] & STATUS3_SKY_DROPPED))
    {
        ...
        if (gBattleMons[otherSkyDropper].status2 & STATUS2_LOCK_CONFUSE && gBattleStruct->turnEffectsTracker != 24)
        {
            ...
            if (!(HasBattlerAbility(otherSkyDropper, ABILITY_OWN_TEMPO)
            ...
            if (gBattleMons[battler].status2 & STATUS2_UPROAR)
            {
                for (gBattlerAttacker = 0; gBattlerAttacker < gBattlersCount; gBattlerAttacker++)
                {
                    if ((gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
                     && !HasBattlerAbility(gBattlerAttacker, ABILITY_SOUNDPROOF))
                    {
        ...
        case ENDTURN_YAWN:  // yawn
            if (gStatuses3[battler] & STATUS3_YAWN)
            {
                ...
                if (!(gStatuses3[battler] & STATUS3_YAWN) && !(gBattleMons[battler].status1 & STATUS1_ANY)
                 ...
                 && !HasBattlerAbility(battler, ABILITY_INSOMNIA) && !UproarWakeUpCheck(battler)
    ...
    do
    {
        ...
        case CANCELLER_POWDER_MOVE:
            if ((gBattleMoves[gCurrentMove].powderMove) && (gBattlerAttacker != gBattlerTarget))
            {
                ...
                else if (B_POWDER_GRASS >= GEN_6 && HasBattlerAbility(gBattlerTarget, ABILITY_OVERCOAT))
                {
                    SetBattlerTriggeredAbility(gBattlerTarget, ABILITY_OVERCOAT);
                    effect = 1;
                }
        ...
        case CANCELLER_MULTIHIT_MOVES:
        {
            ...
            if (gBattleMoves[gCurrentMove].effect == EFFECT_MULTI_HIT)
            {
                ...
                if (gCurrentMove == MOVE_BARRAGE && IsBattlerWeatherAffected(gBattlerAttacker, B_WEATHER_SUN))
                {
                    gMultiHitCounter = 5;
                }
            ...
            if (isMultiHitMove
             && HasBattlerAbility(gBattlerAttacker, ABILITY_FAMILY_FEUD)
             && gDisableStructs[gBattlerAttacker].uniquePersistentStateActive)
            {
                gMultiHitCounter++;
                gDisableStructs[gBattlerAttacker].uniquePersistentStateActive = FALSE;
...
static const u16 sDreamSequenceMoves[] =
{
    MOVE_DREAM_EATER,
    MOVE_NIGHTMARE,
    MOVE_DISABLE,
...
static bool32 TryUseDreamSequenceCalledMove(u32 battler)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_DREAM_SEQUENCE);
    SetAtkCancellerForCalledMove();
...
static const u16 sDistortionMoves[] =
{
    MOVE_ROAR,
    MOVE_SCREECH,
    MOVE_SUPERSONIC,
...
static const u16 sVicejawMoves[] =
{
    MOVE_BITE,
    MOVE_CRUNCH,
    MOVE_FIRE_FANG,
...
static const u16 sAcidRefluxMoves[] =
{
    MOVE_ACID,
    MOVE_GASTRO_ACID,
    MOVE_BELCH,
...
static const u16 sWebTrapMoves[] =
{
    MOVE_STRING_SHOT,
    MOVE_SPIDER_WEB,
    MOVE_STICKY_WEB,
...
static bool32 TryUseDeliveryBagCalledMove(u32 battler)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_DELIVERY_BAG);
    SetAtkCancellerForCalledMove();
    gBattlerAttacker = gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static bool32 TryUseWishmakerCalledMove(u32 battler)
{
    ...
    if (target == battler)
    {
        if (move == MOVE_HELPING_HAND || !CanUseSelfExtraMove(battler))
            return FALSE;
    }
    else
    {
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_WISHMAKER);
    SetAtkCancellerForCalledMove();
    gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static const u16 sBloomBurstMoves[] =
{
    MOVE_PETAL_DANCE,
    MOVE_MAGICAL_LEAF,
    MOVE_LEECH_SEED,
...
static const u16 sStarmobileMoves[] =
{
    MOVE_BLAZING_TORQUE,
    MOVE_WICKED_TORQUE,
    MOVE_NOXIOUS_TORQUE,
...
static const u16 sBasaltShellMoves[] =
{
    MOVE_HARDEN,
    MOVE_SHELL_SMASH,
    MOVE_LAVA_PLUME,
...
static bool32 TryUseBasaltShellCalledMove(u32 battler, u32 target)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_BASALT_SHELL);
    SetAtkCancellerForCalledMove();
    gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static const u16 sBalloonBurstMoves[] =
{
    MOVE_GUST,
    MOVE_MINIMIZE,
    MOVE_TAILWIND,
...
static bool32 TryUseBalloonBurstCalledMove(u32 battler, u32 target)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_BALLOON_BURST);
    SetAtkCancellerForCalledMove();
    gBattlerAttacker = gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static const u16 sKeyringMoves[] =
{
    MOVE_DISABLE,
    MOVE_TORMENT,
    MOVE_ENCORE,
...
static bool32 TryUseKeyringCalledMove(u32 battler)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_KEYRING);
    SetAtkCancellerForCalledMove();
    gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static const u16 sWreckageMoves[] =
{
    MOVE_ANCHOR_SHOT,
    MOVE_WHIRLPOOL,
    MOVE_RAPID_SPIN,
...
static bool32 TryUseWreckageCalledMove(u32 battler, u32 target)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_WRECKAGE);
    SetAtkCancellerForCalledMove();
    gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static const u16 sAdaptiveForceMoves[] =
{
    MOVE_WEATHER_BALL,
    MOVE_TERRAIN_PULSE,
    MOVE_SECRET_POWER,
...
static bool32 TryUseAdaptiveForceCalledMove(u32 battler)
{
    ...
    gBattlerAttacker = battler;
    move = RandomElement(RNG_ROGUE_ADAPTIVE_FORCE, sAdaptiveForceMoves);

    for (i = 0; i < 2; i++, target ^= BIT_FLANK)
    {
        if (!CanUseExtraMove(battler, target))
            continue;

        SetBattlerTriggeredAbility(battler, ABILITY_ADAPTIVE_FORCE);
        SetAtkCancellerForCalledMove();
        gBattlerAbility = battler;
        gBattlerTarget = target;
        gCalledMove = move;
...
static const u16 sBagOfTricksMoves[] =
{
    MOVE_THIEF,
    MOVE_COVET,
    MOVE_SWITCHEROO,
...
static bool32 TryUseBagOfTricksCalledMove(u32 battler, u32 target)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_BAG_OF_TRICKS);
    SetAtkCancellerForCalledMove();
    gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static const u16 sFormationMoves[] =
{
...
static bool32 TryUseFormationCalledMove(u32 battler)
{
    u32 target = BATTLE_OPPOSITE(battler);
    u16 move;
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_FORMATION);
    SetAtkCancellerForCalledMove();
    gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
static bool32 TryUseFalseApplauseCalledMove(u32 battler, u32 target)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_FALSE_APPLAUSE);
    SetAtkCancellerForCalledMove();
    gBattlerAttacker = gBattlerAbility = battler;
    gBattlerTarget = target;
...
static const u16 sCapsaicinCrazeMoves[] =
{
    MOVE_LEAFAGE,
    MOVE_MAGICAL_LEAF,
    MOVE_FIRE_PUNCH,
...
static bool32 TryUseCapsaicinCrazeCalledMove(u32 battler, u32 target)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_CAPSAICIN_CRAZE);
    SetAtkCancellerForCalledMove();
    gBattlerAttacker = gBattlerAbility = battler;
    gBattlerTarget = target;
    ...
    gProtectStructs[battler].extraMoveUsed = TRUE;
    VarSet(VAR_EXTRA_MOVE_DAMAGE, 40);
    VarSet(VAR_TEMP_MOVEEFECT_CHANCE, TEMP_MOVEEFFECT_CLEAR);
...
static bool32 TryUseFuneralRiteCalledMove(u32 battler)
{
    ...
    SetBattlerTriggeredAbility(battler, ABILITY_FUNERAL_RITE);
    SetAtkCancellerForCalledMove();
    gBattlerAttacker = gBattlerAbility = gBattlerTarget = battler;
...
static bool32 TryUseVarietyActCalledMove(u32 battler)
{
    ...
    if (target >= gBattlersCount
     || !IsBattlerAlive(target)
     || (target != battler && GetBattlerSide(target) == GetBattlerSide(battler)))
        return FALSE;

    SetBattlerTriggeredAbility(battler, ABILITY_VARIETY_ACT);
    SetAtkCancellerForCalledMove();
    gBattlerAbility = battler;
    gBattlerTarget = target;
    gCalledMove = move;
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    if (caseID == ABILITYEFFECT_ON_SWITCHIN && special == 0)
    {
        ...
        if (HasBattlerAbility(battler, ABILITY_DIRTY_TRICKS) && !uniqueDone)
        {
            u32 opposingBattler = BATTLE_OPPOSITE(battler);
            u32 i;
        ...
        if (HasBattlerAbility(battler, ABILITY_DROP_OFF) && !uniqueDone)
        {
            u32 opposingBattler = BATTLE_OPPOSITE(battler);
            u32 i;
            ...
            for (i = 0; i < 2; i++, opposingBattler ^= BIT_FLANK)
            {
                if (!CanUseExtraMove(battler, opposingBattler) || !CanFling(battler))
                    continue;
        ...
        if (HasBattlerAbility(battler, ABILITY_BATTLECRY) && !uniqueDone)
        {
            ...
            for (i = 0; i < 2; i++, opposingBattler ^= BIT_FLANK)
            {
                ...
                SetBattlerTriggeredAbility(battler, ABILITY_BATTLECRY);
                SetAtkCancellerForCalledMove();
                gBattlerAttacker = gBattlerAbility = battler;
                gBattlerTarget = opposingBattler;
                gCalledMove = MOVE_GROWL;
                gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
                gProtectStructs[battler].extraMoveUsed = TRUE;
                gSpecialStatuses[battler].switchInUniqueAbilityDone = uniqueDone;
                gSpecialStatuses[battler].switchInAbilityDone = primaryDone;
        ...
        if (HasBattlerAbility(battler, ABILITY_HAUNTED_HARVEST) && !uniqueDone)
        {
            u32 opposingBattler = BATTLE_OPPOSITE(battler);
            u32 i;
            ...
            for (i = 0; i < 2; i++, opposingBattler ^= BIT_FLANK)
            {
                ...
                gBattlerAttacker = gBattlerAbility = battler;
                gBattlerTarget = opposingBattler;
                gCalledMove = MOVE_TRICK_OR_TREAT;
                gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
                gProtectStructs[battler].extraMoveUsed = TRUE;
                gSpecialStatuses[battler].switchInUniqueAbilityDone = uniqueDone;
                gSpecialStatuses[battler].switchInAbilityDone = primaryDone;
        ...
        if (HasBattlerAbility(battler, ABILITY_ADAPTIVE_FORCE)
         && !uniqueDone
          ...
          || (gFieldStatuses & STATUS_FIELD_TERRAIN_ANY)))
        {
            ...
            if (TryUseAdaptiveForceCalledMove(battler))
                return 1;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_FACTORY_LINE)
         && !uniqueDone
         && CountPartyMonsOfType(battler, TYPE_STEEL, TRUE) >= 2)
        {
            uniqueDone = TRUE;
            SetBattlerTriggeredAbility(battler, ABILITY_FACTORY_LINE);
            SetAtkCancellerForCalledMove();
            gBattlerAttacker = gBattlerAbility = battler;
            gBattlerTarget = battler;
            gCalledMove = MOVE_SHIFT_GEAR;
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
            gProtectStructs[battler].extraMoveUsed = TRUE;
            gSpecialStatuses[battler].switchInUniqueAbilityDone = uniqueDone;
            gSpecialStatuses[battler].switchInAbilityDone = primaryDone;
        ...
        if (HasBattlerAbility(battler, ABILITY_RAID_LEADER)
         && !uniqueDone
         && CountPartyMonsOfType(battler, TYPE_STEEL, TRUE) >= 2)
        {
            u32 opposingBattler = BATTLE_OPPOSITE(battler);
            ...
            for (i = 0; i < 2; i++, opposingBattler ^= BIT_FLANK)
            {
                ...
                SetBattlerTriggeredAbility(battler, ABILITY_RAID_LEADER);
                SetAtkCancellerForCalledMove();
                gBattlerAttacker = gBattlerAbility = battler;
                gBattlerTarget = opposingBattler;
        ...
        if (HasBattlerAbility(battler, ABILITY_TUMBLEWEED) && !uniqueDone)
        {
            bool32 hazardsCleared = ClearSideEntryHazards(GetBattlerSide(battler));
        ...
        if (HasBattlerAbility(battler, ABILITY_FRESHLY_BAKED) && !uniqueDone)
        {
            bool32 canHeal = !BATTLER_MAX_HP(battler) && !(gStatuses3[battler] & STATUS3_HEAL_BLOCK);
            bool32 canSetMist = !(gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_MIST);

            if (canHeal || canSetMist)
            {
                uniqueDone = TRUE;
                gSpecialStatuses[battler].switchInUniqueAbilityDone = uniqueDone;
                gSpecialStatuses[battler].switchInAbilityDone = primaryDone;
                SetBattlerTriggeredAbility(battler, ABILITY_FRESHLY_BAKED);
                gBattlerAttacker = battler;
                ...
                if (canHeal && canSetMist)
                    BattleScriptPushCursorAndCallback(BattleScript_FreshlyBakedHealMistActivates);
                else if (canHeal)
                    BattleScriptPushCursorAndCallback(BattleScript_FreshlyBakedHealActivates);
                else
                    BattleScriptPushCursorAndCallback(BattleScript_FreshlyBakedMistActivates);
         ...
         && gSideTimers[GetBattlerSide(battler)].retaliateTimer == 1)
        {
            ...
            if (TryChangeBattleWeather(battler, ENUM_WEATHER_ECLIPSE, TRUE))
            {
                SetBattlerTriggeredAbility(battler, ABILITY_LAST_FAREWELL);
                BattleScriptPushCursorAndCallback(BattleScript_OmenActivates);
                return 1;
            }
            else if (gBattleWeather & B_WEATHER_PRIMAL_ANY && WEATHER_HAS_EFFECT)
    ...
    if (special)
        SetBattlerTriggeredAbility(battler, special);
    else
    ...
    case ABILITYEFFECT_ON_SWITCHIN: // 0
        ...
        case ABILITY_HEARTWING:
            ...
            break;
        case ABILITY_LIFE_BLOSSOM:
            if (CountFaintedPartyAllies(battler) == 0
             && TryChangeBattleTerrain(battler, STATUS_FIELD_MISTY_TERRAIN, &gFieldTimers.terrainTimer))
            {
                BattleScriptPushCursorAndCallback(BattleScript_MistySurgeActivates);
                effect++;
        ...
        case ABILITY_ROYAL_CHARM:
            if (!gSpecialStatuses[battler].switchInAbilityDone)
            {
                gSpecialStatuses[battler].switchInAbilityDone = TRUE;
                gBattlerAttacker = battler;
                BattleScriptPushCursorAndCallback(BattleScript_RoyalCharmActivates);
                effect++;
            }
            break;
    ...
    case ABILITYEFFECT_ENDTURN: // 1
        if (gBattleMons[battler].hp != 0)
        {
            ...
            if (HasBattlerAbility(battler, ABILITY_DREAD)
             && IsBattlerWeatherAffected(battler, B_WEATHER_RAIN))
            {
                u32 target = GetBattlerAtPosition(BATTLE_OPPOSITE(GetBattlerPosition(battler)));
                u32 validToLower = 0;
                ...
                if (target < gBattlersCount)
                {
                    ...
                    {
                        if (CompareStat(target, stat, MIN_STAT_STAGE, CMP_GREATER_THAN))
                            validToLower |= gBitTable[stat];
                    }
                    ...
                    if (validToLower != 0)
                    {
                        do
                        {
                            stat = RandomUniform(RNG_ROGUE_DREAD, STAT_ATK, STAT_SPDEF);
                        } while (!(validToLower & gBitTable[stat]));

                        SetBattlerTriggeredAbility(battler, ABILITY_DREAD);
                        gBattlerTarget = target;
                        SET_STATCHANGER(stat, 1, TRUE);
                        BattleScriptPushCursorAndCallback(BattleScript_FrightmareActivates);
                        effect++;
                    }
                }
...

            {
                ...
                if (HasBattlerAbility(battler, ABILITY_SNOWSTORM)
                 && !gProtectStructs[battler].uniqueAbilityTriggeredThisTurn
                 && TryGetOpposingExtraMoveTarget(battler, &target))
                {
                    SetBattlerTriggeredAbility(battler, ABILITY_SNOWSTORM);
                    SetAtkCancellerForCalledMove();
                    gBattlerAttacker = gBattlerAbility = battler;
                    gBattlerTarget = target;
                    gCalledMove = MOVE_POWDER_SNOW;
                    gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
                    gProtectStructs[battler].extraMoveUsed = TRUE;
                    gProtectStructs[battler].uniqueAbilityTriggeredThisTurn = TRUE;
            ...
            if (HasBattlerAbility(battler, ABILITY_IONIZE))
            {
                ...
                for (i = 0; i < gBattlersCount; i++)
                {
                    gIonizeDmgByBattler[i] = 0;

                    if (!IsBattlerAlive(i))
                        continue;
                    if (IS_BATTLER_OF_TYPE(i, TYPE_ELECTRIC) || IS_BATTLER_OF_TYPE(i, TYPE_GROUND))
                        continue;
                    if (HasBattlerAbility(i, ABILITY_MAGIC_GUARD))
                        continue;

                    gIonizeDmgByBattler[i] = GetNonDynamaxMaxHP(i) / 16;
                    if (gIonizeDmgByBattler[i] == 0)
                        gIonizeDmgByBattler[i] = 1;
                    shouldTrigger = TRUE;
                }

                if (shouldTrigger)
                {
                    SetBattlerTriggeredAbility(battler, ABILITY_IONIZE);
                    BattleScriptPushCursorAndCallback(BattleScript_IonizeActivates);
                    effect++;
                }
            }
    ...
    case ABILITYEFFECT_MOVES_BLOCK: // 2
        {
            ...
            else if (HasBattlerAbility(gBattlerTarget, ABILITY_GOOD_AS_GOLD)
                  && IS_MOVE_STATUS(gCurrentMove)
                  && !(moveTarget & MOVE_TARGET_USER)
                  && !(moveTarget & MOVE_TARGET_OPPONENTS_FIELD)
                  && !(moveTarget & MOVE_TARGET_ALL_BATTLERS))
            {
                SetBattlerTriggeredAbility(gBattlerTarget, ABILITY_GOOD_AS_GOLD);
...
{
    if (HasBattlerAbility(battler, ABILITY_VOLT_ABSORB))
        triggeringAbility = ABILITY_VOLT_ABSORB;
...
{
    if (HasBattlerAbility(battler, ABILITY_WATER_ABSORB))
        triggeringAbility = ABILITY_WATER_ABSORB;
    else if (HasBattlerAbility(battler, ABILITY_DRY_SKIN))
...
{
    triggeringAbility = ABILITY_FLASH_FIRE;
...
else if (moveType == TYPE_FIRE && HasBattlerAbility(battler, ABILITY_WELL_BAKED_BODY))
{
    triggeringAbility = ABILITY_WELL_BAKED_BODY;
    effect = 2;
    statId = STAT_DEF;
    statAmount = 2;
}
else if (gBattleMoves[gCurrentMove].windMove
    && !(GetBattlerMoveTargetType(gBattlerAttacker, gCurrentMove) & MOVE_TARGET_USER)
    && HasBattlerAbility(battler, ABILITY_WIND_RIDER))
{
    triggeringAbility = ABILITY_WIND_RIDER;
    effect = 2;
    statId = STAT_ATK;
}
else if (moveType == TYPE_GROUND && HasBattlerAbility(battler, ABILITY_EARTH_EATER))
{
    triggeringAbility = ABILITY_EARTH_EATER;
}

if (triggeringAbility != ABILITY_NONE && effect == 0)
    effect = 1;

if (triggeringAbility != ABILITY_NONE)
    SetBattlerTriggeredAbility(battler, triggeringAbility);
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
    {
        ...
        case ABILITY_EFFECT_SPORE:
            if (!IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_GRASS)
             && !HasBattlerAbility(gBattlerAttacker, ABILITY_OVERCOAT)
        ...
        case ABILITY_CUTE_CHARM:
            if (!(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
             ...
             && !HasBattlerAbility(gBattlerAttacker, ABILITY_OBLIVIOUS)
             && IsMoveMakingContact(move, gBattlerAttacker)
             && !IsAbilityOnSide(gBattlerAttacker, ABILITY_AROMA_VEIL))
            {
        ...
        case ABILITY_GULP_MISSILE:
             ...
             && IsBattlerAlive(battler))
            {
                ...
                if (gBattleMons[gBattlerTarget].species == SPECIES_CRAMORANT_GORGING)
                {
                    ...
                    if (!HasBattlerAbility(gBattlerAttacker, ABILITY_MAGIC_GUARD))
                    {
                        gBattleMoveDamage = GetNonDynamaxMaxHP(gBattlerAttacker) / 4;
                        if (gBattleMoveDamage == 0)
                            gBattleMoveDamage = 1;
        ...
        {
            SetBattlerTriggeredAbility(battler, ABILITY_BARBED_MONSOON);
            gBattleScripting.moveEffect = MOVE_EFFECT_AFFECTS_USER | MOVE_EFFECT_POISON;
            PREPARE_ABILITY_BUFFER(gBattleTextBuff1, ABILITY_BARBED_MONSOON);
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_AbilityStatusEffect;
            gHitMarker |= HITMARKER_STATUS_ABILITY_EFFECT;
            effect++;
        ...
        if (HasBattlerAbility(battler, ABILITY_FAMILY_FEUD)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && gBattleMons[moveEndAttacker].hp != 0
         && !gProtectStructs[moveEndAttacker].confusionSelfDmg
         && BATTLER_TURN_DAMAGED(moveEndTarget)
        ...
        if (HasBattlerAbility(battler, ABILITY_STARMOBILE)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && !gProtectStructs[moveEndAttacker].confusionSelfDmg
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && HadMoreThanHalfHpNowHasLess(battler)
        ...
        if (HasBattlerAbility(battler, ABILITY_BASALT_SHELL)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && !gProtectStructs[moveEndAttacker].confusionSelfDmg
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && (moveType == TYPE_WATER || moveType == TYPE_ICE || moveType == TYPE_ROCK)
         && IsFinalMultiHitStrike()
         && TryUseBasaltShellCalledMove(battler, moveEndAttacker))
        {
            effect++;
        }

        if (HasBattlerAbility(battler, ABILITY_WRECKAGE)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && (gMoveResultFlags & MOVE_RESULT_NOT_VERY_EFFECTIVE)
         && !gProtectStructs[moveEndAttacker].confusionSelfDmg
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && IsFinalMultiHitStrike()
         && TryUseWreckageCalledMove(battler, moveEndAttacker))
        {
            effect++;
        }
         ...
         && CanUseExtraMove(battler, moveEndAttacker))
        {
            ...
            gBattlerAttacker = gBattlerAbility = battler;
            gBattlerTarget = moveEndAttacker;
            gCalledMove = MOVE_FORESTS_CURSE;
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
            gProtectStructs[battler].extraMoveUsed = TRUE;
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_AbilityUsesCalledMove;
         ...
         && IsMoveMakingContact(move, moveEndAttacker))
        {
            ...
            if (damagingMoveCount == 0)
            {
                u32 triggeredAbility = ABILITY_RAZOR_CARAPACE;
                u32 roughSkinDamage;
                u32 razorDamage;

                SetBattlerTriggeredAbility(battler, ABILITY_RAZOR_CARAPACE);
                roughSkinDamage = GetNonDynamaxMaxHP(moveEndAttacker) / (B_ROUGH_SKIN_DMG >= GEN_4 ? 8 : 16);
                if (roughSkinDamage == 0)
                    roughSkinDamage = 1;
                razorDamage = GetNonDynamaxMaxHP(moveEndAttacker) / 16;
                if (razorDamage == 0)
                    razorDamage = 1;
                gBattleMoveDamage = roughSkinDamage + razorDamage;
                PREPARE_ABILITY_BUFFER(gBattleTextBuff1, triggeredAbility);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_RoughSkinActivates;
                effect++;
            }
        ...
        if (HasBattlerAbility(battler, ABILITY_RALLY)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && battler == moveEndTarget
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && HadMoreThanHalfHpNowHasLess(battler)
         ...
         && !(gBattleStruct->uniqueAbilityUsed[GetBattlerSide(battler)] & gBitTable[gBattlerPartyIndexes[battler]]))
        {
            ...
            {
                gBattleStruct->uniqueAbilityUsed[GetBattlerSide(battler)] |= gBitTable[gBattlerPartyIndexes[battler]];
                SetBattlerTriggeredAbility(battler, ABILITY_RALLY);
                gBattlerAttacker = gBattlerAbility = gEffectBattler = battler;
                gProtectStructs[battler].uniqueAbilityTriggeredThisTurn = TRUE;
                ...
                if (canHeal)
                {
                    ...
                    gBattleMoveDamage *= -1;
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_RallyHealActivates;
                }
                else
                {
                    BattleScriptPushCursor();
                    gBattlescriptCurrInstr = BattleScript_RallyCureOnlyActivates;
                }
                effect++;
            }
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_BALLOON_BURST)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && moveEndAttacker != battler
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && IsBattlerAlive(battler)
         ...
         && !(TestSheerForceFlag(gBattlerAttacker, gCurrentMove))
         && !(gBattleStruct->uniqueAbilityUsed[GetBattlerSide(battler)] & gBitTable[gBattlerPartyIndexes[battler]])
         && TryUseBalloonBurstCalledMove(battler, moveEndAttacker))
        {
            effect++;
        }

        if (HasBattlerAbility(battler, ABILITY_PRESSURE_VALVE)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && moveEndAttacker != battler
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && IsBattlerAlive(battler)
         ...
         && (moveType == TYPE_FIRE || moveType == TYPE_WATER))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_PRESSURE_VALVE);
            gEffectBattler = battler;
            gBattlerAbility = battler;
            gDisableStructs[battler].uniquePersistentStateActive = TRUE;
            BattleScriptPushCursor();
        ...
        if (HasBattlerAbility(battler, ABILITY_CURSED_TABLET)
         && battler == moveEndTarget
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && (gMoveResultFlags & MOVE_RESULT_SUPER_EFFECTIVE)
         ...
         && CanUseExtraMove(battler, moveEndAttacker))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_CURSED_TABLET);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = battler;
            gBattlerTarget = moveEndAttacker;
            gCalledMove = MOVE_RUINATION;
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
            gProtectStructs[battler].extraMoveUsed = TRUE;
            gBattleStruct->uniqueAbilityUsed[GetBattlerSide(battler)] |= gBitTable[gBattlerPartyIndexes[battler]];
            BattleScriptPushCursor();
        ...
        if (HasBattlerAbility(battler, ABILITY_FACET_CUT)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && BATTLER_TURN_DAMAGED(moveEndTarget)
         && (gMoveResultFlags & MOVE_RESULT_SUPER_EFFECTIVE)
         && IsFinalMultiHitStrike()
         ...
         && CanUseSelfExtraMove(battler))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_FACET_CUT);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = gBattlerTarget = battler;
            gCalledMove = IS_MOVE_PHYSICAL(move) ? MOVE_REFLECT : MOVE_LIGHT_SCREEN;
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
            gProtectStructs[battler].extraMoveUsed = TRUE;
            gDisableStructs[battler].uniqueOncePerSwitchInUsed = TRUE;
            BattleScriptPushCursor();
    ...
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
    {
        ...
        if (HasBattlerAbility(battler, ABILITY_HECKLER)
         && IsBattlerAlive(battler)
         && (gBattleMoves[move].soundMove || IsMoveMakingContact(move, battler))
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && !gProtectStructs[battler].confusionSelfDmg
         && IsFinalMultiHitStrike()
         ...
         && CanUseExtraMove(battler, gBattlerTarget))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_HECKLER);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = battler;
            gCalledMove = MOVE_TAUNT;
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
        ...
        if (HasBattlerAbility(battler, ABILITY_BAD_MOUTH)
         && IsBattlerAlive(battler)
         ...
         && CanUseExtraMove(battler, gBattlerTarget))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_BAD_MOUTH);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = battler;
            gCalledMove = MOVE_PARTING_SHOT;
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
            gProtectStructs[battler].extraMoveUsed = TRUE;
            gDisableStructs[battler].uniqueOncePerSwitchInUsed = TRUE;
            BattleScriptPushCursor();
        ...
        if (HasBattlerAbility(battler, ABILITY_RUMBLE_ROLL)
         && IsBattlerAlive(battler)
         && moveType == TYPE_GROUND
         && !IS_MOVE_STATUS(move)
         && DidMoveSucceedForMoveEndEffects(battler)
        ...
        {
            bool32 hazardsCleared = ClearSideEntryHazards(GetBattlerSide(battler));
            bool32 canRaiseDef = CompareStat(battler, STAT_DEF, MAX_STAT_STAGE, CMP_LESS_THAN);
        ...
        if (HasBattlerAbility(battler, ABILITY_FALSE_APPLAUSE)
         && IsBattlerAlive(battler)
         && IsBattlerAlive(gBattlerTarget)
         && GetBattlerSide(battler) != GetBattlerSide(gBattlerTarget)
         && IsMoveMakingContact(move, battler)
         ...
         && !IsDynamaxed(gBattlerTarget)
         && CanUseSelfExtraMoveAfterMoveEndDamage(battler, move)
         && TryUseFalseApplauseCalledMove(battler, gBattlerTarget))
            effect++;

        if (HasBattlerAbility(battler, ABILITY_DIRGE)
         && IsBattlerAlive(battler)
         && IsBattlerAlive(gBattlerTarget)
         && GetBattlerSide(battler) != GetBattlerSide(gBattlerTarget)
         && gBattleMoves[move].soundMove
        ...
        if (HasBattlerAbility(battler, ABILITY_SHOWSTOPPER)
         && IsBattlerAlive(battler)
         && IsBattlerAlive(gBattlerTarget)
         && GetBattlerSide(battler) != GetBattlerSide(gBattlerTarget)
         ...
         && (CompareStat(gBattlerTarget, STAT_SPEED, MIN_STAT_STAGE, CMP_GREATER_THAN)
          || GetBattlerAbility(gBattlerTarget) == ABILITY_MIRROR_ARMOR))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_SHOWSTOPPER);
            gBattleScripting.moveEffect = MOVE_EFFECT_SPD_MINUS_1;
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_AbilityStatusEffect;
            gHitMarker |= HITMARKER_STATUS_ABILITY_EFFECT;
        ...
        if (HasBattlerAbility(battler, ABILITY_PHOTOKERATITIS)
         && IsBattlerAlive(battler)
         && IsBattlerAlive(gBattlerTarget)
         && GetBattlerSide(battler) != GetBattlerSide(gBattlerTarget)
         && IsBattlerWeatherAffected(battler, B_WEATHER_SNOW)
         ...
         && (CompareStat(gBattlerTarget, STAT_ACC, MIN_STAT_STAGE, CMP_GREATER_THAN)
          || GetBattlerAbility(gBattlerTarget) == ABILITY_MIRROR_ARMOR))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_PHOTOKERATITIS);
        ...
        if (HasBattlerAbility(battler, ABILITY_CAPSAICIN_CRAZE)
         && IsBattlerAlive(battler)
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gMoveResultFlags & (MOVE_RESULT_NO_EFFECT | MOVE_RESULT_MISSED | MOVE_RESULT_FAILED | MOVE_RESULT_DOESNT_AFFECT_FOE))
         && !(gBattleStruct->lastMoveFailed & gBitTable[battler])
         ...
         && IsFinalMultiHitStrike()
         && CanUseSelfExtraMoveAfterMoveEndDamage(battler, move)
         && TryUseCapsaicinCrazeCalledMove(battler, gBattlerTarget))
        {
            effect++;
        }

        if (HasBattlerAbility(battler, ABILITY_FUNERAL_RITE)
         && IsBattlerAlive(battler)
         ...
         && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
         && CanUseSelfExtraMoveAfterMoveEndDamage(battler, move)
         && TryUseFuneralRiteCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_LIVEWIRE_NEST)
         && IsBattlerAlive(battler)
         && (moveType == TYPE_BUG || moveType == TYPE_ELECTRIC)
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gMoveResultFlags & (MOVE_RESULT_NO_EFFECT | MOVE_RESULT_MISSED | MOVE_RESULT_FAILED | MOVE_RESULT_DOESNT_AFFECT_FOE))
         && !(gBattleStruct->lastMoveFailed & gBitTable[battler])
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         ...
         && TryChangeBattleTerrain(battler, STATUS_FIELD_INFESTED_TERRAIN, &gFieldTimers.terrainTimer))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_LIVEWIRE_NEST);
            gDisableStructs[battler].uniqueOncePerSwitchInUsed = TRUE;
            BattleScriptPushCursor();
            gBattlescriptCurrInstr = BattleScript_CallAlliesActivates;
            effect++;
        ...
        if (HasBattlerAbility(battler, ABILITY_CHEFS_PICK)
         && IsBattlerAlive(battler)
         ...
         && !(gBattleStruct->uniqueAbilityUsed[GetBattlerSide(battler)] & gBitTable[gBattlerPartyIndexes[battler]]))
        {
            ...
            if (IsBattlerAlive(partnerBattler) && CanUseExtraMove(battler, partnerBattler))
            {
                SetBattlerTriggeredAbility(battler, ABILITY_CHEFS_PICK);
                SetAtkCancellerForCalledMove();
                gBattlerAttacker = gBattlerAbility = battler;
                gBattlerTarget = partnerBattler;
                gCalledMove = MOVE_HELPING_HAND;
            ...
            else if (CanUseSelfExtraMove(battler))
            {
                SetBattlerTriggeredAbility(battler, ABILITY_CHEFS_PICK);
                SetAtkCancellerForCalledMove();
                gBattlerAttacker = gBattlerAbility = gBattlerTarget = battler;
        ...
        if (HasBattlerAbility(battler, ABILITY_DUSTSTORM)
         && IsBattlerAlive(battler)
         && gDisableStructs[battler].uniquePersistentStateActive
         && !gProtectStructs[battler].confusionSelfDmg
         && !(gBattleMons[battler].status1 & STATUS1_SLEEP)
         && !(gBattleMons[battler].status1 & STATUS1_FREEZE))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_DUSTSTORM);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = gBattlerTarget = battler;
        ...
        if ((HasBattlerAbility(battler, ABILITY_OPENING_VERSE)
          || HasBattlerAbility(battler, ABILITY_FINAL_STEP))
         && IsBattlerAlive(battler)
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gMoveResultFlags & (MOVE_RESULT_NO_EFFECT | MOVE_RESULT_MISSED | MOVE_RESULT_FAILED | MOVE_RESULT_DOESNT_AFFECT_FOE))
         && !(gBattleStruct->lastMoveFailed & gBitTable[battler])
         ...
         && IsFinalMultiHitStrike())
        {
            bool32 consumedVerse = FALSE;

            if (GetMeloettaVerseState(battler) == UNIQUE_VERSE_OPENING
             && IsMoveMakingContact(move, battler)
             && !IS_MOVE_STATUS(move)
             && CompareStat(battler, STAT_SPEED, MAX_STAT_STAGE, CMP_LESS_THAN))
            {
                SetBattlerTriggeredAbility(battler, ABILITY_OPENING_VERSE);
                SET_STATCHANGER(STAT_SPEED, 1, FALSE);
                PREPARE_STAT_BUFFER(gBattleTextBuff1, STAT_SPEED);
                gBattlerAttacker = battler;
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_AttackerAbilityStatRaise;
                SetMeloettaVerseState(battler, UNIQUE_VERSE_NONE);
                consumedVerse = TRUE;
                effect++;
            }
            ...
            if (!consumedVerse)
            {
                if (HasBattlerAbility(battler, ABILITY_OPENING_VERSE)
                 && gBattleMoves[move].soundMove)
                    SetMeloettaVerseState(battler, UNIQUE_VERSE_OPENING);
                else if (HasBattlerAbility(battler, ABILITY_FINAL_STEP)
                      && IsMoveMakingContact(move, battler)
                      && !IS_MOVE_STATUS(move))
                    SetMeloettaVerseState(battler, UNIQUE_VERSE_FINAL);
            }
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_FEEDING_FRENZY)
         ...
         && TARGET_TURN_DAMAGED
         && GetBattlerSide(battler) != GetBattlerSide(gBattlerTarget)
         && (gBattleStruct->hpBefore[gBattlerTarget] * 2) <= gBattleMons[gBattlerTarget].maxHP
         && IsFinalMultiHitStrike()
         && CanUseExtraMove(battler, gBattlerTarget))
        {
        ...
        if (HasBattlerAbility(battler, ABILITY_FINISHER)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && !IS_MOVE_STATUS(move)
         && TARGET_TURN_DAMAGED
         && GetBattlerSide(battler) != GetBattlerSide(gBattlerTarget)
         && HadAtLeastQuarterHpNowHasLess(gBattlerTarget)
         && IsFinalMultiHitStrike()
         && CanUseExtraMove(battler, gBattlerTarget))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_FINISHER);
            gTempMove = gCurrentMove;
            gCurrentMove = MOVE_CRUNCH;
            gProtectStructs[battler].extraMoveUsed = TRUE;
            VarSet(VAR_EXTRA_MOVE_DAMAGE, 40);
            VarSet(VAR_TEMP_MOVEEFECT_CHANCE, 0);
            VarSet(VAR_TEMP_MOVEEFFECT, 0);
        ...
        if (HasBattlerAbility(battler, ABILITY_DREAM_SEQUENCE)
         && move == MOVE_HYPNOSIS
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && IsFinalMultiHitStrike()
         && TryUseDreamSequenceCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_BLOOM_BURST)
         ...
         && !gProtectStructs[battler].confusionSelfDmg
         && IsFinalMultiHitStrike()
         && TryUseBloomBurstCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_FLOWING_LESSON)
         && IsBattlerAlive(battler)
         && moveType == TYPE_WATER
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
        ...
        if (HasBattlerAbility(battler, ABILITY_RESOLUTE_BLADE)
         && IsBattlerAlive(battler)
         && moveType == TYPE_FIGHTING
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
        ...
        if (HasBattlerAbility(battler, ABILITY_RIFT)
         && gDisableStructs[battler].uniquePersistentStateActive
         && IsFinalMultiHitStrike())
        {
            gDisableStructs[battler].uniquePersistentStateActive = FALSE;
        ...
        if (HasBattlerAbility(battler, ABILITY_TERRITORIAL)
         ...
         && IsFinalMultiHitStrike())
        {
            SetBattlerTriggeredAbility(battler, ABILITY_TERRITORIAL);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = battler;
            gBattlerTarget = battler;
            gCalledMove = MOVE_FOCUS_ENERGY;
        ...
        if (HasBattlerAbility(battler, ABILITY_VARIETY_ACT)
         ...
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && IsFinalMultiHitStrike()
         && TryUseVarietyActCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_KEYRING)
         ...
         && IsFinalMultiHitStrike()
         && !gDisableStructs[battler].uniqueOncePerSwitchInUsed
         && TryUseKeyringCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_SWITCHBOARD)
         && move == MOVE_NUZZLE
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && IsFinalMultiHitStrike()
         ...
         && CanUseExtraMove(battler, gBattlerTarget))
        {
            SetBattlerTriggeredAbility(battler, ABILITY_SWITCHBOARD);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = battler;
            gCalledMove = MOVE_ENCORE;
            gHitMarker &= ~HITMARKER_ATTACKSTRING_PRINTED;
        ...
        if (HasBattlerAbility(battler, ABILITY_HEALING_CURRENT)
         && IsHealingMove(move)
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && IsFinalMultiHitStrike()
         && IsDoubleBattle())
        {
            ...
            if (IsBattlerAlive(partner) && GetBattlerSide(partner) == GetBattlerSide(battler))
            {
                if (!BATTLER_MAX_HP(partner))
                {
                    s32 healAmount = max(1, GetNonDynamaxMaxHP(partner) / 8);

                    gBattleMons[partner].hp += healAmount;
                    if (gBattleMons[partner].hp > gBattleMons[partner].maxHP)
                        gBattleMons[partner].hp = gBattleMons[partner].maxHP;

                    BtlController_EmitSetMonData(partner, BUFFER_A, REQUEST_HP_BATTLE, 0, sizeof(gBattleMons[partner].hp), &gBattleMons[partner].hp);
                    MarkBattlerForControllerExec(partner);
                    triggered = TRUE;
                }

                if (gBattleMons[partner].status1 & STATUS1_BURN)
                {
                    gBattleMons[partner].status1 &= ~STATUS1_BURN;
                    BtlController_EmitSetMonData(partner, BUFFER_A, REQUEST_STATUS_BATTLE, 0, sizeof(gBattleMons[partner].status1), &gBattleMons[partner].status1);
                    MarkBattlerForControllerExec(partner);
                    triggered = TRUE;
                }
            }
            ...
            if (triggered)
            {
                SetBattlerTriggeredAbility(battler, ABILITY_HEALING_CURRENT);
                gBattlerAttacker = battler;
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_AbilityPopupReturn;
                effect++;
        ...
        if (HasBattlerAbility(battler, ABILITY_BAG_OF_TRICKS)
         && moveType == TYPE_DARK
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && IsFinalMultiHitStrike()
         && !gDisableStructs[battler].uniqueOncePerSwitchInUsed
         && TryUseBagOfTricksCalledMove(battler, gBattlerTarget))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_STORYBOARD)
         ...
         && !gDisableStructs[battler].uniqueOncePerSwitchInUsed)
        {
            SetBattlerTriggeredAbility(battler, ABILITY_STORYBOARD);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = battler;
        ...
        if (HasBattlerAbility(battler, ABILITY_FORMATION)
         && move == MOVE_NO_RETREAT
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && !gProtectStructs[battler].confusionSelfDmg
         && IsFinalMultiHitStrike()
         && TryUseFormationCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_WISHMAKER)
         ...
         && !gProtectStructs[battler].extraMoveUsed
         && IsFinalMultiHitStrike()
         && TryUseWishmakerCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_FLOWER_RITUAL)
         && IS_MOVE_STATUS(move)
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && IsBattlerAlive(battler)
         ...
         && !gProtectStructs[battler].extraMoveUsed
         && IsFinalMultiHitStrike()
         && CountPartyMonsOfType(battler, TYPE_FAIRY, TRUE) >= 3)
        {
            SetBattlerTriggeredAbility(battler, ABILITY_FLOWER_RITUAL);
            gBattleStruct->atkCancellerTracker = 0;
            gBattlerAttacker = gBattlerAbility = battler;
            gBattlerTarget = battler;
        ...
        if (HasBattlerAbility(battler, ABILITY_MAELSTROM)
         && (move == MOVE_RECOVER || move == MOVE_STRENGTH_SAP)
         && DidMoveSucceedForMoveEndEffects(battler)
         && !(gHitMarker & HITMARKER_UNABLE_TO_USE_MOVE)
         && !gProtectStructs[gBattlerAttacker].confusionSelfDmg
         && IsFinalMultiHitStrike())
        {
            ...
            if (target < gBattlersCount)
            {
                SetBattlerTriggeredAbility(battler, ABILITY_MAELSTROM);
                gBattleStruct->atkCancellerTracker = 0;
                gBattlerAttacker = gBattlerAbility = battler;
                gBattlerTarget = target;
                gCalledMove = MOVE_WHIRLPOOL;
        ...
        if (HasBattlerAbility(battler, ABILITY_DISTORTION)
         ...
         && !gProtectStructs[battler].confusionSelfDmg
         && IsFinalMultiHitStrike()
         && TryUseDistortionCalledMove(battler))
        {
            effect++;
        }
        ...
        if (HasBattlerAbility(battler, ABILITY_REGAL_PRESSURE)
         && IsBattlerAlive(battler)
         && IsBattlerAlive(gBattlerTarget)
         && GetBattlerSide(battler) != GetBattlerSide(gBattlerTarget)
         && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
         ...
         && AreBattlersOfOppositeGender(battler, gBattlerTarget))
        {
            u32 highestStat = GetHighestStatId(gBattlerTarget);

            if (CompareStat(gBattlerTarget, highestStat, MIN_STAT_STAGE, CMP_GREATER_THAN)
             || GetBattlerAbility(gBattlerTarget) == ABILITY_MIRROR_ARMOR)
            {
                SetBattlerTriggeredAbility(battler, ABILITY_REGAL_PRESSURE);
                switch (highestStat)
                {
                case STAT_ATK:
                    gBattleScripting.moveEffect = MOVE_EFFECT_ATK_MINUS_1;
        ...
        {
            gDisableStructs[battler].uniquePersistentStateActive = FALSE;
            if (CompareStat(gBattlerTarget, STAT_DEF, MIN_STAT_STAGE, CMP_GREATER_THAN)
             || GetBattlerAbility(gBattlerTarget) == ABILITY_MIRROR_ARMOR)
            {
                SetBattlerTriggeredAbility(battler, ABILITY_FLOWING_LESSON);
                gBattleScripting.moveEffect = MOVE_EFFECT_DEF_MINUS_1;
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_AbilityStatusEffect;
                gHitMarker |= HITMARKER_STATUS_ABILITY_EFFECT;
        ...
        {
            gDisableStructs[battler].uniquePersistentStateActive = FALSE;
            if (CompareStat(gBattlerTarget, STAT_SPDEF, MIN_STAT_STAGE, CMP_GREATER_THAN)
             || GetBattlerAbility(gBattlerTarget) == ABILITY_MIRROR_ARMOR)
            {
                SetBattlerTriggeredAbility(battler, ABILITY_RESOLUTE_BLADE);
                gBattleScripting.moveEffect = MOVE_EFFECT_SP_DEF_MINUS_1;
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_AbilityStatusEffect;
                gHitMarker |= HITMARKER_STATUS_ABILITY_EFFECT;
    ...
    case ABILITYEFFECT_IMMUNITY: // 5
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            ...
            u32 triggeredAbility = ABILITY_NONE;
            ...
            {
                StringCopy(gBattleTextBuff1, gStatusConditionString_PoisonJpn);
                triggeredAbility = HasBattlerAbilityIgnoreMoldBreaker(battler, ABILITY_PASTEL_VEIL) ? ABILITY_PASTEL_VEIL : ABILITY_IMMUNITY;
                effect = 1;
            }
            ...
            case ABILITY_OWN_TEMPO:
                if (gBattleMons[battler].status2 & STATUS2_CONFUSION)
                {
                    ...
                    triggeredAbility = ABILITY_OWN_TEMPO;
                    effect = 2;
                }
                break;
            case ABILITY_LIMBER:
                ...
                {
                    StringCopy(gBattleTextBuff1, gStatusConditionString_ParalysisJpn);
                    triggeredAbility = ABILITY_LIMBER;
                    effect = 1;
                }
                break;
            ...
            case ABILITY_VITAL_SPIRIT:
                if (gBattleMons[battler].status1 & STATUS1_SLEEP)
                {
                    gBattleMons[battler].status2 &= ~STATUS2_NIGHTMARE;
                    StringCopy(gBattleTextBuff1, gStatusConditionString_SleepJpn);
                    triggeredAbility = battlerAbility;
                    effect = 1;
                }
                break;
            case ABILITY_WATER_VEIL:
            ...
            case ABILITY_MAGMA_ARMOR:
                ...
                {
                    StringCopy(gBattleTextBuff1, gStatusConditionString_IceJpn);
                    triggeredAbility = ABILITY_MAGMA_ARMOR;
                    effect = 1;
                }
                break;
            case ABILITY_OBLIVIOUS:
                ...
                {
                    triggeredAbility = ABILITY_OBLIVIOUS;
                    effect = 3;
                }
            ...
            if (effect != 0)
            {
                ...
                gBattleScripting.battler = battler;
                SetBattlerTriggeredAbility(battler, triggeredAbility);
                BtlController_EmitSetMonData(battler, BUFFER_A, REQUEST_STATUS_BATTLE, 0, 4, &gBattleMons[battler].status1);
                MarkBattlerForControllerExec(battler);
                return effect;
            }
...
static bool32 IsBattlerAbilitySuppressedByMoldBreaker(u32 battler, u32 ability)
{
    ...
    return ability < ABILITIES_COUNT
        && sAbilitiesAffectedByMoldBreaker[ability]
...
bool32 IsBattlerTerrainAffected(u32 battler, u32 terrainFlag)
{
    if (!(gFieldStatuses & terrainFlag))
        return FALSE;
    else if (HasBattlerAbility(battler, ABILITY_CLUELESS))
        return FALSE;
...
bool32 HasEnoughHpToEatBerry(u32 battler, u32 hpFraction, u32 itemId)
{
    ...
    if (hpFraction <= 4 && HasBattlerAbility(battler, ABILITY_GLUTTONY) && isBerry
         && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 2)
    {
...
static bool32 ShouldDelayWhiteHerbAtMoveEnd(u32 battler)
{
    ...
    if (battler == gBattlerTarget
     ...
     && !DoesSubstituteBlockMove(gBattlerAttacker, battler, gCurrentMove)
     && !(gMoveResultFlags & MOVE_RESULT_NO_EFFECT)
     && ( !HasBattlerAbility(battler, ABILITY_STICKY_HOLD) || !IsBattlerAlive(battler)))
    {
        return TRUE;
    }
    ...
    case ITEMEFFECT_NORMAL:
        if (gBattleMons[battler].hp)
        {
            ...
            case HOLD_EFFECT_BLACK_SLUDGE:
                ...
                else if (!HasBattlerAbility(battler, ABILITY_MAGIC_GUARD) && !moveTurn)
                {
                    gBattleMoveDamage = GetNonDynamaxMaxHP(battler) / 8;
                    if (gBattleMoveDamage == 0)
                        gBattleMoveDamage = 1;
    ...
    case MOVE_TARGET_SELECTED:
        ...
        else
        {
            ...
            if (gBattleMoves[move].type == TYPE_ELECTRIC
                ...
                && !HasBattlerAbility(targetBattler, ABILITY_LIGHTNING_ROD))
            {
                ...
                && !HasBattlerAbility(targetBattler, ABILITY_STORM_DRAIN))
            {
                targetBattler ^= BIT_FLANK;
...
u32 GetBattlerHoldEffect(u32 battler, bool32 checkNegating)
{
    if (checkNegating)
    {
        ...
        if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM)
            return HOLD_EFFECT_NONE;
        if (HasBattlerAbility(battler, ABILITY_KLUTZ))
            return HOLD_EFFECT_NONE;
    }
...
static inline u32 CalcMoveBasePowerAfterModifiers(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk, u32 weather)
{
    ...
    case EFFECT_VENOSHOCK:
        if (gBattleMons[battlerDef].status1 & STATUS1_PSN_ANY)
            modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
        if (IsBattlerWeatherAffected(battlerAtk, B_WEATHER_ACID_RAIN))
            modifier = uq4_12_multiply(modifier, UQ_4_12(2.0));
        break;
...
static inline u32 CalcAttackStat(u32 move, u32 battlerAtk, u32 battlerDef, u32 moveType, bool32 isCrit, bool32 updateFlags, u32 atkAbility, u32 defAbility, u32 holdEffectAtk)
{
    ...
    else
    {
        if (IS_MOVE_PHYSICAL(move) && ShouldDualitySwapOffensiveStats(battlerAtk, move, moveType))
        {
            ...
            if (updateFlags)
                RecordAbilityBattle(battlerAtk, ABILITY_DUALITY);
        }
        else if (IS_MOVE_SPECIAL(move) && ShouldDualitySwapOffensiveStats(battlerAtk, move, moveType))
        {
    ...
    if (HasBattlerAbility(battlerDef, ABILITY_BLUBBER)
     && (moveType == TYPE_FIRE || moveType == TYPE_FIGHTING))
    {
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.5));
        if (updateFlags)
            RecordAbilityBattle(battlerDef, ABILITY_BLUBBER);
    }
...
static inline uq4_12_t GetDefenderAbilitiesModifier(u32 move, u32 moveType, u32 battlerAtk, u32 battlerDef, uq4_12_t typeEffectivenessModifier, bool32 updateFlags, u32 abilityDef)
{
    ...
    {
        if (updateFlags)
            RecordAbilityBattle(battlerDef, ABILITY_MAIN_EVENT);
        return UQ_4_12(0.5);
    }

    if (HasBattlerAbility(battlerDef, ABILITY_PERMAFROST)
     && (moveType == TYPE_FIGHTING || moveType == TYPE_ROCK || moveType == TYPE_STEEL))
    {
        if (updateFlags)
                            ...
                            u32 holdEffectAtk, u32 holdEffectDef, u32 abilityAtk, u32 abilityDef)
{
    ...
    DAMAGE_APPLY_MODIFIER(GetWeatherDamageModifier(battlerAtk, battlerDef, move, moveType, holdEffectAtk, holdEffectDef, weather));
...
static inline void MulByTypeEffectiveness(uq4_12_t *modifier, u32 move, u32 moveType, u32 battlerDef, u32 defType, u32 battlerAtk, bool32 recordAbilities)
{
    ...
    if (moveType == TYPE_GHOST
     && defType == TYPE_GRASS
     && battlerAtk < gBattlersCount
     && HasBattlerAbility(battlerAtk, ABILITY_GRAVE_GROVE))
        mod = UQ_4_12(2.0);
    if (gBattleMoves[move].soundMove
...
bool32 CanFling(u32 battler)
{
    ...
    if (item == ITEM_NONE
      || (B_KLUTZ_FLING_INTERACTION >= GEN_5 && HasBattlerAbility(battler, ABILITY_KLUTZ))
      || gFieldStatuses & STATUS_FIELD_MAGIC_ROOM
