...
{
  if (AI_THINKING_STRUCT->aiFlags & AI_FLAG_ACE_POKEMON && !(gBattleStruct->forcedSwitch & gBitTable[battler]) &&
...

{
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
  {
    ...
    if (aiMove != MOVE_NONE)
    {
      ...
      if (aiMoveEffect == EFFECT_REFLECT || aiMoveEffect == EFFECT_LIGHT_SCREEN || aiMoveEffect == EFFECT_SPIKES ||
          aiMoveEffect == EFFECT_TOXIC_SPIKES || aiMoveEffect == EFFECT_STEALTH_ROCK ||
          aiMoveEffect == EFFECT_STICKY_WEB || aiMoveEffect == EFFECT_LEECH_SEED || aiMoveEffect == EFFECT_EXPLOSION ||
          aiMoveEffect == EFFECT_SLEEP || aiMoveEffect == EFFECT_YAWN || aiMoveEffect == EFFECT_TOXIC ||
          aiMoveEffect == EFFECT_WILL_O_WISP || aiMoveEffect == EFFECT_PARALYZE || aiMoveEffect == EFFECT_TRICK ||
          aiMoveEffect == EFFECT_TRICK_ROOM || aiMoveEffect == EFFECT_WONDER_ROOM ||
          aiMoveEffect == EFFECT_PSYCHO_SHIFT || aiMoveEffect == EFFECT_FAKE_OUT)
      {
        hasStatusMove = TRUE;
      }
  ...
  if (((getsOneShot &&
        gBattleMons[opposingBattler].speed > gBattleMons[battler].speed)  // If the player OHKOs and outspeeds OR OHKOs,
                                                                          // doesn't outspeed but isn't 2HKO'd
       || (getsOneShot && gBattleMons[opposingBattler].speed <= gBattleMons[battler].speed &&
           maxDamageDealt < gBattleMons[opposingBattler].hp / 2)) &&
      (gBattleMons[battler].hp >=
           gBattleMons[battler].maxHP / 2  // And the current mon has at least 1/2 their HP, or 1/4 HP and Regenerator
  ...
  if (typeEffectiveness >= UQ_4_12(2.0))  // If the player has at least a 2x type advantage
  {
    if (!hasSuperEffectiveMove  // If the AI doesn't have a super effective move
        && (gBattleMons[battler].hp >= gBattleMons[battler].maxHP /
                    2  // And the current mon has at least 1/2 their HP, or 1/4 HP and Regenerator
            || (aiAbility == ABILITY_REGENERATOR && gBattleMons[battler].hp >= gBattleMons[battler].maxHP / 4)))
    {
...
static bool8 ShouldSwitchIfWonderGuard(u32 battler)
{
  ...
  return FALSE;  // There is not a single Pokemon in the party that has a super effective move against a mon with Wonder
                 // Guard.
}
...
static bool8 FindMonThatAbsorbsOpponentsMove(u32 battler)
{
  ...
  } else if (gBattleMoves[gLastLandedMoves[battler]].type == TYPE_GRASS)
  {
    ...
    numAbsorbingAbilities = 1;
  } else if (gBattleMoves[gLastLandedMoves[battler]].type == TYPE_FLYING)
  {
    absorbingTypeAbilities[0] = ABILITY_AERODYNAMICS;
    numAbsorbingAbilities = 1;
...
static bool8 ShouldSwitchIfGameStatePrompt(u32 battler)
{
  ...
  {
    // Yawn
    if (gStatuses3[battler] & STATUS3_YAWN && AI_CanSleep(battler, monAbility) &&
        gBattleMons[battler].hp > gBattleMons[battler].maxHP / 3)
    {
      ...
      if (IsDoubleBattle())
      {
        if (IsBattlerAlive(BATTLE_PARTNER(battler)) && (GetAIChosenMove(BATTLE_PARTNER(battler)) == MOVE_UPROAR))
        {
          switchMon = FALSE;
        }
        ...
        if (*(gBattleStruct->AI_monToSwitchIntoId + BATTLE_PARTNER(battler)) != PARTY_SIZE)  // Partner is switching
        {
          ...
          for (i = firstId; i < lastId; i++)
          {
            ...
            if (IsValidForBattle(&party[i]) && i != gBattlerPartyIndexes[battler] &&
                i != gBattlerPartyIndexes[BATTLE_PARTNER(battler)] && IsBattlerGrounded(battler) &&
      ...
      if ((monAbility == ABILITY_NATURAL_CURE || monAbility == ABILITY_EARLY_BIRD) ||
          holdEffect == (HOLD_EFFECT_CURE_SLP | HOLD_EFFECT_CURE_STATUS) || HasMove(battler, MOVE_SLEEP_TALK) ||
          (HasMoveEffect(battler, MOVE_SNORE) &&
           AI_GetTypeEffectiveness(MOVE_SNORE, battler, opposingBattler) >= UQ_4_12(1.0)) ||
          (IsBattlerGrounded(battler) &&
      ...
      if (gBattleMons[battler].statStages[STAT_EVASION] > (DEFAULT_STAT_STAGE + 3) &&
          ...
          !(gBattleMons[battler].status2 & STATUS2_FORESIGHT) && !(gStatuses3[battler] & STATUS3_MIRACLE_EYED))
      {
        switchMon = FALSE;
      }
    }
    ...
    // Secondary Damage
    if (monAbility != ABILITY_MAGIC_GUARD && !AiExpectsToFaintPlayer(battler))
    {
      // Toxic
      moduloChance = 2;  // 50%
      ...
      // Cursed
      moduloChance = 2;  // 50%
      if (gBattleMons[battler].status2 & STATUS2_CURSED && (Random() % (moduloChance * chanceReducer)) == 0)
      {
        switchMon = TRUE;
      }
      ...
      // Nightmare
      moduloChance = 3;  // 33.3%
      if (gBattleMons[battler].status2 & STATUS2_NIGHTMARE && (Random() % (moduloChance * chanceReducer)) == 0)
      {
        switchMon = TRUE;
      }
      ...
      // Leech Seed
      moduloChance = 4;  // 25%
      if (gStatuses3[battler] & STATUS3_LEECHSEED && (Random() % (moduloChance * chanceReducer)) == 0)
      {
        switchMon = TRUE;
      }
    }
    ...
    // Infatuation
    if (gBattleMons[battler].status2 & STATUS2_INFATUATION && !AiExpectsToFaintPlayer(battler))
    {
      switchMon = TRUE;
    }
    ...
    if (gStatuses3[opposingBattler] & STATUS3_SEMI_INVULNERABLE)
    {
      ...
      if (!AI_OpponentCanFaintAiWithMod(battler, 0) && AnyStatIsRaised(battler))
      {
        switchMon = FALSE;
      }
      if (AiExpectsToFaintPlayer(battler) && !AI_STRIKES_FIRST(battler, opposingBattler, 0) &&
...
static bool8 ShouldSwitchIfAbilityBenefit(u32 battler)
{
  ...
  if (gStatuses3[battler] & STATUS3_GASTRO_ACID || IsNeutralizingGasOnField())
  {
    return FALSE;
  }
  ...
  switch (AI_DATA->abilities[battler])
  {
    case ABILITY_NATURAL_CURE:
      ...
      if ((gBattleMons[battler].status1 & STATUS1_ANY) && (gBattleMons[battler].hp >= gBattleMons[battler].maxHP / 2) &&
          AI_DATA->mostSuitableMonId[battler] != PARTY_SIZE && Random() % (moduloChance * chanceReducer) == 0)
      {
        break;
      }
...
static u32 GetBestMonBatonPass(struct Pokemon* party,
                               int firstId,
                               int lastId,
                               u8 invalidMons,
                               int aliveCount,
...
static u32 GetBestMonTypeMatchup(struct Pokemon* party,
                                 int firstId,
                                 int lastId,
                                 u8 invalidMons,
                                 u32 battler,
...
static u32 GetBestMonDmg(struct Pokemon* party,
                         int firstId,
                         int lastId,
                         u8 invalidMons,
                         u32 battler,
...
static bool32 IsMonGrounded(u16 heldItemEffect, u32 ability, u8 type1, u8 type2)
{
      ...
      (heldItemEffect == HOLD_EFFECT_AIR_BALLOON && ability != ABILITY_KLUTZ))
  {
    ...
    if ((heldItemEffect == HOLD_EFFECT_IRON_BALL && ability != ABILITY_KLUTZ) ||
...
static u32 GetSwitchinHazardsDamage(u32 battler, struct BattlePokemon* battleMon)
{
  ...
  u32 hazardFlags = gSideStatuses[GetBattlerSide(battler)] &
       ...
       !((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || ability == ABILITY_KLUTZ)))
  {
    ...
    {
      hazardDamage += GetStealthHazardDamageByTypesAndHP(gBattleMoves[MOVE_STEALTH_ROCK].type, defType1, defType2,
                                                         battleMon->maxHP);
    }
    // Spikes
    ...
    if ((hazardFlags & SIDE_STATUS_TOXIC_SPIKES) &&
        (defType1 != TYPE_POISON && defType2 != TYPE_POISON && defType1 != TYPE_STEEL && defType2 != TYPE_STEEL &&
         ability != ABILITY_IMMUNITY && ability != ABILITY_POISON_HEAL && ability != ABILITY_COMATOSE && status == 0 &&
         !(hazardFlags & SIDE_STATUS_SAFEGUARD) && !(IsAbilityOnSide(battler, ABILITY_PASTEL_VEIL)) &&
         !(IsBattlerTerrainAffected(battler, STATUS_FIELD_MISTY_TERRAIN)) && !(IsAbilityStatusProtected(battler)) &&
...
static s32 GetSwitchinWeatherImpact(void)
{
  s32 weatherImpact = 0, maxHP = AI_DATA->switchinCandidate.battleMon.maxHP,
      ability = AI_DATA->switchinCandidate.battleMon.ability;
  ...
  if (WEATHER_HAS_EFFECT)
  {
    ...
    if (item != ITEM_SAFETY_GOGGLES)
    {
      if ((gBattleWeather & B_WEATHER_HAIL) &&
          (AI_DATA->switchinCandidate.battleMon.type1 != TYPE_ICE ||
          ...
          ability != ABILITY_OVERCOAT && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_ICE_BODY)
      {
        ...
        if (weatherImpact == 0)
        {
          weatherImpact = 1;
                 ...
                 (AI_DATA->switchinCandidate.battleMon.type1 != TYPE_GROUND &&
                  AI_DATA->switchinCandidate.battleMon.type2 != TYPE_GROUND &&
                  AI_DATA->switchinCandidate.battleMon.type1 != TYPE_ROCK &&
                  AI_DATA->switchinCandidate.battleMon.type2 != TYPE_ROCK &&
                  AI_DATA->switchinCandidate.battleMon.type1 != TYPE_STEEL &&
                  AI_DATA->switchinCandidate.battleMon.type2 != TYPE_STEEL && ability != ABILITY_OVERCOAT &&
...
{
  u32 recurringHealing = 0, maxHP = AI_DATA->switchinCandidate.battleMon.maxHP,
      ability = AI_DATA->switchinCandidate.battleMon.ability;
  u16 item = AI_DATA->switchinCandidate.battleMon.item;
...
{
  u32 passiveDamage = 0, maxHP = AI_DATA->switchinCandidate.battleMon.maxHP,
      ability = AI_DATA->switchinCandidate.battleMon.ability;
  u16 item = AI_DATA->switchinCandidate.battleMon.item;
  ...
  {
    if (item == ITEM_BLACK_SLUDGE && AI_DATA->switchinCandidate.battleMon.type1 != TYPE_POISON &&
        AI_DATA->switchinCandidate.battleMon.type2 != TYPE_POISON)
    {
...
static u32 GetSwitchinStatusDamage(u32 battler)
{
  ...
  u32 status = AI_DATA->switchinCandidate.battleMon.status1, ability = AI_DATA->switchinCandidate.battleMon.ability,
      maxHP = AI_DATA->switchinCandidate.battleMon.maxHP;
  u32 statusDamage = 0;
  ...
  // Apply hypothetical poisoning from Toxic Spikes, which means the first turn of damage already added in
  // GetSwitchinHazardsDamage Do this last to skip one iteration of Poison / Toxic damage, and start counting Toxic
  // damage one turn later.
  if (tSpikesLayers != 0 &&
      (defType1 != TYPE_POISON && defType2 != TYPE_POISON && ability != ABILITY_IMMUNITY &&
       ability != ABILITY_POISON_HEAL && status == 0 &&
       !(heldItemEffect == HOLD_EFFECT_HEAVY_DUTY_BOOTS &&
         (((gFieldStatuses & STATUS_FIELD_MAGIC_ROOM) || ability == ABILITY_KLUTZ))) &&
       heldItemEffect != HOLD_EFFECT_CURE_PSN && heldItemEffect != HOLD_EFFECT_CURE_STATUS &&
       IsMonGrounded(heldItemEffect, ability, defType1, defType2)))
  {
    if (tSpikesLayers == 1)
    {
      AI_DATA->switchinCandidate.battleMon
          .status1 = STATUS1_POISON;  // Assign "hypothetical" status to the switchin candidate so we can get the damage
                                      // it would take from TSpikes
      AI_DATA->switchinCandidate.hypotheticalStatus = TRUE;
    }
    if (tSpikesLayers == 2)
    {
      AI_DATA->switchinCandidate.battleMon
          .status1 = STATUS1_TOXIC_POISON;  // Assign "hypothetical" status to the switchin candidate so we can get the
                                            // damage it would take from TSpikes
      AI_DATA->switchinCandidate.battleMon.status1 += STATUS1_TOXIC_TURN(1);
      AI_DATA->switchinCandidate.hypotheticalStatus = TRUE;
    }
  }
...
{
  u32 startingHP = AI_DATA->switchinCandidate.battleMon.hp -
      GetSwitchinHazardsDamage(battler, &AI_DATA->switchinCandidate.battleMon);
  ...
  u16 maxHP = AI_DATA->switchinCandidate.battleMon.maxHP, item = AI_DATA->switchinCandidate.battleMon.item,
      heldItemEffect = gItems[AI_DATA->switchinCandidate.battleMon.item].holdEffect;
  u8 weatherDuration = gWishFutureKnock.weatherDuration,
     holdEffectParam = gItems[AI_DATA->switchinCandidate.battleMon.item].holdEffectParam;
  ...
  if ((damageTaken + statusDamage + recurringDamage <= recurringHealing) ||
      damageTaken + statusDamage + recurringDamage == 0)
  {
    return startingHP;
  }
  ...
  while (currentHP > 0)
  {
    ...
    // If mon is still alive, apply weather impact first, as it might KO the mon before it can heal with its item (order
    // is weather -> item -> status)
    if (currentHP >= 0)
    {
    ...
    if (AI_DATA->switchinCandidate.battleMon.ability != ABILITY_KLUTZ && usedSingleUseHealingItem == FALSE)
    {
      ...
      } else if (currentHP < maxHP / CONFUSE_BERRY_HP_FRACTION && opposingAbility != ABILITY_UNNERVE &&
                 (item == ITEM_AGUAV_BERRY || item == ITEM_FIGY_BERRY || item == ITEM_IAPAPA_BERRY ||
                  item == ITEM_MAGO_BERRY || item == ITEM_WIKI_BERRY))
      {
...
static u16 GetSwitchinTypeMatchup(u32 opposingBattler, struct BattlePokemon battleMon)
{
  ...
  u8 atkType1 = gSpeciesInfo[gBattleMons[opposingBattler].species].types[0],
     atkType2 = gSpeciesInfo[gBattleMons[opposingBattler].species].types[1], defType1 = battleMon.type1,
     defType2 = battleMon.type2;
...
// Mid battle, it integrates GetBestMonTypeMatchup (vanilla with modifications), GetBestMonDefensive (custom), and
// GetBestMonBatonPass (vanilla with modifications) After a KO, integrates GetBestMonRevengeKiller (custom),
// GetBestMonTypeMatchup (vanilla with modifications), GetBestMonBatonPass (vanilla with modifications), and
// GetBestMonDmg (vanilla) the Type Matchup code will prioritize switching into a mon with the best type matchup and
// also a super effective move, or just best type matchup if no super effective move is found the Most Defensive code
// will prioritize switching into the mon that takes the most hits to KO, with a minimum of 4 hits required to be
// considered a valid option the Baton Pass code will prioritize switching into a mon with Baton Pass if it can get in,
// boost, and BP out without being KO'd, and randomizes between multiple valid options the Revenge Killer code will
// prioritize, in order, OHKO and outspeeds / OHKO, slower but not 2HKO'd / 2HKO, outspeeds and not OHKO'd / 2HKO,
// slower but not 3HKO'd the Most Damage code will prioritize switching into whatever mon deals the most damage, which
// is generally not as good as having a good Type Matchup Everything runs in the same loop to minimize computation time.
// This makes it harder to read, but hopefully the comments can guide you!

static u32 GetBestMonIntegrated(struct Pokemon* party,
                                int firstId,
                                int lastId,
                                u32 battler,
                                u32 opposingBattler,
                                u8 battlerIn1,
                                u8 battlerIn2,
                                bool8 isSwitchAfterKO)
{
  int revengeKillerId = PARTY_SIZE, slowRevengeKillerId = PARTY_SIZE, fastThreatenId = PARTY_SIZE,
      slowThreatenId = PARTY_SIZE, damageMonId = PARTY_SIZE;
  int batonPassId = PARTY_SIZE, typeMatchupId = PARTY_SIZE, typeMatchupEffectiveId = PARTY_SIZE,
      defensiveMonId = PARTY_SIZE, aceMonId = PARTY_SIZE;
  ...
  s32 playerMonSpeed = gBattleMons[opposingBattler].speed, playerMonHP = gBattleMons[opposingBattler].hp, aiMonSpeed,
      maxDamageDealt = 0, damageDealt = 0;
  ...
  {
    // Check mon validity
    if (!IsValidForBattle(&party[i]) || gBattlerPartyIndexes[battlerIn1] == i ||
        gBattlerPartyIndexes[battlerIn2] == i || i == *(gBattleStruct->monToSwitchIntoId + battlerIn1) ||
    ...
    if (AI_DATA->switchinCandidate.battleMon.ability == ABILITY_TRUANT &&
        IsTruantMonVulnerable(battler, opposingBattler))
    {
      continue;
    }

    // Get max number of hits for player to KO AI mon
    hitsToKO = GetSwitchinHitsToKO(GetMaxDamagePlayerCouldDealToSwitchin(battler, opposingBattler,
                                                                         AI_DATA->switchinCandidate.battleMon),
                                   battler);
    ...
    {
      if ((hitsToKO > hitsToKOThreshold && AI_DATA->switchinCandidate.battleMon.speed > playerMonSpeed) ||
          hitsToKO > hitsToKOThreshold + 1)  // Need to take an extra hit if slower
      {
    ...
    for (j = 0; j < MAX_MON_MOVES; j++)
    {
      ...
      {
        damageDealt = AI_CalcPartyMonDamage(aiMove, battler, opposingBattler, AI_DATA->switchinCandidate.battleMon,
                                            TRUE);
      }
      ...
      if (aiMove == MOVE_BATON_PASS &&
          ((hitsToKO > hitsToKOThreshold + 1 && AI_DATA->switchinCandidate.battleMon.speed < playerMonSpeed) ||
           (hitsToKO > hitsToKOThreshold && AI_DATA->switchinCandidate.battleMon.speed > playerMonSpeed)))
      {
        bits |= gBitTable[i];
      }
      ...
      if (aiMove != MOVE_NONE && gBattleMoves[aiMove].power != 0)
      {
        if (typeMatchup < bestResistEffective)
        {
          if (AI_GetTypeEffectiveness(aiMove, battler, opposingBattler) >= UQ_4_12(2.0))
          {
            // Assuming a super effective move would do significant damage or scare the player out, so not being as
            // conservative here
        ...
        if (damageDealt > playerMonHP)
        {
          ...
          if ((aiMonSpeed > playerMonSpeed || gBattleMoves[aiMove].priority > 0) &&
              AI_DATA->switchinCandidate.battleMon.hp >
                  GetSwitchinHazardsDamage(battler, &AI_DATA->switchinCandidate.battleMon))
          {
  ...
  } else
  {
    ...
    // If ace mon is the last available Pokemon and U-Turn/Volt Switch was used - switch to the mon.
             ...
             (gBattleMoves[gLastUsedMove].effect == EFFECT_HIT_ESCAPE ||
              gBattleMoves[gLastUsedMove].effect == EFFECT_PARTING_SHOT))
    {
      return aceMonId;
    }
  }
...
u8 GetMostSuitableMonToSwitchInto(u32 battler, bool32 switchAfterMonKOd)
{
  ...
  // Split ideal mon decision between after previous mon KO'd (prioritize offensive options) and after switching active
  // mon out (prioritize defensive options), and expand the scope of both. Only use better mon selection if
  // AI_FLAG_SMART_MON_CHOICES is set for the trainer.
  ...
  else
  {
    ...
    for (i = firstId; i < lastId; i++)
    {
      if (!IsValidForBattle(&party[i]) || gBattlerPartyIndexes[battlerIn1] == i ||
          gBattlerPartyIndexes[battlerIn2] == i || i == *(gBattleStruct->monToSwitchIntoId + battlerIn1) ||
          i == *(gBattleStruct->monToSwitchIntoId + battlerIn2) ||
          (GetMonAbility(&party[i]) == ABILITY_TRUANT &&
           IsTruantMonVulnerable(battler, opposingBattler)))  // While not really invalid per say, not really wise to
                                                              // switch into this mon.)
      {
    ...
    switch (ItemId_GetBattleUsage(item))
    {
      ...
      case EFFECT_ITEM_CURE_STATUS:
        ...
        if (itemEffects[3] & ITEM3_FREEZE &&
            (gBattleMons[battler].status1 & STATUS1_FREEZE || gBattleMons[battler].status1 & STATUS1_FROSTBITE))
        {
          shouldUse = TRUE;
        }
      ...
      case EFFECT_ITEM_SET_FOCUS_ENERGY:
        if (!gDisableStructs[battler].isFirstTurn || gBattleMons[battler].status2 & STATUS2_FOCUS_ENERGY ||
            AI_OpponentCanFaintAiWithMod(battler, 0))
        {
          break;
        }
...
static bool32 AI_ShouldHeal(u32 battler, u32 healAmount)
{
  ...
  if (gBattleMons[battler].hp < gBattleMons[battler].maxHP / 4 || gBattleMons[battler].hp == 0 ||
      (healAmount != 0 && gBattleMons[battler].maxHP - gBattleMons[battler].hp > healAmount))
  {
    // We have low enough HP to consider healing
    shouldHeal = !AI_OpponentCanFaintAiWithMod(battler,
                                               healAmount);  // if target can kill us even after we heal, why bother
  }
