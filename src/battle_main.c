...
EWRAM_DATA uq4_12_t gMoveResultEffectiveness = UQ_4_12(1.0);
...
static void DoBattleIntro(void)
{
    ...
    case 20: // set dex and battle vars
        if (!gBattleControllerExecFlags)
        {
            for (battler = 0; battler < gBattlersCount; battler++)
            {
                SwitchInClearSetData(battler, FALSE);
...
static void TryDoEventsBeforeFirstTurn(void)
{
    ...
    while (gBattleStruct->switchInAbilitiesCounter < gBattlersCount)
    {
        ...
        if (AbilityBattleEffects(ABILITYEFFECT_ON_SWITCHIN, gBattlerAttacker, 0, 0, 0)
         || (gBattleWeather & B_WEATHER_ANY && WEATHER_HAS_EFFECT && AbilityBattleEffects(ABILITYEFFECT_ON_WEATHER, gBattlerAttacker, 0, 0, 0))
         || (gFieldStatuses & STATUS_FIELD_TERRAIN_ANY && AbilityBattleEffects(ABILITYEFFECT_ON_TERRAIN, gBattlerAttacker, 0, 0, 0))
...
static u32 GetBattlerTotalSpeedStatArgsInternal(u32 battler, u32 ability, u32 holdEffect, bool32 ignoreLimberSpeedStages, bool32 ignoreSpeedStages)
{
    ...
    // other abilities
    if (ability == ABILITY_QUICK_FEET && (gBattleMons[battler].status1 & STATUS1_ANY || IsBattlerEffectivelyBurned(battler)))
        speed = (speed * 150) / 100;
    ...
    if (holdEffect == HOLD_EFFECT_MACHO_BRACE
     ...
     || holdEffect == HOLD_EFFECT_HOURGLASS)
        ...
        speed *= 2;
    else if (holdEffect == HOLD_EFFECT_WINGED_BOOTS
...
s8 GetMovePriority(u32 battler, u16 move)
{
    ...
    if (HasBattlerAbility(battler, ABILITY_VOLT_BREAK)
    ...
    if (HasBattlerAbility(battler, ABILITY_SKYFIRE)
     && gBattleStruct->moveTarget[battler] < gBattlersCount
     && GetBattlerSide(gBattleStruct->moveTarget[battler]) != GetBattlerSide(battler)
     && IsBattlerAlive(gBattleStruct->moveTarget[battler])
...
void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
{
    ...
    else if (gBattleMoves[move].soundMove && HasBattlerAbility(battlerAtk, ABILITY_REGAL_DECREE))
    {
