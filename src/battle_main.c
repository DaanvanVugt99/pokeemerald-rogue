...
EWRAM_DATA u8 gDisplayedStringBattle[425] = {
    0};  // Increased in size to fit Juan's defeat text (SootopolisCity_Gym_1F_Text_JuanDefeat)
...
static const struct ScanlineEffectParams sIntroScanlineParams16Bit = {&REG_BG3HOFS, SCANLINE_EFFECT_DMACNT_16BIT, 1};

// unused
static const struct ScanlineEffectParams sIntroScanlineParams32Bit = {&REG_BG3HOFS, SCANLINE_EFFECT_DMACNT_32BIT, 1};
...
const u8 gTypeNames[NUMBER_OF_MON_TYPES][TYPE_NAME_LENGTH + 1] = {
    [TYPE_NORMAL] = _("Normal"), [TYPE_FIGHTING] = _("Fight"),  [TYPE_FLYING] = _("Flying"),
    [TYPE_POISON] = _("Poison"), [TYPE_GROUND] = _("Ground"),   [TYPE_ROCK] = _("Rock"),
    [TYPE_BUG] = _("Bug"),       [TYPE_GHOST] = _("Ghost"),     [TYPE_STEEL] = _("Steel"),
    [TYPE_MYSTERY] = _("???"),   [TYPE_FIRE] = _("Fire"),       [TYPE_WATER] = _("Water"),
    [TYPE_GRASS] = _("Grass"),   [TYPE_ELECTRIC] = _("Electr"), [TYPE_PSYCHIC] = _("Psychc"),
    [TYPE_ICE] = _("Ice"),       [TYPE_DRAGON] = _("Dragon"),   [TYPE_DARK] = _("Dark"),
    [TYPE_FAIRY] = _("Fairy"),   [TYPE_STELLAR] = _("Stellr"),
};
...
#  endif
    [TRAINER_CLASS_COOLTRAINER] = ITEM_ULTRA_BALL,   [TRAINER_CLASS_COLLECTOR] = ITEM_PREMIER_BALL,
    [TRAINER_CLASS_SWIMMER_M] = ITEM_DIVE_BALL,      [TRAINER_CLASS_BLACK_BELT] = ITEM_ULTRA_BALL,
    [TRAINER_CLASS_AQUA_LEADER] = ITEM_MASTER_BALL,  [TRAINER_CLASS_GENTLEMAN] = ITEM_LUXURY_BALL,
...
#  endif
    [TRAINER_CLASS_SWIMMER_F] = ITEM_DIVE_BALL,      [TRAINER_CLASS_COOLTRAINER_2] = ITEM_ULTRA_BALL,
...
const u8* const gStatusConditionStringsTable[][2] = {{gStatusConditionString_PoisonJpn, gText_Poison},
        ...
        if (gReceivedRemoteLinkPlayers)
        {
          if (IsLinkTaskFinished())
          {
            ...
            *(&gBattleStruct->multiBuffer.linkBattlerHeader.versionSignatureHi) = 3;
            ...
            SendBlock(BitmaskAllOtherLinkPlayers(), &gBattleStruct->multiBuffer.linkBattlerHeader,
...
void BattleMainCB2(void)
{
  ...
  // If we are processing a palette fade we need to temporarily fall back to 1x speed otherwise there is graphical
  // corruption
...
void SpriteCB_WildMon(struct Sprite* sprite)
{
  ...
  {
    BeginNormalPaletteFade((0x10000 << sprite->sBattler) | (0x10000 << BATTLE_PARTNER(sprite->sBattler)), 0, 10, 10,
                           RGB(8, 8, 8));
  } else
  {
...
static void SpriteCB_WildMonShowHealthbox(struct Sprite* sprite)
{
  if (sprite->animEnded)
  {
    ...
    if (WILD_DOUBLE_BATTLE)
    {
      BeginNormalPaletteFade((0x10000 << sprite->sBattler) | (0x10000 << BATTLE_PARTNER(sprite->sBattler)), 0, 10, 0,
                             RGB(8, 8, 8));
    } else
    {
...
void EndBounceEffect(u8 battler, u8 which)
{
  ...
  if (which == BOUNCE_HEALTHBOX)
  {
    ...
    bouncerSpriteId = gSprites[gBattleSpritesDataPtr->healthBoxesData[battler].healthboxBounceSpriteId]
                          .sBouncerSpriteId;
...
static void BattleStartClearSetData(void)
{
  ...
  gBattleScripting.battleStyle = Rogue_GetConfigToggle(CONFIG_TOGGLE_SWITCH_MODE) ? OPTIONS_BATTLE_STYLE_SHIFT
...
void SwitchInClearSetData(u32 battler)
{
  ...
  if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS)
  {
    gBattleMons[battler].status2 &= (STATUS2_CONFUSION | STATUS2_FOCUS_ENERGY | STATUS2_SUBSTITUTE |
                                     STATUS2_ESCAPE_PREVENTION | STATUS2_CURSED);
    gStatuses3[battler] &= (STATUS3_LEECHSEED_BATTLER | STATUS3_LEECHSEED | STATUS3_ALWAYS_HITS | STATUS3_PERISH_SONG |
                            STATUS3_ROOTED | STATUS3_GASTRO_ACID | STATUS3_EMBARGO | STATUS3_TELEKINESIS |
                            STATUS3_MAGNET_RISE | STATUS3_HEAL_BLOCK | STATUS3_AQUA_RING | STATUS3_POWER_TRICK);
    gStatuses4[battler] &= (STATUS4_MUD_SPORT | STATUS4_WATER_SPORT | STATUS4_INFINITE_CONFUSION);
    for (i = 0; i < gBattlersCount; i++)
    {
      if (GetBattlerSide(battler) != GetBattlerSide(i) && (gStatuses3[i] & STATUS3_ALWAYS_HITS) != 0 &&
...
// Allow for illegal abilities within tests.
  ...
  if (gTestRunnerEnabled)
  {
    ...
    {
      gBattleMons[i].ability = gBattleStruct->overwrittenAbilities[i] = TestRunner_Battle_GetForcedAbility(side,
                                                                                                           partyIndex);
    }
  }
#endif  // TESTING
...
const u8* FaintClearSetData(u32 battler)
{
  ...
  gBattleMons[battler].status2 = 0;
  gStatuses3[battler] &= STATUS3_GASTRO_ACID;  // Edge case: Keep Gastro Acid if pokemon's ability can have effect after
                                               // fainting, for example Innards Out.
  gStatuses4[battler] = 0;
...
static void DoBattleIntro(void)
{
  ...
  switch (*state)
  {
    ...
    case 4:  // Copy battler data gotten in cases 0 and 1. Draw trainer/mon sprite.
      for (battler = 0; battler < gBattlersCount; battler++)
      {
        ...
        } else
        {
          ...
          gBattleMons[battler].type2 = gSpeciesInfo[gBattleMons[battler].species].types[1];
          gBattleMons[battler].type3 = TYPE_MYSTERY;
          gBattleMons[battler].ability = GetAbilityBySpecies(gBattleMons[battler].species,
        ...
        switch (GetBattlerPosition(battler))
        {
          ...
          case B_POSITION_OPPONENT_LEFT:
            ...
            } else  // wild mon 1
            {
              ...
              MarkBattlerForControllerExec(battler);
              gBattleResults.lastOpponentSpecies = GetMonData(&gEnemyParty[gBattlerPartyIndexes[battler]],
          ...
          case B_POSITION_OPPONENT_RIGHT:
            if (gBattleTypeFlags & BATTLE_TYPE_TRAINER)
            {
              if (gBattleTypeFlags & (BATTLE_TYPE_MULTI | BATTLE_TYPE_TWO_OPPONENTS) &&
                  !BATTLE_TWO_VS_ONE_OPPONENT)  // opponent 2 if exists
              {
                BtlController_EmitDrawTrainerPic(battler, BUFFER_A);
                MarkBattlerForControllerExec(battler);
              }
    ...
    case 13:  // second opponent's mon send out
      ...
      if (B_FAST_INTRO == TRUE &&
          !(gBattleTypeFlags &
            (BATTLE_TYPE_RECORDED | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_RECORDED_IS_MASTER | BATTLE_TYPE_LINK)))
      {
    ...
    case 16:  // print player sends out
      if (!(gBattleTypeFlags & BATTLE_TYPE_SAFARI))
      {
        ...
        if (B_FAST_INTRO == TRUE && gBattleTypeFlags & BATTLE_TYPE_TRAINER &&
            !(gBattleTypeFlags &
              (BATTLE_TYPE_RECORDED | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_RECORDED_IS_MASTER | BATTLE_TYPE_LINK)) &&
...
// Allow for illegal abilities within tests.
  ...
  if (gTestRunnerEnabled && gBattleStruct->switchInAbilitiesCounter == 0)
  {
    for (i = 0; i < gBattlersCount; ++i)
    {
      ...
      if (TestRunner_Battle_GetForcedAbility(side, partyIndex))
      {
        gBattleMons[i].ability = gBattleStruct
                                     ->overwrittenAbilities[i] = TestRunner_Battle_GetForcedAbility(side, partyIndex);
      }
    }
  }
#endif  // TESTING
  ...
  for (i = 0; i < gBattlersCount; i++)
  {
    if (gQueuedStatBoosts[i].stats != 0 && !gProtectStructs[i].eatMirrorHerb &&
        gProtectStructs[i].activateOpportunist == 0)
    {
      gBattlerAttacker = i;
...
void BattleTurnPassed(void)
{
  ...
  *(&gBattleStruct->absentBattlerFlags) = gAbsentBattlerFlags;
  ...
  {
    BattleScriptExecute(i == 1 ? BattleScript_TrainerASlideMsgEnd2 : BattleScript_TrainerBSlideMsgEnd2);
  } else if ((i = ShouldDoTrainerSlide(GetBattlerAtPosition(B_POSITION_OPPONENT_LEFT),
...
u8 IsRunningFromBattleImpossible(u32 battler)
{
  ...
  if (GetBattlerPosition(battler) == B_POSITION_PLAYER_RIGHT && WILD_DOUBLE_BATTLE &&
      IsBattlerAlive(GetBattlerAtPosition(B_POSITION_PLAYER_LEFT)))  // The second pokemon cannot run from a double wild
                                                                     // battle, unless it's the only alive mon.
  {
  ...
  for (battler = 0; battler < gBattlersCount; battler++)
  {
    ...
    switch (gBattleCommunication[battler])
    {
            ...
            gBattleCommunication[GetBattlerAtPosition(BATTLE_PARTNER(position))] == STATE_WAIT_ACTION_CONFIRMED)
        {
          ...
          } else
          {
                       ...
                       gChosenActionByBattler[GetBattlerAtPosition(B_POSITION_PLAYER_LEFT)] != B_ACTION_NOTHING_FAINTED)
            {
              ...
              gChosenActionByBattler[battler] = B_ACTION_NOTHING_FAINTED;  // Not fainted, but it cannot move, because
                                                                           // of the throwing ball.
              gBattleCommunication[battler] = STATE_WAIT_ACTION_CONFIRMED_STANDBY;
            } else
            {
              gBattleStruct->itemPartyIndex[battler] = PARTY_SIZE;
              BtlController_EmitChooseAction(battler, BUFFER_A, gChosenActionByBattler[0],
                                             gBattleResources->bufferB[0][1] | (gBattleResources->bufferB[0][2] << 8));
              MarkBattlerForControllerExec(battler);
      ...
      case STATE_WAIT_ACTION_CHOSEN:  // Try to perform an action.
        if (!(gBattleControllerExecFlags &
              ((gBitTable[battler]) | (0xF << 28) | (gBitTable[battler] << 4) | (gBitTable[battler] << 8) |
          ...
          switch (gBattleResources->bufferB[battler][1])
          {
            case B_ACTION_USE_MOVE:
              ...
              } else
              {
                ...
                BtlController_EmitChooseMove(battler, BUFFER_A, (gBattleTypeFlags & BATTLE_TYPE_DOUBLE) != 0, FALSE,
                                             &moveInfo);
                MarkBattlerForControllerExec(battler);
              }
              break;
            case B_ACTION_USE_ITEM:
              ...
              if (((gBattleTypeFlags &
                    (BATTLE_TYPE_LINK | BATTLE_TYPE_FRONTIER_NO_PYRAMID | BATTLE_TYPE_EREADER_TRAINER |
            ...
            case B_ACTION_SWITCH:
              *(gBattleStruct->battlerPartyIndexes + battler) = gBattlerPartyIndexes[battler];
              ...
              {
                BtlController_EmitChoosePokemon(battler, BUFFER_A, PARTY_ACTION_CANT_SWITCH, PARTY_SIZE, ABILITY_NONE,
              ...
              {
                BtlController_EmitChoosePokemon(battler, BUFFER_A, ((i - 1) << 4) | PARTY_ACTION_ABILITY_PREVENTS,
                                                PARTY_SIZE, gBattleMons[i - 1].ability,
                                                gBattleStruct->battlerPartyOrders[battler]);
              } else
              {
                if (battler == 2 && gChosenActionByBattler[0] == B_ACTION_SWITCH)
                {
                  BtlController_EmitChoosePokemon(battler, BUFFER_A, PARTY_ACTION_CHOOSE_MON,
                ...
                {
                  BtlController_EmitChoosePokemon(battler, BUFFER_A, PARTY_ACTION_CHOOSE_MON, PARTY_SIZE, ABILITY_NONE,
                                                  gBattleStruct->battlerPartyOrders[battler]);
                }
              }
              MarkBattlerForControllerExec(battler);
            ...
            case B_ACTION_CANCEL_PARTNER:
              ...
              gBattleCommunication[GetBattlerAtPosition(
                  BATTLE_PARTNER(GetBattlerPosition(battler)))] = STATE_BEFORE_ACTION_CHOSEN;
              RecordedBattle_ClearBattlerAction(battler, 1);
              if (gBattleMons[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler)))].status2 &
                      STATUS2_MULTIPLETURNS ||
                  gBattleMons[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler)))].status2 &
                      STATUS2_RECHARGE)
              {
                BtlController_EmitEndBounceEffect(battler, BUFFER_A);
                MarkBattlerForControllerExec(battler);
                return;
              } else if (gChosenActionByBattler[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler)))] ==
                         B_ACTION_SWITCH)
              {
                         ...
                         B_ACTION_RUN)
              {
                RecordedBattle_ClearBattlerAction(GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler))), 1);
                             ...
                             B_ACTION_USE_MOVE &&
                         (gProtectStructs[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler)))]
                              .noValidMoves ||
                          gDisableStructs[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler)))]
                              .encoredMove))
              {
                RecordedBattle_ClearBattlerAction(GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler))), 1);
              } else if (gBattleTypeFlags & BATTLE_TYPE_PALACE &&
                         gChosenActionByBattler[GetBattlerAtPosition(BATTLE_PARTNER(GetBattlerPosition(battler)))] ==
                             B_ACTION_USE_MOVE)
              {
              ...
              switch (gBattleResources->bufferB[battler][1])
              {
                ...
                default:
                  ...
                  } else
                  {
                    ...
                    gChosenMoveByBattler[battler] = gBattleMons[battler]
                                                        .moves[gBattleStruct->chosenMovePositions[battler]];
                    gBattleStruct->moveTarget[battler] = gBattleResources->bufferB[battler][3];
                    ...
                    {
                      gBattleStruct->dynamax
                          .baseMove[battler] = gBattleMons[battler].moves[gBattleStruct->chosenMovePositions[battler]];
                    ...
                    if (gTestRunnerEnabled)
                    {
                      TestRunner_Battle_CheckChosenMove(battler, gChosenMoveByBattler[battler],
                                                        gBattleStruct->moveTarget[battler]);
                    }
                  }
                  break;
              }
...
u32 GetBattlerTotalSpeedStatArgs(u32 battler, u32 ability, u32 holdEffect)
{
  ...
  if (WEATHER_HAS_EFFECT)
  {
    ...
    {
      speed *= 2;
    } else if (ability == ABILITY_CHLOROPHYLL && holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA &&
  ...
  s8 GetMovePriority(u32 battler, u16 move)
  {
    ...
    // Grassy Glide bonus
    if (gBattleMoves[move].effect == EFFECT_GRASSY_GLIDE && (gFieldStatuses & STATUS_FIELD_GRASSY_TERRAIN) &&
                                ...
                                s32 priority1, s32 priority2)
  {
    ...
    if (priority1 == priority2)
    {
      ...
      {
        strikesFirst = 0;
      } else if (ability1 == ABILITY_MYCELIUM_MIGHT && ability2 != ABILITY_MYCELIUM_MIGHT &&
                 IS_MOVE_STATUS(gChosenMoveByBattler[battler1]))
      {
        strikesFirst = 1;
      } else if (ability2 == ABILITY_MYCELIUM_MIGHT && ability1 != ABILITY_MYCELIUM_MIGHT &&
                 IS_MOVE_STATUS(gChosenMoveByBattler[battler2]))
      {
        strikesFirst = 0;
      } else
      {
  ...
  u32 GetWhichBattlerFaster(u32 battler1, u32 battler2, bool32 ignoreChosenMoves)
  {
    ...
    return GetWhichBattlerFasterArgs(battler1, battler2, ignoreChosenMoves, ability1, ability2, holdEffectBattler1,
                                     holdEffectBattler2, speedBattler1, speedBattler2, priority1, priority2);
  }
  ...
  static void TurnValuesCleanUp(bool8 var0)
  {
    ...
    gSideStatuses[B_SIDE_PLAYER] &= ~(SIDE_STATUS_QUICK_GUARD | SIDE_STATUS_WIDE_GUARD | SIDE_STATUS_CRAFTY_SHIELD |
                                      SIDE_STATUS_MAT_BLOCK);
    gSideStatuses[B_SIDE_OPPONENT] &= ~(SIDE_STATUS_QUICK_GUARD | SIDE_STATUS_WIDE_GUARD | SIDE_STATUS_CRAFTY_SHIELD |
                                        SIDE_STATUS_MAT_BLOCK);
         ...
         gBattleStruct->tera.toTera))
    {
      ...
      for (i = 0; i < gBattlersCount; i++)
      {
        ...
        if (gBattleStruct->mega.toEvolve & gBitTable[order[i]] && !(gProtectStructs[order[i]].noValidMoves))
        {
          ...
          if (GetBattleFormChangeTargetSpecies(gBattlerAttacker, FORM_CHANGE_BATTLE_MEGA_EVOLUTION_MOVE) !=
              SPECIES_NONE)
          {
    ...
    if (!(gHitMarker & HITMARKER_RUN))
    {
      ...
      for (i = 0; i < gBattlersCount; i++)
      {
        if (!(gBattleStruct->focusPunchBattlers & gBitTable[battlers[i]]) &&
            !(gBattleMons[battlers[i]].status1 & STATUS1_SLEEP) && !(gDisableStructs[battlers[i]].truantCounter) &&
  ...
  static void TryChangingTurnOrderEffects(u32 battler1, u32 battler2)
  {
    ...
    // Battler 1
    // Quick Draw
    if (ability1 == ABILITY_QUICK_DRAW && !IS_MOVE_STATUS(gChosenMoveByBattler[battler1]) &&
        RandomPercentage(RNG_QUICK_DRAW, 30))
    {
    ...
    // Battler 2
    // Quick Draw
    if (ability2 == ABILITY_QUICK_DRAW && !IS_MOVE_STATUS(gChosenMoveByBattler[battler2]) &&
        RandomPercentage(RNG_QUICK_DRAW, 30))
    {
    ...
    if (!gProtectStructs[battler2].quickDraw &&
        ((holdEffectBattler2 == HOLD_EFFECT_QUICK_CLAW &&
          RandomPercentage(RNG_QUICK_CLAW, GetBattlerHoldEffectParam(battler2))) ||
    ...
    if (!(gHitMarker & HITMARKER_RUN))
    {
      while (gBattleStruct->quickClawBattlerId < gBattlersCount)
      {
        ...
        if (gChosenActionByBattler[battler] == B_ACTION_USE_MOVE &&
            ...
            !(gBattleMons[battler].status1 & STATUS1_SLEEP) && !(gDisableStructs[gBattlerAttacker].truantCounter) &&
    ...
    *(&gBattleStruct->savedTurnActionNumber) = gCurrentTurnActionNumber;
    ...
    } else
    {
      if (gBattleStruct->savedTurnActionNumber !=
          gCurrentTurnActionNumber)  // action turn has been done, clear hitmarker bits for another battler
      {
        gHitMarker &= ~HITMARKER_NO_ATTACKSTRING;
        gHitMarker &= ~HITMARKER_UNABLE_TO_USE_MOVE;
      }
  ...
  static void HandleEndTurn_FinishBattle(void)
  {
    ...
    {
      if (!(gBattleTypeFlags &
            (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_SAFARI |
             BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_WALLY_TUTORIAL | BATTLE_TYPE_FRONTIER)))
      {
        for (battler = 0; battler < gBattlersCount; battler++)
        {
          if (GetBattlerSide(battler) == B_SIDE_PLAYER)
          {
            ...
            {
              gBattleResults.playerMon1Species = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]],
                                                            MON_DATA_SPECIES, NULL);
              GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], MON_DATA_NICKNAME,
                         gBattleResults.playerMon1Name);
            } else
            {
              gBattleResults.playerMon2Species = GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]],
                                                            MON_DATA_SPECIES, NULL);
              GetMonData(&gPlayerParty[gBattlerPartyIndexes[battler]], MON_DATA_NICKNAME,
      ...
      if (!(gBattleTypeFlags &
            (BATTLE_TYPE_LINK | BATTLE_TYPE_RECORDED_LINK | BATTLE_TYPE_TRAINER | BATTLE_TYPE_FIRST_BATTLE |
      ...
      for (i = 0; i < PARTY_SIZE; i++)
      {
        ...
        // Natural Cure - Heal status conditions at the end of battle
        if (GetMonAbility(&gPlayerParty[i]) == ABILITY_NATURAL_CURE &&
            GetMonData(&gPlayerParty[i], MON_DATA_STATUS, NULL) != 0)
        {
          // Clear the status condition
          SetMonData(&gPlayerParty[i], MON_DATA_STATUS, 0);
        }

        // Regenerator - Restore 1/3 of max HP at the end of battle
        if (GetMonAbility(&gPlayerParty[i]) == ABILITY_REGENERATOR)
        {
          u16 maxHP = GetMonData(&gPlayerParty[i], MON_DATA_MAX_HP, NULL);
          u16 currentHP = GetMonData(&gPlayerParty[i], MON_DATA_HP, NULL);
          u16 healAmount = maxHP / 3;
          ...
          SetMonData(&gPlayerParty[i], MON_DATA_HP, &currentHP);
        }
  ...
  static void FreeResetData_ReturnToOvOrDoEvolutions(void)
  {
    if (!gPaletteFade.active)
    {
      ...
      if (!(gBattleTypeFlags &
          ...
          (B_EVOLUTION_AFTER_WHITEOUT >= GEN_6 || gBattleOutcome == B_OUTCOME_WON ||
  ...
  static void TrySpecialEvolution(
      void)  // Attempts to perform non-level related battle evolutions (not the script command).
  {
    s32 i;
  ...
  static void ReturnFromBattleToOverworld(void)
  {
    ...
    if (gBattleTypeFlags & BATTLE_TYPE_ROAMER)
    {
      ...
      if ((gBattleOutcome == B_OUTCOME_WON) ||
          gBattleOutcome ==
              B_OUTCOME_CAUGHT)  // Bug: When Roar is used by roamer, gBattleOutcome is B_OUTCOME_PLAYER_TELEPORTED (5).
...
// if you experience the follower de-syncing with the player after battle, set POST_BATTLE_FOLLOWER_FIX to TRUE in
// include/constants/global.h
  ...
  void SetTypeBeforeUsingMove(u32 move, u32 battlerAtk)
  {
    ...
    {
      u8 typeBits = ((gBattleMons[battlerAtk].hpIV & 1) << 0) | ((gBattleMons[battlerAtk].attackIV & 1) << 1) |
          ((gBattleMons[battlerAtk].defenseIV & 1) << 2) | ((gBattleMons[battlerAtk].speedIV & 1) << 3) |
          ((gBattleMons[battlerAtk].spAttackIV & 1) << 4) | ((gBattleMons[battlerAtk].spDefenseIV & 1) << 5);

      // Subtract 4 instead of 1 below because 3 types are excluded (TYPE_NORMAL and TYPE_MYSTERY and TYPE_FAIRY and
      // TYPE_STELLAR) The final + 1 skips past Normal, and the following conditional skips TYPE_MYSTERY
    ...
    if (gBattleMoves[move].type == TYPE_NORMAL && gBattleMoves[move].effect != EFFECT_HIDDEN_POWER &&
        gBattleMoves[move].effect != EFFECT_WEATHER_BALL && gBattleMoves[move].effect != EFFECT_CHANGE_TYPE_ON_ITEM &&
        gBattleMoves[move].effect != EFFECT_NATURAL_GIFT &&
        !(gBattleMoves[move].effect == EFFECT_TERA_BLAST && IsTerastallized(battlerAtk)) &&
        !(gBattleMoves[move].effect == EFFECT_TERA_STARSTORM &&
          gBattleMons[battlerAtk].species == SPECIES_TERAPAGOS_STELLAR) &&
    ...
    } else if (gBattleMoves[move].type != TYPE_NORMAL && gBattleMoves[move].effect != EFFECT_HIDDEN_POWER &&
               gBattleMoves[move].effect != EFFECT_WEATHER_BALL && attackerAbility == ABILITY_NORMALIZE)
    {
      gBattleStruct->dynamicMoveType = TYPE_NORMAL | F_DYNAMIC_TYPE_SET;
      if (!IsDynamaxed(battlerAtk))
      {
    ...
    } else if ((gBattleMoves[move].flags & FLAG_SOUND_BASED) && attackerAbility == ABILITY_LIQUID_VOICE)
    {
      gBattleStruct->dynamicMoveType = TYPE_WATER | F_DYNAMIC_TYPE_SET;
