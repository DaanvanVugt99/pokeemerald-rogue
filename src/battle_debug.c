...
static const struct BitfieldInfo sStatus1Bitfield[] = {
    {/*Sleep*/ 3, 0},     {/*Poison*/ 1, 3},       {/*Burn*/ 1, 4},          {/*Freeze*/ 1, 5},
    {/*Paralysis*/ 1, 6}, {/*Toxic Poison*/ 1, 7}, {/*Toxic Counter*/ 4, 8}, {/*Frostbite*/ 1, 12},
};
...
static const struct BitfieldInfo sAIBitfield[] = {
    {/*Check bad move*/ 1, 0},    {/*Try To Faint*/ 1, 1}, {/*Viability*/ 1, 2},
    {/*Set up first turn*/ 1, 3}, {/*Risky*/ 1, 4},        {/*Prefer Strongest Move*/ 1, 5},
    {/*Prefer Baton Pass*/ 1, 6}, {/*In Doubles*/ 1, 7},   {/*Hp aware*/ 1, 8},
    {/*Unknown*/ 1, 9},
};
...
static const struct ListMenuItem sAIListItems[] = {
    {sText_NoBadMoves, 0},      {sText_TryFaint, 1},  {sText_Viability, 2},
    {sText_SetUpFirstTurn, 3},  {sText_Risky, 4},     {sText_StrongestMove, 5},
    {sText_PreferBatonPass, 6}, {sText_InDoubles, 7}, {sText_HpAware, 8},
    // {sText_Unknown, 9},
};

static const struct ListMenuItem sStatsListItems[] = {
    {sText_CurrHp, 0}, {sText_MaxHp, 1}, {gText_Attack, 2}, {gText_Defense, 3},
    {gText_Speed, 4},  {gText_SpAtk, 5}, {gText_SpDef, 6},
};

static const struct ListMenuItem sStatus1ListItems[] = {
    {gText_Sleep, 0},     {gText_Poison, 1},      {gText_Burn, 2},         {sText_Freeze, 3},
    {gText_Paralysis, 4}, {sText_ToxicPoison, 5}, {sText_ToxicCounter, 6}, {sText_Frostbite, 7},
};

static const struct ListMenuItem sStatus2ListItems[] = {
    {gText_Confusion, 0},     {sText_Flinch, 1},      {sText_Uproar, 2},     {sText_Bide, 3},    {sText_LockConfuse, 4},
    {sText_MultipleTurns, 5}, {sText_FocusEnergy, 6}, {sText_Recharge, 7},   {sText_Rage, 8},    {sText_Substitute, 9},
    {sText_DestinyBond, 10},  {sText_CantEscape, 11}, {sText_Nightmare, 12}, {sText_Cursed, 13}, {sText_Foresight, 14},
    {sText_DefenseCurl, 15},  {sText_Tormented, 16},
};

static const struct ListMenuItem sStatus3ListItems[] = {
    {sText_AlwaysHits, 0}, {sText_Minimized, 1},  {sText_ChargedUp, 2}, {sText_Rooted, 3},
    {sText_Yawned, 4},     {sText_Imprisoned, 5}, {sText_NoCrit, 6},    {sText_GastroAcid, 7},
    {sText_SmackDown, 8},  {sText_MiracleEye, 9}, {sText_AquaRing, 10}, {sText_LaserFocus, 11},
};
...
static const struct ListMenuTemplate sMainListTemplate = {.items = sMainListItems,
                                                          .moveCursorFunc = NULL,
                                                          .itemPrintFunc = NULL,
...
static const struct ListMenuTemplate sSecondaryListTemplate = {.items = sSecondaryListItems,
                                                               .moveCursorFunc = NULL,
                                                               .itemPrintFunc = NULL,
                                                               .totalItems = 0,
                                                               .maxShowed = 0,
...
static const struct WindowTemplate sMainListWindowTemplate =
    {.bg = 0, .tilemapLeft = 1, .tilemapTop = 3, .width = 9, .height = 12, .paletteNum = 0xF, .baseBlock = 0x2};

static const struct WindowTemplate sSecondaryListWindowTemplate =
    {.bg = 0, .tilemapLeft = 12, .tilemapTop = 3, .width = 10, .height = 2, .paletteNum = 0xF, .baseBlock = 0xA0};

static const struct WindowTemplate sModifyWindowTemplate =
    {.bg = 0, .tilemapLeft = 25, .tilemapTop = 2, .width = 4, .height = 2, .paletteNum = 0xF, .baseBlock = 0x200};

static const struct WindowTemplate sBattlerWindowTemplate =
    {.bg = 0, .tilemapLeft = 10, .tilemapTop = 0, .width = 14, .height = 2, .paletteNum = 0xF, .baseBlock = 0x300};
...
static const bool8 sHasChangeableEntries[LIST_ITEM_COUNT] = {
    [LIST_ITEM_MOVES] = TRUE,       [LIST_ITEM_AI_MOVES_PTS] = TRUE, [LIST_ITEM_PP] = TRUE,
    [LIST_ITEM_ABILITY] = TRUE,     [LIST_ITEM_TYPES] = TRUE,        [LIST_ITEM_HELD_ITEM] = TRUE,
...
static void PutMovesPointsText(struct BattleDebugMenu* data)
{
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
  {
    ...
    for (count = 0, j = 0; j < MAX_BATTLERS_COUNT; j++)
    {
      ...
      ConvertIntToDecimalStringN(text, gBattleStruct->aiFinalScore[data->aiBattlerId][battlerDef][i],
                                 STR_CONV_MODE_RIGHT_ALIGN, 3);
      AddTextPrinterParameterized(data->aiMovesWindowId, FONT_NORMAL, text, 83 + count * 54, i * 15, 0, NULL);

      ConvertIntToDecimalStringN(text, AI_DATA->simulatedDmg[data->aiBattlerId][battlerDef][i],
                                 STR_CONV_MODE_RIGHT_ALIGN, 3);
  ...
  switch (data->aiViewState)
  {
    case 0:
      ...
      for (count = 0, i = 0; i < MAX_BATTLERS_COUNT; i++)
      {
        ...
        {
          data->spriteIds.aiIconSpriteIds[i] = CreateMonIcon(gBattleMons[i].species, SpriteCallbackDummy,
                                                             95 + (count * 60), 17, 0, 0,
                                                             GetGenderForSpecies(gBattleMons[i].species,
                                                                                 gBattleMons[i].genderFlag));
      ...
      data->aiMonSpriteId = CreateMonPicSprite(gBattleMons[data->aiBattlerId].species,
                                               ...
                                               GetGenderForSpecies(gBattleMons[data->aiBattlerId].species,
                                                                   gBattleMons[data->aiBattlerId].genderFlag),
                                               TRUE, 39, 130, 15, TAG_NONE);
      data->aiViewState++;
      break;
    // Put text
    case 1:
...
static void PutAiPartyText(struct BattleDebugMenu* data)
{
  ...
  for (i = 0; i < count; i++)
  {
    ...
    txtPtr = StringCopyN(text, gAbilityNames[aiMons[i].ability],
                         7);  // The screen is too small to fit the whole string, so we need to drop the last letters.
    *txtPtr = EOS;
    ...
    *txtPtr = EOS;
    AddTextPrinterParameterized5(data->aiMovesWindowId, FONT_SMALL_NARROW, text, i * 41, 35 + (j + 1) * 15, 0, NULL, 0,
                                 0);
  }
  ...
  switch (data->aiViewState)
  {
    case 0:
      ...
      for (i = 0; i < AI_PARTY->count[GetBattlerSide(data->aiBattlerId)]; i++)
      {
        ...
        gSprites[data->spriteIds.aiPartyIcons[i]].sConditionSpriteId = CreateSprite(&gSpriteTemplate_StatusIcons,
                                                                                    (i * 41) + 15, 7, 0);
...
static void UpdateBattlerValue(struct BattleDebugMenu* data)
{
  ...
  switch (data->modifyArrows.typeOfVal)
  {
    ...
    case VAL_BITFIELD_32:
      *(u32*)(data->modifyArrows.modifiedValPtr) &= ~(
          GetBitfieldToAndValue(data->bitfield[data->currentSecondaryListItemId].currBit,
                                data->bitfield[data->currentSecondaryListItemId].bitsCount));
      *(u32*)(data->modifyArrows.modifiedValPtr) |= (data->modifyArrows.currValue
                                                     << data->bitfield[data->currentSecondaryListItemId].currBit);
      break;
    ...
    case VAR_IN_LOVE:
      if (data->modifyArrows.currValue)
      {
        ...
        {
          gBattleMons[data->battlerId].status2 |= STATUS2_INFATUATED_WITH(
              BATTLE_PARTNER(BATTLE_OPPOSITE(data->battlerId)));
        }
      } else
      {
...
static void SetUpModifyArrows(struct BattleDebugMenu* data)
{
  ...
  switch (data->currentMainListItemId)
  {
    ...
    case LIST_ITEM_PP:
      data->modifyArrows.minValue = 0;
      data->modifyArrows
          .maxValue = CalculatePPWithBonus(gBattleMons[data->battlerId].moves[data->currentSecondaryListItemId],
                                           gBattleMons[data->battlerId].ppBonuses, data->currentSecondaryListItemId);
    ...
    case LIST_ITEM_TYPES:
      ...
      data->modifyArrows.maxDigits = 2;
      data->modifyArrows.modifiedValPtr = (u8*)((&gBattleMons[data->battlerId].type1) +
                                                data->currentSecondaryListItemId);
      data->modifyArrows.typeOfVal = VAL_U8;
    ...
    case LIST_ITEM_STATS:
      ...
      {
        data->modifyArrows.modifiedValPtr = (u16*)((&gBattleMons[data->battlerId].attack) +
                                                   (data->currentSecondaryListItemId - 2));
        data->modifyArrows.currValue = *(u16*)((&gBattleMons[data->battlerId].attack) +
                                               (data->currentSecondaryListItemId - 2));
      }
      data->modifyArrows.typeOfVal = VAL_U16;
      break;
    case LIST_ITEM_STAT_STAGES:
      ...
      {
        data->modifyArrows.modifiedValPtr = &gBattleMons[data->battlerId]
                                                 .statStages[data->currentSecondaryListItemId + STAT_ATK];
        data->modifyArrows.typeOfVal = VAL_U8;
        data->modifyArrows.currValue = gBattleMons[data->battlerId]
                                           .statStages[data->currentSecondaryListItemId + STAT_ATK];
      }
      break;
    case LIST_ITEM_VARIOUS:
    ...
    case LIST_ITEM_STATUS1:
      ...
      data->modifyArrows.currValue = GetBitfieldValue(gBattleMons[data->battlerId].status1,
                                                      data->bitfield[data->currentSecondaryListItemId].currBit,
                                                      data->bitfield[data->currentSecondaryListItemId].bitsCount);
    ...
    case LIST_ITEM_STATUS2:
      ...
      data->modifyArrows.currValue = GetBitfieldValue(gBattleMons[data->battlerId].status2,
                                                      data->bitfield[data->currentSecondaryListItemId].currBit,
                                                      data->bitfield[data->currentSecondaryListItemId].bitsCount);
      data->modifyArrows.typeOfVal = VAL_BITFIELD_32;
      goto CASE_ITEM_STATUS;
    case LIST_ITEM_STATUS3:
      ...
      data->modifyArrows.currValue = GetBitfieldValue(gStatuses3[data->battlerId],
                                                      data->bitfield[data->currentSecondaryListItemId].currBit,
                                                      data->bitfield[data->currentSecondaryListItemId].bitsCount);
      data->modifyArrows.typeOfVal = VAL_BITFIELD_32;
      goto CASE_ITEM_STATUS;
    case LIST_ITEM_STATUS4:
      ...
      data->modifyArrows.currValue = GetBitfieldValue(gStatuses4[data->battlerId],
                                                      data->bitfield[data->currentSecondaryListItemId].currBit,
                                                      data->bitfield[data->currentSecondaryListItemId].bitsCount);
      data->modifyArrows.typeOfVal = VAL_BITFIELD_32;
      goto CASE_ITEM_STATUS;
    case LIST_ITEM_AI:
      ...
      data->modifyArrows.currValue = GetBitfieldValue(gBattleResources->ai->aiFlags,
                                                      data->bitfield[data->currentSecondaryListItemId].currBit,
                                                      data->bitfield[data->currentSecondaryListItemId].bitsCount);
      data->modifyArrows.typeOfVal = VAL_BITFIELD_32;
      goto CASE_ITEM_STATUS;
    ...
    case LIST_ITEM_SIDE_STATUS:
      ...
      if (data->currentSecondaryListItemId == LIST_SIDE_SPIKES)
      {
        data->modifyArrows.maxValue = 3;
                 ...
                 data->currentSecondaryListItemId == LIST_SIDE_STICKY_WEB)
      {
        data->modifyArrows.maxValue = 1;
      } else
      {
