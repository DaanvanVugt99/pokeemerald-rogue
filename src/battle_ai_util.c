...
static const s8 sAiAbilityRatings[ABILITIES_COUNT] =
{
    ...
    [ABILITY_BIG_PECKS] = 1,
    [ABILITY_BLAZE] = 5,
    [ABILITY_BONE_KEEPER] = 6,
    [ABILITY_BULLETPROOF] = 7,
    [ABILITY_NATURAL_ORDER] = 7,
    [ABILITY_PRESSURE_VALVE] = 5,
    ...
    [ABILITY_FLUFFY] = 5,
    [ABILITY_AURA_GUARD] = 5,
    [ABILITY_FORECAST] = 6,
    [ABILITY_FOREWARN] = 2,
    [ABILITY_FRIEND_GUARD] = 0,
    [ABILITY_FRISK] = 3,
    [ABILITY_FUNERAL_DANCE] = 6,
...
static bool32 ShouldFailForIllusion(u32 illusionSpecies, u32 battlerId)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // The used move can be learned from Move Tutors.
        for (j = 0; gRoguePokemonProfiles[illusionSpecies].tutorMoves[j] != MOVE_NONE; j++)
        {
            if (gRoguePokemonProfiles[illusionSpecies].tutorMoves[j] == move)
                break;
        }
        if (gRoguePokemonProfiles[illusionSpecies].tutorMoves[j] != MOVE_NONE)
            continue;

        // The used move can be learned from a TM or HM.
        if (CanSpeciesLearnTM(illusionSpecies, move))
            continue;
...
u32 AI_GetBattlerMoveTargetType(u32 battlerId, u32 move)
{
    ...
    else if (HasBattlerAbility(battlerId, ABILITY_ARRAY_PROTOCOL)
...
bool32 ShouldAbsorb(u32 battlerAtk, u32 battlerDef, u32 move, s32 damage)
{
    if (move == 0xFFFF || AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER)
    {
        ...
        if (IsBattlerHealBlocked(battlerAtk))
            healDmg = 0;
...
bool32 PartnerMoveEffectIsTerrain(u32 battlerAtkPartner, u32 partnerMove)
{
    ...
    if (partnerMove != MOVE_NONE
     && (gBattleMoves[partnerMove].effect == EFFECT_GRASSY_TERRAIN
      ...
      || gBattleMoves[partnerMove].effect == EFFECT_INFESTED_TERRAIN
      || gBattleMoves[partnerMove].effect == EFFECT_PLAIN_TERRAIN))
        return TRUE;
...
bool32 ShouldUseZMove(u32 battlerAtk, u32 battlerDef, u32 chosenMove)
{
    ...
    if (IsViableZMove(battlerAtk, chosenMove))
    {
        ...
        if (gBattleMons[battlerDef].ability == ABILITY_DISGUISE && IsBattlerDisguiseIntact(battlerDef))
