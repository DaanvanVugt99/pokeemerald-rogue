...
{
  memcpy(AI_PARTY->mons[GetBattlerSide(battler)][gBattlerPartyIndexes[battler]].moves, gBattleMons[battler].moves,
         MAX_MON_MOVES * sizeof(u16));
}
...
void SaveBattlerData(u32 battlerId)
{
  ...
  // Save and restore types even for AI controlled battlers in case it gets
  // changed during move evaluation process.
...
void SetBattlerData(u32 battlerId)
{
  if (!BattlerHasAi(battlerId))
  {
    ...
    {
      // If the battler's type has not been changed, AI assumes the types of the
      // illusion mon.
...
bool32 IsTruantMonVulnerable(u32 battlerAI, u32 opposingBattler)
{
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
  {
    ...
    if (gBattleMoves[move].effect == EFFECT_SEMI_INVULNERABLE &&
        AI_WhoStrikesFirst(battlerAI, opposingBattler, GetAIChosenMove(battlerAI)) == AI_IS_SLOWER)
    {
      return TRUE;
    }
  }
...
{
  if (ability == ABILITY_OVERCOAT || (B_POWDER_GRASS >= GEN_6 && IS_BATTLER_OF_TYPE(battler, TYPE_GRASS)) ||
...
// This function checks if all physical/special moves are either unusable or
// unreasonable to use. Consider a pokemon boosting their attack against a ghost
// pokemon having only normal-type physical attacks.
...
// To save computation time this function has 2 variants. One saves, sets and
// restores battlers, while the other doesn't.
...
s32 AI_CalcDamage(u32 move, u32 battlerAtk, u32 battlerDef, u8* typeEffectiveness, bool32 considerZPower, u32 weather)
{
  ...
  effectivenessMultiplier = CalcTypeEffectivenessMultiplier(move, moveType, battlerAtk, battlerDef,
                                                            aiData->abilities[battlerDef], FALSE);
  if (gBattleMoves[move].power)
  {
    ...
    // Certain moves like Rollout calculate damage based on values which change
    // during the move execution, but before calling dmg calc.
    switch (gBattleMoves[move].effect)
    {
      ...
      case EFFECT_FURY_CUTTER:
        fixedBasePower = CalcFuryCutterBasePower(gBattleMoves[move].power,
    ...
    normalDmg = CalculateMoveDamageVars(move, battlerAtk, battlerDef, moveType, fixedBasePower, effectivenessMultiplier,
                                        weather, FALSE, aiData->holdEffects[battlerAtk],
                                        aiData->holdEffects[battlerDef], aiData->abilities[battlerAtk],
                                        aiData->abilities[battlerDef]);

    critChanceIndex = CalcCritChanceStageArgs(battlerAtk, battlerDef, move, FALSE, aiData->abilities[battlerAtk],
    ...
    if (critChanceIndex > 1)  // Consider crit damage only if a move has at least +1 crit chance
    {
      s32 critDmg = CalculateMoveDamageVars(move, battlerAtk, battlerDef, moveType, fixedBasePower,
                                            effectivenessMultiplier, weather, TRUE, aiData->holdEffects[battlerAtk],
                                            aiData->holdEffects[battlerDef], aiData->abilities[battlerAtk],
                                            aiData->abilities[battlerDef]);
    ...
    if (!gBattleStruct->zmove.active)
    {
      ...
      switch (gBattleMoves[move].effect)
      {
        ...
        case EFFECT_SUPER_FANG:
          dmg = (aiData->abilities[battlerAtk] == ABILITY_PARENTAL_BOND ? max(2, gBattleMons[battlerDef].hp * 3 / 4)
      ...
      // Apply double-strike multiplier from flags BEFORE the switch handles
      // other effects
  ...
  switch (gBattleMoves[move].effect)
  {
    ...
    case EFFECT_HIT_ESCAPE:
      if (CountUsablePartyMons(battlerAtk) != 0 &&
          ShouldPivot(battlerAtk, battlerDef, abilityDef, move, AI_THINKING_STRUCT->movesetIndex))
      {
        return TRUE;
      }
      break;
...
// Checks if one of the moves has side effects or perks, assuming equal dmg or
// equal no of hits to KO
u32 AI_WhichMoveBetter(u32 move1, u32 move2, u32 battlerAtk, u32 battlerDef, s32 noOfHitsToKo)
{
  ...
  if (AI_DATA->holdEffects[battlerAtk] != HOLD_EFFECT_PROTECTIVE_PADS &&
      (AI_DATA->holdEffects[battlerDef] == HOLD_EFFECT_ROCKY_HELMET || defAbility == ABILITY_IRON_BARBS ||
...
uq4_12_t AI_GetTypeEffectiveness(u32 move, u32 battlerAtk, u32 battlerDef)
{
  ...
  SetTypeBeforeUsingMove(move, battlerAtk);
  GET_MOVE_TYPE(move, moveType);
  typeEffectiveness = CalcTypeEffectivenessMultiplier(move, moveType, battlerAtk, battlerDef,
                                                      AI_DATA->abilities[battlerDef], FALSE);
...
u32 AI_WhoStrikesFirst(u32 battlerAI, u32 battler2, u32 moveConsidered)
{
  ...
  } else
  {
    if (prioAI > prioBattler2)
    {
      return AI_IS_FASTER;  // if we didn't know any of battler 2's moves to
                            // compare priorities, assume they don't have a
                            // prio+ move
    }
    // Priorities are the same(at least comparing to moves the AI is aware of),
    // decide by speed.
    if (GetWhichBattlerFasterArgs(battlerAI, battler2, TRUE, AI_DATA->abilities[battlerAI],
                                  AI_DATA->abilities[battler2], AI_DATA->holdEffects[battlerAI],
                                  AI_DATA->holdEffects[battler2], AI_DATA->speedStats[battlerAI],
                                  AI_DATA->speedStats[battler2], prioAI, prioBattler2) == 0)
    {
      return AI_IS_FASTER;
    } else
    {
...
// If numHits > 1, check if the target will be KO'ed by that number of hits
// (ignoring healing effects)
bool32 CanAIFaintTarget(u32 battlerAtk, u32 battlerDef, u32 numHits)
{
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
  {
    ...
    {
      // Use the pre-calculated value in simulatedDmg instead of re-calculating
      // it
...
s32 AI_DecideKnownAbilityForTurn(u32 battlerId)
{
  ...
  // We've had ability overwritten by e.g. Worry Seed. It is not part of
  // AI_PARTY in case of switching
...
// different from IsBattlerGrounded in that we don't always know battler's hold
// effect or ability
...
bool32 IsMoveRedirectionPrevented(u32 move, u32 atkAbility)
{
  ...
  if (move == MOVE_SKY_DROP || move == MOVE_SNIPE_SHOT || atkAbility == ABILITY_PROPELLER_TAIL ||
...
bool32 IsMoveEncouragedToHit(u32 battlerAtk, u32 battlerDef, u32 move)
{
  ...
  if (B_TOXIC_NEVER_MISS >= GEN_6 && gBattleMoves[move].effect == EFFECT_TOXIC &&
  ...
  if (((IsBattlerWeatherAffected(battlerAtk, B_WEATHER_RAIN)) &&
      ...
      (B_MINIMIZE_DMG_ACC >= GEN_6 && (gStatuses3[battlerDef] & STATUS3_MINIMIZED) &&
...
bool32 ShouldTryOHKO(u32 battlerAtk, u32 battlerDef, u32 atkAbility, u32 defAbility, u32 move)
{
  ...
  {
    return FALSE;  // probabilistically speaking, focus band should activate so
                   // dont OHKO
...
bool32 ShouldSetSandstorm(u32 battler, u32 ability, u32 holdEffect)
{
  ...
  if (ability == ABILITY_SAND_VEIL || ability == ABILITY_SAND_RUSH || ability == ABILITY_SAND_FORCE ||
      ability == ABILITY_OVERCOAT || ability == ABILITY_MAGIC_GUARD || holdEffect == HOLD_EFFECT_SAFETY_GOGGLES ||
      IS_BATTLER_OF_TYPE(battler, TYPE_ROCK) || IS_BATTLER_OF_TYPE(battler, TYPE_STEEL) ||
...
bool32 ShouldSetHail(u32 battler, u32 ability, u32 holdEffect)
{
  ...
  if (ability == ABILITY_SNOW_CLOAK || ability == ABILITY_ICE_BODY || IS_FORECAST_ABILITY(ability) ||
      ability == ABILITY_SLUSH_RUSH || ability == ABILITY_MAGIC_GUARD || ability == ABILITY_OVERCOAT ||
      holdEffect == HOLD_EFFECT_SAFETY_GOGGLES || IS_BATTLER_OF_TYPE(battler, TYPE_ICE) ||
      HasMove(battler, MOVE_BLIZZARD) || HasMoveEffect(battler, EFFECT_AURORA_VEIL) ||
      HasMoveEffect(battler, EFFECT_WEATHER_BALL))
  {
    return TRUE;
  }
...
bool32 ShouldSetRain(u32 battlerAtk, u32 atkAbility, u32 holdEffect)
{
  ...
  if (holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA &&
      (atkAbility == ABILITY_SWIFT_SWIM || IS_FORECAST_ABILITY(atkAbility) || atkAbility == ABILITY_HYDRATION ||
       atkAbility == ABILITY_RAIN_DISH || atkAbility == ABILITY_DRY_SKIN ||
       HasMoveEffect(battlerAtk, EFFECT_ELECTRO_SHOT) || HasMoveEffect(battlerAtk, EFFECT_THUNDER) ||
       HasMoveEffect(battlerAtk, EFFECT_HURRICANE) || HasMoveEffect(battlerAtk, EFFECT_WEATHER_BALL) ||
...
bool32 ShouldSetSun(u32 battlerAtk, u32 atkAbility, u32 holdEffect)
{
  ...
  if (holdEffect != HOLD_EFFECT_UTILITY_UMBRELLA &&
      (atkAbility == ABILITY_CHLOROPHYLL || atkAbility == ABILITY_FLOWER_GIFT || IS_FORECAST_ABILITY(atkAbility) ||
       atkAbility == ABILITY_LEAF_GUARD || atkAbility == ABILITY_SOLAR_POWER || atkAbility == ABILITY_HARVEST ||
       HasMoveEffect(battlerAtk, EFFECT_SOLAR_BEAM) || HasMoveEffect(battlerAtk, EFFECT_MORNING_SUN) ||
       HasMoveEffect(battlerAtk, EFFECT_SYNTHESIS) || HasMoveEffect(battlerAtk, EFFECT_MOONLIGHT) ||
       HasMoveEffect(battlerAtk, EFFECT_WEATHER_BALL) || HasMoveEffect(battlerAtk, EFFECT_GROWTH) ||
...
bool32 ShouldSetSnow(u32 battler, u32 ability, u32 holdEffect)
{
  ...
  if (ability == ABILITY_SNOW_CLOAK || ability == ABILITY_ICE_BODY || IS_FORECAST_ABILITY(ability) ||
      ability == ABILITY_SLUSH_RUSH || IS_BATTLER_OF_TYPE(battler, TYPE_ICE) || HasMove(battler, MOVE_BLIZZARD) ||
      HasMoveEffect(battler, EFFECT_AURORA_VEIL) || HasMoveEffect(battler, EFFECT_WEATHER_BALL))
  {
    return TRUE;
  }
  return FALSE;
...
bool32 ShouldSetAcidRain(u32 battler, u32 ability, u32 holdEffect)
{
  ...
  if (IS_BATTLER_OF_TYPE(battler, TYPE_BUG) || IS_BATTLER_OF_TYPE(battler, TYPE_POISON) ||
      HasMoveEffect(battler, EFFECT_WEATHER_BALL))
  {
    return TRUE;
  }
...
void ProtectChecks(u32 battlerAtk, u32 battlerDef, u32 move, u32 predictedMove, s32* score)
{
  ...
  /*if (GetMoveResultFlags(predictedMove) & (MOVE_RESULT_NO_EFFECT |
  MOVE_RESULT_MISSED))
  {
      ADJUST_SCORE_PTR(-5);
      return;
...
bool32 ShouldLowerStat(u32 battler, u32 battlerAbility, u32 stat)
{
  ...
  {
    if (AI_DATA->holdEffects[battler] == HOLD_EFFECT_CLEAR_AMULET || battlerAbility == ABILITY_CLEAR_BODY ||
        battlerAbility == ABILITY_WHITE_SMOKE || battlerAbility == ABILITY_FULL_METAL_BODY)
    {
      return FALSE;
    }

    // If AI is faster and doesn't have any mons left, lowering speed doesn't
    // give any
    if (stat == STAT_SPEED)
    {
      if (AI_WhoStrikesFirst(sBattler_AI, battler, AI_THINKING_STRUCT->moveConsidered) == AI_IS_FASTER &&
          CountUsablePartyMons(sBattler_AI) == 0 && !HasMoveEffect(sBattler_AI, EFFECT_ELECTRO_BALL))
      {
        return FALSE;
      }
    }
...
bool32 ShouldLowerAttack(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  if (AI_STRIKES_FIRST(battlerAtk, battlerDef, AI_THINKING_STRUCT->moveConsidered) &&
      (AI_THINKING_STRUCT->aiFlags & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0))
  {
    return FALSE;  // Don't bother lowering stats if can kill enemy.
  }

  if (gBattleMons[battlerDef].statStages[STAT_ATK] > 4 && HasMoveWithSplit(battlerDef, SPLIT_PHYSICAL) &&
      defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEAR_BODY && defAbility != ABILITY_WHITE_SMOKE &&
      defAbility != ABILITY_FULL_METAL_BODY && defAbility != ABILITY_HYPER_CUTTER &&
...
bool32 ShouldLowerDefense(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  ...
  if (gBattleMons[battlerDef].statStages[STAT_DEF] > 4 && HasMoveWithSplit(battlerAtk, SPLIT_PHYSICAL) &&
      defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEAR_BODY && defAbility != ABILITY_WHITE_SMOKE &&
      defAbility != ABILITY_FULL_METAL_BODY && defAbility != ABILITY_BIG_PECKS &&
      AI_DATA->holdEffects[battlerDef] != HOLD_EFFECT_CLEAR_AMULET)
  {
    return TRUE;
  }
...
bool32 ShouldLowerSpeed(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  ...
  if (!AI_STRIKES_FIRST(battlerAtk, battlerDef, AI_THINKING_STRUCT->moveConsidered) && defAbility != ABILITY_CONTRARY &&
      defAbility != ABILITY_CLEAR_BODY && defAbility != ABILITY_FULL_METAL_BODY && defAbility != ABILITY_WHITE_SMOKE &&
      AI_DATA->holdEffects[battlerDef] != HOLD_EFFECT_CLEAR_AMULET)
  {
    return TRUE;
  }
...
bool32 ShouldLowerSpAtk(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  ...
  if (gBattleMons[battlerDef].statStages[STAT_SPATK] > 4 && HasMoveWithSplit(battlerDef, SPLIT_SPECIAL) &&
      defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEAR_BODY && defAbility != ABILITY_FULL_METAL_BODY &&
      defAbility != ABILITY_WHITE_SMOKE && AI_DATA->holdEffects[battlerDef] != HOLD_EFFECT_CLEAR_AMULET)
  {
    return TRUE;
  }
  return FALSE;
...
bool32 ShouldLowerSpDef(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  ...
  if (gBattleMons[battlerDef].statStages[STAT_SPDEF] > 4 && HasMoveWithSplit(battlerAtk, SPLIT_SPECIAL) &&
      defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEAR_BODY && defAbility != ABILITY_FULL_METAL_BODY &&
      defAbility != ABILITY_WHITE_SMOKE && AI_DATA->holdEffects[battlerDef] != HOLD_EFFECT_CLEAR_AMULET)
  {
    return TRUE;
...
bool32 ShouldLowerAccuracy(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  ...
  if (defAbility != ABILITY_CONTRARY && defAbility != ABILITY_CLEAR_BODY && defAbility != ABILITY_WHITE_SMOKE &&
      defAbility != ABILITY_FULL_METAL_BODY && defAbility != ABILITY_KEEN_EYE && defAbility != ABILITY_MINDS_EYE &&
...
bool32 ShouldLowerEvasion(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  ...
  if (gBattleMons[battlerDef].statStages[STAT_EVASION] > DEFAULT_STAT_STAGE && defAbility != ABILITY_CONTRARY &&
      defAbility != ABILITY_CLEAR_BODY && defAbility != ABILITY_FULL_METAL_BODY && defAbility != ABILITY_WHITE_SMOKE &&
      AI_DATA->holdEffects[battlerDef] != HOLD_EFFECT_CLEAR_AMULET)
  {
    return TRUE;
...
bool32 HasMoveWithLowAccuracy(u32 battlerAtk,
                              u32 battlerDef,
                              u32 accCheck,
                              bool32 ignoreStatus,
                              u32 atkAbility,
...
bool32 HasSleepMoveWithLowAccuracy(u32 battlerAtk, u32 battlerDef)
{
  ...
  for (i = 0; i < MAX_MON_MOVES; i++)
  {
    ...
    if (!(gBitTable[i] & moveLimitations))
    {
      if (gBattleMoves[moves[i]].effect == EFFECT_SLEEP && AI_DATA->moveAccuracy[battlerAtk][battlerDef][i] < 85)
      {
        return TRUE;
      }
    }
...
{
  if (!IS_BATTLER_OF_TYPE(battlerId, TYPE_ROCK) && !IS_BATTLER_OF_TYPE(battlerId, TYPE_GROUND) &&
      !IS_BATTLER_OF_TYPE(battlerId, TYPE_STEEL) && ability != ABILITY_SAND_VEIL && ability != ABILITY_SAND_FORCE &&
      ability != ABILITY_SAND_RUSH && ability != ABILITY_OVERCOAT)
  {
    return TRUE;
  }
  return FALSE;
...
{
  if (!IS_BATTLER_OF_TYPE(battlerId, TYPE_ICE) && ability != ABILITY_SNOW_CLOAK && ability != ABILITY_OVERCOAT &&
...
u32 GetBattlerSecondaryDamage(u32 battlerId)
{
  ...
  secondaryDamage = GetLeechSeedDamage(battlerId) + GetNightmareDamage(battlerId) + GetCurseDamage(battlerId) +
      GetTrapDamage(battlerId) + GetPoisonDamage(battlerId) + GetWeatherDamage(battlerId);
...
{
  if (GetBattlerSecondaryDamage(battler) != 0 && gBattleMons[battler].hp <= gBattleMons[battler].maxHP / 16)
  {
    return TRUE;
  }
  return FALSE;
...
static bool32 PartyBattlerShouldAvoidHazards(u32 currBattler, u32 switchBattler)
{
  ...
  u32 flags = gSideStatuses[GetBattlerSide(currBattler)] &
      (SIDE_STATUS_SPIKES | SIDE_STATUS_STEALTH_ROCK | SIDE_STATUS_STICKY_WEB | SIDE_STATUS_TOXIC_SPIKES);
  s32 hazardDamage = 0;
...
{
  bool32 hasStatBoost = AnyUsefulStatIsRaised(battlerAtk) ||
      gBattleMons[battlerDef].statStages[STAT_EVASION] >= 9;  // Significant boost in evasion for any class
  bool32 shouldSwitch;
  u32 battlerToSwitch;
  ...
  if (!IsDoubleBattle())
  {
    ...
    /*if (IsPredictedToSwitch(battlerDef, battlerAtk) && !hasStatBoost)
        return PIVOT; // Try pivoting so you can switch to a better matchup to
       counter your new opponent*/

    if (AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER)  // Attacker goes first
    {
      if (!CanAIFaintTarget(battlerAtk, battlerDef,
                            0))  // Can't KO foe otherwise
      {
        if (CanAIFaintTarget(battlerAtk, battlerDef, 2))
        {
          ...
          if (!IS_MOVE_STATUS(move) &&
              (shouldSwitch ||
               (AtMaxHp(battlerDef) &&
                (AI_DATA->holdEffects[battlerDef] == HOLD_EFFECT_FOCUS_SASH ||
                 (B_STURDY >= GEN_5 && defAbility == ABILITY_STURDY) || defAbility == ABILITY_MULTISCALE ||
        ...
        } else if (!hasStatBoost)
        {
          ...
          if (gSideStatuses[battlerAtk] & SIDE_STATUS_SPIKES && switchScore >=
          SWITCHING_INCREASE_CAN_REMOVE_HAZARDS) return PIVOT;*/

          /*if (BattlerWillFaintFromSecondaryDamage(battlerAtk,
             AI_DATA->abilities[battlerAtk]) && switchScore >=
             SWITCHING_INCREASE_WALLS_FOE) return PIVOT;*/

          /*if (IsClassDamager(class) && switchScore >=
          SWITCHING_INCREASE_HAS_SUPER_EFFECTIVE_MOVE)
          {
    ...
    } else  // Opponent Goes First
    {
      if (CanTargetFaintAi(battlerDef, battlerAtk))
      {
        ...
        {
          return DONT_PIVOT;  // If you're going to faint because you'll go
                              // second, use a different move
        } else
        {
          return CAN_TRY_PIVOT;  // You're probably going to faint anyways so if
                                 // for some reason you don't, better switch
        }
      } else if (CanTargetFaintAiWithMod(battlerDef, battlerAtk, 0,
                                         2))  // Foe can 2HKO AI
      {
      ...
      } else  // Foe can 3HKO+ AI
      {
        ...
        {
          if (!BattlerWillFaintFromSecondaryDamage(battlerAtk,
                                                   AI_DATA->abilities[battlerAtk])  // This is the only move that
                                                                                    // can KO
        ...
        } else if (CanAIFaintTarget(battlerAtk, battlerDef, 2))
        {
          ...
          if (IS_MOVE_STATUS(move) &&
              (shouldSwitch  // Damaging move
                             //&& (switchScore >=
                             // SWITCHING_INCREASE_RESIST_ALL_MOVES +
                             // SWITCHING_INCREASE_KO_FOE //remove hazards
        ...
        {
          // if (IsClassDamager(class) && switchScore >=
          // SWITCHING_INCREASE_KO_FOE) return PIVOT; //Only switch if way
          // better matchup
          ...
          if (!hasStatBoost)
          {
            ...
            // if (BattlerWillFaintFromSecondaryDamage(battlerAtk) &&
            // switchScore >= SWITCHING_INCREASE_HAS_SUPER_EFFECTIVE_MOVE)
            // return PIVOT;
...
bool32 IsBattlerIncapacitated(u32 battler, u32 ability)
{
  ...
  {
    return TRUE;  // if battler has thawing move we assume they will definitely
                  // use it, and thus being frozen should be neglected
  }
...
bool32 AI_CanSleep(u32 battler, u32 ability)
{
  if (ability == ABILITY_INSOMNIA || ability == ABILITY_VITAL_SPIRIT || ability == ABILITY_COMATOSE ||
      gBattleMons[battler].status1 & STATUS1_ANY || gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SAFEGUARD ||
...
{
  if (!AI_CanSleep(battlerDef, defAbility) || DoesSubstituteBlockMove(battlerAtk, battlerDef, move) ||
      PartnerMoveEffectIsStatusSameTarget(BATTLE_PARTNER(battlerAtk), battlerDef,
                                          partnerMove))  // shouldn't try to sleep mon that partner is trying to
                                                         // make sleep
  {
    return FALSE;
  }
...
static bool32 AI_CanBePoisoned(u32 battlerAtk, u32 battlerDef, u32 move)
{
  ...
  if (!(CanPoisonType(battlerAtk, battlerDef)) || gSideStatuses[GetBattlerSide(battlerDef)] & SIDE_STATUS_SAFEGUARD ||
      gBattleMons[battlerDef].status1 & STATUS1_ANY || ability == ABILITY_IMMUNITY || ability == ABILITY_COMATOSE ||
      AI_IsAbilityOnSide(battlerDef, ABILITY_PASTEL_VEIL) || gBattleMons[battlerDef].status1 & STATUS1_ANY ||
      IsAbilityStatusProtected(battlerDef) || AI_IsTerrainAffected(battlerDef, STATUS_FIELD_MISTY_TERRAIN))
  {
    return FALSE;
  }
  return TRUE;
...
bool32 ShouldPoisonSelf(u32 battler, u32 ability)
{
  if (AI_CanBePoisoned(battler, battler, 0) &&
      (ability == ABILITY_MARVEL_SCALE || ability == ABILITY_POISON_HEAL || ability == ABILITY_QUICK_FEET ||
  ...
  {
    return TRUE;  // battler can be poisoned and has move/ability that
                  // synergizes with being poisoned
  }
  return FALSE;
}
...
bool32 AI_CanPoison(u32 battlerAtk, u32 battlerDef, u32 defAbility, u32 move, u32 partnerMove)
{
  ...
  {
    return FALSE;
  } else if (defAbility != ABILITY_CORROSION &&
...
{
  if (ability == ABILITY_LIMBER || ability == ABILITY_COMATOSE || IS_BATTLER_OF_TYPE(battler, TYPE_ELECTRIC) ||
...
{
  if ((gBattleMons[battler].status2 & STATUS2_CONFUSION) || (ability == ABILITY_OWN_TEMPO) ||
...
bool32 AI_CanBeBurned(u32 battler, u32 ability)
{
  if (ability == ABILITY_WATER_VEIL || ability == ABILITY_WATER_BUBBLE || ability == ABILITY_COMATOSE ||
      IS_BATTLER_OF_TYPE(battler, TYPE_FIRE) || gBattleMons[battler].status1 & STATUS1_ANY ||
      IsAbilityStatusProtected(battler) || gSideStatuses[GetBattlerSide(battler)] & SIDE_STATUS_SAFEGUARD)
  {
    return FALSE;
  }
  return TRUE;
...
{
  if (ability == ABILITY_MAGMA_ARMOR || ability == ABILITY_COMATOSE || IS_BATTLER_OF_TYPE(battler, TYPE_ICE) ||
      gBattleMons[battler].status1 & STATUS1_ANY || IsAbilityStatusProtected(battler) ||
...
bool32 ShouldBurnSelf(u32 battler, u32 ability)
{
  if (AI_CanBeBurned(battler, ability) &&
      (ability == ABILITY_QUICK_FEET || ability == ABILITY_HEATPROOF || ability == ABILITY_MAGIC_GUARD ||
...
bool32 AI_CanGiveFrostbite(u32 battlerAtk,
                           u32 battlerDef,
                           u32 defAbility,
...
bool32 AI_CanBeInfatuated(u32 battlerAtk, u32 battlerDef, u32 defAbility)
{
  if ((gBattleMons[battlerDef].status2 & STATUS2_INFATUATION) ||
      ...
      defAbility == ABILITY_OBLIVIOUS || !AreBattlersOfOppositeGender(battlerAtk, battlerDef) ||
...
u32 ShouldTryToFlinch(u32 battlerAtk, u32 battlerDef, u32 atkAbility, u32 defAbility, u32 move)
{
  ...
  {
    return 0;
  } else if ((atkAbility == ABILITY_SERENE_GRACE || gBattleMons[battlerDef].status1 & STATUS1_PARALYSIS ||
...
{
  if (!gDisableStructs[battlerAtk].isFirstTurn || AI_DATA->abilities[battlerAtk] == ABILITY_GORILLA_TACTICS ||
...
bool32 ShouldAbsorb(u32 battlerAtk, u32 battlerDef, u32 move, s32 damage)
{
  if (move == 0xFFFF || AI_WhoStrikesFirst(battlerAtk, battlerDef, move) == AI_IS_FASTER)
  {
    ...
    {
      return TRUE;  // target can't faint attacker at all, attacker health is
                    // about half, 2/3rds rate of encouraging healing
    }
  } else
  {
    // opponent goes first
...
bool32 ShouldSetScreen(u32 battlerAtk, u32 battlerDef, u32 moveEffect)
{
  ...
  switch (moveEffect)
  {
    case EFFECT_AURORA_VEIL:
      // Use only in Hail and only if AI doesn't already have Reflect, Light
      // Screen or Aurora Veil itself active.
      ...
      break;
    case EFFECT_REFLECT:
      // Use only if the player has a physical move and AI doesn't already have
      // Reflect itself active.
      if (HasMoveWithSplit(battlerDef, SPLIT_PHYSICAL) && !(gSideStatuses[atkSide] & SIDE_STATUS_REFLECT))
      {
        return TRUE;
      }
      break;
    case EFFECT_LIGHT_SCREEN:
      // Use only if the player has a special move and AI doesn't already have
      // Light Screen itself active.
      if (HasMoveWithSplit(battlerDef, SPLIT_SPECIAL) && !(gSideStatuses[atkSide] & SIDE_STATUS_LIGHTSCREEN))
      {
        return TRUE;
      }
      break;
...
bool32 DoesPartnerHaveSameMoveEffect(u32 battlerAtkPartner, u32 battlerDef, u32 move, u32 partnerMove)
{
  ...
  if (gBattleMoves[move].effect == gBattleMoves[partnerMove].effect && partnerMove != MOVE_NONE &&
...
{
  if (move != MOVE_NONE &&
      (gBattleMoves[move].effect == EFFECT_SUNNY_DAY || gBattleMoves[move].effect == EFFECT_RAIN_DANCE ||
       gBattleMoves[move].effect == EFFECT_SANDSTORM || gBattleMoves[move].effect == EFFECT_HAIL ||
       gBattleMoves[move].effect == EFFECT_SNOWSCAPE || gBattleMoves[move].effect == EFFECT_ACID_RAIN))
  {
    return TRUE;
  }
  return FALSE;
...
bool32 ShouldUseWishAromatherapy(u32 battlerAtk, u32 battlerDef, u32 move)
{
  ...
  if (CountUsablePartyMons(battlerAtk) == 0 &&
      (CanTargetFaintAi(battlerDef, battlerAtk) ||
       BattlerWillFaintFromSecondaryDamage(battlerAtk, AI_DATA->abilities[battlerAtk])))
  {
  ...
  } else
  {
    ...
    {
      case EFFECT_WISH:
        return ShouldRecover(battlerAtk, battlerDef, move,
                             50);  // Switch recovery isn't good idea in doubles
      case EFFECT_HEAL_BELL:
        if (hasStatus)
        {
          return TRUE;
...
// party logic
s32 AI_CalcPartyMonDamage(u32 move,
                          u32 battlerAtk,
                          u32 battlerDef,
                          struct BattlePokemon switchinCandidate,
                          bool8 isPartyMonAttacker)
{
  s32 dmg;
  u8 effectiveness;
...
bool32 IsPartyFullyHealedExceptBattler(u32 battlerId)
{
  ...
  for (i = 0; i < PARTY_SIZE; i++)
  {
    if (i != gBattlerPartyIndexes[battlerId] && GetMonData(&party[i], MON_DATA_HP) != 0 &&
        GetMonData(&party[i], MON_DATA_SPECIES_OR_EGG) != SPECIES_NONE &&
        GetMonData(&party[i], MON_DATA_SPECIES_OR_EGG) != SPECIES_EGG &&
...
static const u16 sRecycleEncouragedItems[] = {
    ITEM_CHESTO_BERRY, ITEM_LUM_BERRY,    ITEM_STARF_BERRY, ITEM_SITRUS_BERRY,
    ITEM_MICLE_BERRY,  ITEM_CUSTAP_BERRY, ITEM_MENTAL_HERB, ITEM_FOCUS_SASH,
    // TODO expand this
};

// Its assumed that the berry is strategically given, so no need to check
...
void IncreaseStatUpScore(u32 battlerAtk, u32 battlerDef, u32 statId, s32* score)
{
  ...
  {
    return;  // Damaging moves would get a score boost from AI_TryToFaint or
             // PreferStrongestMove so we don't consider them here
  }
  ...
  switch (statId)
  {
    ...
    case STAT_SPDEF:
      ...
      break;
    case STAT_ACC:
      if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 80, TRUE, AI_DATA->abilities[battlerAtk],
                                 AI_DATA->abilities[battlerDef], AI_DATA->holdEffects[battlerAtk],
                                 AI_DATA->holdEffects[battlerDef]))
      {
        ADJUST_SCORE_PTR(2);  // has moves with less than 80% accuracy
      } else if (HasMoveWithLowAccuracy(battlerAtk, battlerDef, 90, TRUE, AI_DATA->abilities[battlerAtk],
                                        AI_DATA->abilities[battlerDef], AI_DATA->holdEffects[battlerAtk],
                                        AI_DATA->holdEffects[battlerDef]))
      {
        ADJUST_SCORE_PTR(1);
...
{
  if (((AI_THINKING_STRUCT->aiFlags & AI_FLAG_TRY_TO_FAINT) && CanAIFaintTarget(battlerAtk, battlerDef, 0)) ||
      AI_DATA->holdEffects[battlerDef] == HOLD_EFFECT_CURE_PSN ||
      ...
      AI_DATA->hpPercents[battlerDef] > 20)
  {
    ...
    if (HasMoveEffect(battlerAtk, EFFECT_VENOSHOCK) || HasMoveEffect(battlerAtk, EFFECT_HEX) ||
        HasMoveEffect(battlerAtk, EFFECT_VENOM_DRENCH) || AI_DATA->abilities[battlerAtk] == ABILITY_MERCILESS)
    {
      ADJUST_SCORE_PTR(2);
    } else
    {
...
void IncreaseBurnScore(u32 battlerAtk, u32 battlerDef, u32 move, s32* score)
{
  ...
  if (AI_CanBurn(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move,
                 AI_DATA->partnerMove))
  {
...
void IncreaseParalyzeScore(u32 battlerAtk, u32 battlerDef, u32 move, s32* score)
{
  ...
  if (AI_CanParalyze(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef], move, AI_DATA->partnerMove))
  {
    ...
    if ((defSpeed >= atkSpeed && defSpeed / 2 < atkSpeed)  // You'll go first after paralyzing foe
        || HasMoveEffect(battlerAtk, EFFECT_HEX) || HasMoveEffect(battlerAtk, EFFECT_FLINCH_HIT) ||
...
void IncreaseConfusionScore(u32 battlerAtk, u32 battlerDef, u32 move, s32* score)
{
  ...
  if (AI_CanConfuse(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move,
                    AI_DATA->partnerMove) &&
      AI_DATA->holdEffects[battlerDef] != HOLD_EFFECT_CURE_CONFUSION &&
      AI_DATA->holdEffects[battlerDef] != HOLD_EFFECT_CURE_STATUS)
  {
    if (gBattleMons[battlerDef].status1 & STATUS1_PARALYSIS || gBattleMons[battlerDef].status2 & STATUS2_INFATUATION ||
...
void IncreaseFrostbiteScore(u32 battlerAtk, u32 battlerDef, u32 move, s32* score)
{
  ...
  if (AI_CanGiveFrostbite(battlerAtk, battlerDef, AI_DATA->abilities[battlerDef], BATTLE_PARTNER(battlerAtk), move,
                          AI_DATA->partnerMove))
  {
...
{
  if (gBattleMoves[move].flags & FLAG_MAKES_CONTACT && ability != ABILITY_LONG_REACH &&
...
{
  // simple logic. just upgrades chosen move to z move if possible, unless
  // regular move would kill opponent
  ...
  if (IsViableZMove(battlerAtk, chosenMove))
  {
    ...
    if (!IS_MOVE_STATUS(chosenMove) &&
        AI_CalcDamageSaveBattlers(chosenMove, battlerAtk, battlerDef, &effectiveness, FALSE) >=
            gBattleMons[battlerDef].hp)
    {
      return FALSE;  // don't waste damaging z move if can otherwise faint
                     // target
    }
