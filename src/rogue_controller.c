...
static bool8 TryOpenHubPokeblockBundle(u16 itemId, u8 rewardCount)
{
    u8 i;
    u8 j;
    u16 rewards[POKEBLOCKS_PER_BIG_BUNDLE];

    AGB_ASSERT(rewardCount <= ARRAY_COUNT(rewards));

    for(i = 0; i < rewardCount; ++i)
        rewards[i] = ChooseRunRewardPokeblock();
    ...
    for(i = 0; i < rewardCount; ++i)
    {
        if(!AddBagItem(rewards[i], 1))
        {
            // Roll back the complete transaction if any rolled reward cannot
            // fit, leaving the bundle available to open later.
            while(i != 0)
            {
                --i;
                AGB_ASSERT(RemoveBagItem(rewards[i], 1));
            }

            AGB_ASSERT(AddBagItem(itemId, 1));
            return FALSE;
        }
    }
    ...
    for(i = 0; i < rewardCount; ++i)
    {
        ...
        for(j = i + 1; j < rewardCount; ++j)
        {
            if(rewards[j] == rewards[i])
                ++count;
        }

        Rogue_PushPopup_AddItem(rewards[i], count);
    }

    return TRUE;
}

u16 Rogue_OpenHubPokeblockBundles(void)
{
    u16 rewardCount = 0;
    ...
    while(CheckBagHasItem(ITEM_POKEBLOCK_BUNDLE, 1)
        && TryOpenHubPokeblockBundle(ITEM_POKEBLOCK_BUNDLE, POKEBLOCKS_PER_SMALL_BUNDLE))
        rewardCount += POKEBLOCKS_PER_SMALL_BUNDLE;

    while(CheckBagHasItem(ITEM_BIG_POKEBLOCK_BUNDLE, 1)
        && TryOpenHubPokeblockBundle(ITEM_BIG_POKEBLOCK_BUNDLE, POKEBLOCKS_PER_BIG_BUNDLE))
        rewardCount += POKEBLOCKS_PER_BIG_BUNDLE;
