...
struct PokemonSubstruct3
{
  ...
  /* 0x09 */ u32 championRibbon : 1;  // Given when defeating the Champion. Because both RSE and FRLG use it, later
                                      // generations don't specify from which region it comes from.
  /* 0x0A */ u32 winningRibbon : 1;  // Given at the Battle Tower's Level 50 challenge by winning a set of seven battles
                                     // that extends the current streak to 56 or more.
  /* 0x0A */ u32 victoryRibbon : 1;  // Given at the Battle Tower's Level 100 challenge by winning a set of seven
                                     // battles that extends the current streak to 56 or more.
  /* 0x0A */ u32
      artistRibbon : 1;  // Given at the Contest Hall by winning a Master Rank contest with at least 800 points, and
                         // agreeing to have the Pokémon's portrait placed in the museum after being offered.
  ...
  // If set, a Pokémon is a fateful encounter in FRLG's summary screen if hatched & for all Pokémon in Gen 4+ summary
  // screens. Set for in-game event island legendaries, events distributed after a certain date, & Pokémon from XD: Gale
  // of Darkness. Not to be confused with METLOC_FATEFUL_ENCOUNTER.
...
#define NUM_SUBSTRUCT_BYTES \
  (max(sizeof(struct PokemonSubstruct0), \
       max(sizeof(struct PokemonSubstruct1), \
           max(sizeof(struct PokemonSubstruct2), sizeof(struct PokemonSubstruct3)))))
...
struct SpeciesInfo /* size >= 0x8E */
{
  ...
  /* 0x15 */ u8 growthRate;
  /* 0x16 */ u8 eggGroups[2];
  /* 0x18 */ u16 abilities[NUM_ABILITY_SLOTS];  // 3 abilities
  /* 0x1E */ u8 safariZoneFleeRate;
  /* 0x1F */ u8 padding1;
  /* 0x20 */ u16 uniqueAbility;

  // Pokédex data
  /* 0x22 */ u8 categoryName[13];
  /* 0x2F */ u8 speciesName[POKEMON_NAME_LENGTH + 1];  // Usually 11 (10 + null)
  /* 0x3A */ u16 cryId;
  /* 0x3C */ u16 natDexNum;
  /* 0x3E */ u16 height;
  /* 0x40 */ u16 weight;
  /* 0x42 */ u16 pokemonScale;
  /* 0x44 */ u16 pokemonOffset;
  /* 0x46 */ u16 trainerScale;
  /* 0x48 */ u16 trainerOffset;
  /* 0x4A */ const u8* description;
  ...
  // Graphics
  /* 0x4F */ u8 frontAnimDelay;
  /* 0x50 */ u8 frontAnimId;
  /* 0x51 */ u8 backAnimId;
  /* 0x52 */ const union AnimCmd* const* frontAnimFrames;
  /* 0x56 */ const u32* frontPic;
  /* 0x5A */ const u32* frontPicFemale;
  /* 0x5E */ const u32* backPic;
  /* 0x62 */ const u32* backPicFemale;
  /* 0x66 */ const u32* palette;
  /* 0x6A */ const u32* paletteFemale;
  /* 0x6E */ const u32* shinyPalette;
  /* 0x72 */ const u32* shinyPaletteFemale;
  /* 0x76 */ const u8* iconSprite;
  /* 0x7A */ const u8* iconSpriteFemale;
  /* 0x7E */ const u8* footprint;

  // Sprite bounding box
  /* 0x82 */ u8 frontPicSize;
  /* 0x83 */ u8 frontPicSizeFemale;
  /* 0x84 */ u8 frontPicYOffset;
  /* 0x85 */ u8 backPicSize;
  /* 0x86 */ u8 backPicSizeFemale;
  /* 0x87 */ u8 backPicYOffset;

  // Icon palette and elevation
  /* 0x88 */ u8 iconPalIndex : 3;
  u8 iconPalIndexFemale : 3;
  u8 stub : 1;
  u8 padding3 : 1;
  /* 0x89 */ u8 enemyMonElevation;

  // Flags
  /* 0x8A */ u32 isLegendary : 1;
  u32 isMythical : 1;
  u32 isUltraBeast : 1;
  u32 isParadoxForm : 1;
...
#define GET_UNOWN_LETTER(personality) \
  (((((personality) & 0x03000000) >> 18) | (((personality) & 0x00030000) >> 12) | \
    (((personality) & 0x00000300) >> 6) | (((personality) & 0x00000003) >> 0)) % \
   NUM_UNOWN_FORMS)
