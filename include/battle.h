...
#define RESOURCE_FLAG_STENCH 0x100
...
{
  struct AiPartyMon mons[NUM_BATTLE_SIDES]
                        [PARTY_SIZE];  // 2 parties(player, opponent). Used to save information on opposing party.
...
struct AiLogicData
{
  ...
  u8 mostSuitableMonId[MAX_BATTLERS_COUNT];  // Stores result of GetMostSuitableMonToSwitchInto, which decides which
                                             // generic mon the AI would switch into if they decide to switch. This can
                                             // be overruled by specific mons found in ShouldSwitch; the final resulting
                                             // mon is stored in AI_monToSwitchIntoId.
  struct SwitchinCandidate
      switchinCandidate;  // Struct used for deciding which mon to switch to in battle_ai_switch_items.c
...
struct BattleStruct
{
  ...
  bool8 terrainDone;
  u8 isAtkCancelerForCalledMove;  // Certain cases in atk canceler should only be checked once, when the original move
                                  // is called, however others need to be checked the twice.
  ...
  s32 aiFinalScore[MAX_BATTLERS_COUNT][MAX_BATTLERS_COUNT]
                  [MAX_MON_MOVES];  // AI, target, moves to make debugging easier
  ...
  u8 skyDropTargets[MAX_BATTLERS_COUNT];  // For Sky Drop, to account for if multiple Pokemon use Sky Drop in a double
                                          // battle.
  // When using a move which hits multiple opponents which is then bounced by a target, we need to make sure, the move
  // hits both opponents, the one with bounce, and the one without.
  ...
  u16 overwrittenAbilities[MAX_BATTLERS_COUNT];  // abilities overwritten during battle (keep separate from battle
                                                 // history in case of switching)
  bool8 allowedToChangeFormInWeather[PARTY_SIZE]
                                    [NUM_BATTLE_SIDES];  // For each party member and side, used by Ice Face.
...
// The palaceFlags member of struct BattleStruct contains 1 flag per move to indicate which moves the AI should
// consider, and 1 flag per battler to indicate whether the battler is awake and at <= 50% HP (which affects move
// choice). The assert below is to ensure palaceFlags is large enough to store these flags without overlap.
STATIC_ASSERT(sizeof(((struct BattleStruct*)0)->palaceFlags) * 8 >= MAX_BATTLERS_COUNT + MAX_MON_MOVES,
              PalaceFlagsTooSmall)

#define DYNAMIC_TYPE_MASK ((1 << 6) - 1)
#define F_DYNAMIC_TYPE_IGNORE_PHYSICALITY \
  (1 << 6)  // If set, the dynamic type's physicality won't be used for certain move effects.
#define F_DYNAMIC_TYPE_SET \
  (1 << 7)  // Set for all dynamic types to distinguish a dynamic type of Normal (0) from no dynamic type.
...
#define IS_EFFECT_RECOIL(effect) \
  (effect == EFFECT_RECOIL_25 || effect == EFFECT_RECOIL_IF_MISS || effect == EFFECT_RECOIL_50 || \
   effect == EFFECT_RECOIL_33 || effect == EFFECT_RECOIL_33_STATUS)
...
#define BATTLER_TURN_DAMAGED(battlerId) \
  ((gSpecialStatuses[battlerId].physicalDmg != 0 || gSpecialStatuses[battlerId].specialDmg != 0) || \
   (gBattleStruct->enduredDamage & gBitTable[battlerId]))

#define IS_BATTLER_OF_TYPE(battlerId, type) \
  ((GetBattlerType(battlerId, 0, FALSE) == type || GetBattlerType(battlerId, 1, FALSE) == type || \
    (GetBattlerType(battlerId, 2, FALSE) != TYPE_MYSTERY && GetBattlerType(battlerId, 2, FALSE) == type)))
#define IS_BATTLER_OF_BASE_TYPE(battlerId, type) \
  ((GetBattlerType(battlerId, 0, TRUE) == type || GetBattlerType(battlerId, 1, TRUE) == type || \
    (GetBattlerType(battlerId, 2, TRUE) != TYPE_MYSTERY && GetBattlerType(battlerId, 2, TRUE) == type)))
#define IS_BATTLER_TYPELESS(battlerId) \
  (GetBattlerType(battlerId, 0, FALSE) == TYPE_MYSTERY && GetBattlerType(battlerId, 1, FALSE) == TYPE_MYSTERY && \
   GetBattlerType(battlerId, 2, FALSE) == TYPE_MYSTERY)
...
#define IS_BATTLER_PROTECTED(battlerId) \
  (gProtectStructs[battlerId].protected || gSideStatuses[GetBattlerSide(battlerId)] & SIDE_STATUS_WIDE_GUARD || \
   ...
   gSideStatuses[GetBattlerSide(battlerId)] & SIDE_STATUS_MAT_BLOCK || gProtectStructs[battlerId].spikyShielded || \
   gProtectStructs[battlerId].kingsShielded || gProtectStructs[battlerId].banefulBunkered || \
   gProtectStructs[battlerId].burningBulwarked || gProtectStructs[battlerId].obstructed || \
...
#define SET_STATCHANGER(statId, stage, goesDown) \
  (gBattleScripting.statChanger = (statId) + ((stage) << 3) + (goesDown << 7))
