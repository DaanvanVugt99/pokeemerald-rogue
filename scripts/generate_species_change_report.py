...
STAT_BUFF_FLAGS_PATH = (
    REPO_ROOT / "src" / "data" / "rogue_stat_buff_flags.h"
)
...
def render_stat_buff_flags_header(
    baseline: dict[str, SpeciesRecord],
    current: dict[str, SpeciesRecord],
) -> str:
    runtime_stat_names = (
    ...
    for key in sorted(set(baseline) & set(current)):
        raised_stats = [
            runtime_stat_names[index]
            for index, (old, new) in enumerate(
                zip(baseline[key].stats, current[key].stats)
            )
            if new > old
        ]
        if not raised_stats:
        ...
        flag_expression = " | ".join(
            f"(1u << {stat})" for stat in raised_stats
        )
        lines.append(f"    case {key}:")
        lines.append(f"        return {flag_expression};")
    lines.extend(
        (
            "    default:",
            "        return 0;",
...
def generate_reports(check: bool) -> bool:
    ...
    stat_buff_flags = render_stat_buff_flags_header(baseline, current)
    markdown_ok = update_or_check(MARKDOWN_PATH, markdown, check)
    csv_ok = update_or_check(CSV_PATH, csv_contents, check)
    stat_buff_flags_ok = update_or_check(
        STAT_BUFF_FLAGS_PATH, stat_buff_flags, check
    )
    return markdown_ok and csv_ok and stat_buff_flags_ok
