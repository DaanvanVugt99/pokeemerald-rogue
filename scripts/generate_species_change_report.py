#!/usr/bin/env python3
"""Generate canonical-vs-Divergence species data artifacts.
...
comparison. The canonical baseline is committed as CSV to keep routine
...
SPECIES_BASELINES_PATH = REPO_ROOT / "src" / "data" / "rogue_species_baselines.h"
DEFAULT_BASELINE_REF = "91a4feca60251da39ac320bac91d7c59a8a7dd9a"

BASELINE_SPECIES_ALIASES = {
    "SPECIES_MEOWSTIC_FEMALE_MEGA": "SPECIES_MEOWSTIC_F_MEGA",
    "SPECIES_MEOWSTIC_MALE_MEGA": "SPECIES_MEOWSTIC_M_MEGA",
}
...
def load_baseline() -> tuple[str, dict[str, SpeciesRecord]]:
    ...
    records: dict[str, SpeciesRecord] = {}
    for row in reader:
        key = BASELINE_SPECIES_ALIASES.get(row["species"], row["species"])
        records[key] = SpeciesRecord(
            key=key,
            name=row["pokemon"],
...
def render_species_baselines_header(
    baseline: dict[str, SpeciesRecord],
    current: dict[str, SpeciesRecord],
) -> str:
    stat_names = ("STAT_HP", "STAT_ATK", "STAT_DEF", "STAT_SPATK", "STAT_SPDEF", "STAT_SPEED")
    lines = [
    ...
    for key in sorted(set(baseline) & set(current)):
        ...
        lines.extend([
            f"    [{key}] = {{",
            "        .stats = {" + ", ".join(
                f"[{name}] = {value}" for name, value in zip(stat_names, record.stats)
            ) + "},",
...
def generate_reports(check: bool) -> bool:
    ...
    species_baselines = render_species_baselines_header(baseline, current)
    markdown_ok = update_or_check(MARKDOWN_PATH, markdown, check)
    csv_ok = update_or_check(CSV_PATH, csv_contents, check)
    species_baselines_ok = update_or_check(
        SPECIES_BASELINES_PATH, species_baselines, check
    )
    return markdown_ok and csv_ok and species_baselines_ok
