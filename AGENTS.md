...
- Pokémon-profile generation requires Mono plus `msbuild` or `xbuild`.
  ...
  offline.
  The launcher builds the profile-only `ProfilePipeline.csproj`; it does not
  require the Windows sprite generator or PresentationCore. Restore the
  generator's Newtonsoft.Json dependency if neither its NuGet package nor its
  existing `bin/Release` copy is available.
...
- Species stats, types, or regular Ability changes must keep the generated
  species-change artifacts and baked runtime data current:
  - Regenerate: `make species-report`
  - Verify only: `make check-species-report`
- Approved rebalance implementation is tracked in `docs/rebalance_implementation.md`
  and `tools/data/rebalance_manifest.json`. Run
  `python3 scripts/check_rebalance_manifest.py` for partial checkpoints and add
  `--require-complete` for final ledger signoff. A partial pass is not completion.
  The report separates new evolution move gaps from historical gaps; newly
  added access missing from an already-covered evolution is a failure.
  Explicit manifest invariants also check approved counterpart BSTs, distinct
  Ability counts, starter second slots, and matching canonical Mega/form deltas.
  Extend those groups deliberately; do not erase intentional form exceptions.
  The checker also rejects runtime stat/type/Ability or learnset changes outside
  explicit manifest targets, including changes inherited by unlisted forms.
  Run `python3 scripts/test_rebalance_manifest.py` when changing the checker.
- Do not hand-edit `docs/generated/species_changes.md`,
  `docs/generated/species_changes.csv`, or
  `src/data/rogue_species_baselines.h`; they are generated by
  `scripts/generate_species_change_report.py`. Do not hand-edit
  `src/data/rogue_bake_data.h`; it is generated by
  `scripts/generate_rogue_bake_data.sh`, and `make species-report` runs both
  generators in the required order.
