...
AI_SINGLE_BATTLE_TEST("AI prefers priority moves if it's slower and can kill target")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Speed(1); Moves(MOVE_QUICK_ATTACK, MOVE_BODY_SLAM); }
    } WHEN {
...
AI_SINGLE_BATTLE_TEST("AI will choose a random move if it's faster and can kill target")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_WOBBUFFET) { Speed(100); Moves(MOVE_QUICK_ATTACK, MOVE_BODY_SLAM); }
    } WHEN {
        TURN { EXPECT_MOVES(opponent, MOVE_QUICK_ATTACK, MOVE_BODY_SLAM); SEND_OUT(player, 1); }
    } SCENE {
        MESSAGE("Wobbuffet fainted!");
    }
}
...
AI_SINGLE_BATTLE_TEST("AI will choose a priority move if it is slower then the target and will be killed")
{
    GIVEN {
        ...
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_WOBBUFFET) { Speed(100); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(60); Speed(1); Moves(MOVE_QUICK_ATTACK, MOVE_BODY_SLAM); }
    } WHEN {
        TURN { MOVE(player, MOVE_BODY_SLAM); EXPECT_MOVE(opponent, MOVE_BODY_SLAM); }
        TURN { MOVE(player, MOVE_BODY_SLAM); EXPECT_MOVE(opponent, MOVE_QUICK_ATTACK); }
    } SCENE {
