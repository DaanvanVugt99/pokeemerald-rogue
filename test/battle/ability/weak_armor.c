...
SINGLE_BATTLE_TEST("Weak Armor lowers Defense by 1 and boosts Speed by 2 when hit by a physical attack")
{
  ...
  SCENE
  {
    ...
    if (move == MOVE_TACKLE)
    {
      ...
      MESSAGE("Slugma's Weak Armor lowered its Sp. Def!");
      MESSAGE("Slugma's Weak Armor raised its Attack!");
      MESSAGE("Slugma's Weak Armor raised its Sp. Atk!");
...
SINGLE_BATTLE_TEST("Weak Armor doesn't interrupt multi hit moves if Defense can't go any lower")
{
  ...
  WHEN
  {
    ...
    TURN
    {
      MOVE(opponent, MOVE_FURY_SWIPES);
    }
  }
  SCENE
  {
