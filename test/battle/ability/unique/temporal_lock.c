...
{
    GIVEN {
        PLAYER(SPECIES_DIALGA) { Speed(100); Ability(ABILITY_PRESSURE); UniqueAbility(ABILITY_TEMPORAL_LOCK); Moves(MOVE_ROAR_OF_TIME); }
        ...
        HP_BAR(opponent);
        ABILITY_POPUP(player, ABILITY_TEMPORAL_LOCK);
...
{
    GIVEN {
        PLAYER(SPECIES_DIALGA_ORIGIN) { Speed(100); Ability(ABILITY_PRESSURE); UniqueAbility(ABILITY_TEMPORAL_LOCK); Moves(MOVE_ROAR_OF_TIME); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(500); MaxHP(500); Speed(50); Moves(MOVE_CELEBRATE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_DIALGA) { Speed(100); Ability(ABILITY_PRESSURE); UniqueAbility(ABILITY_TEMPORAL_LOCK); Moves(MOVE_DRAGON_CLAW); }
...
SINGLE_BATTLE_TEST("Time Rift no longer triggers on switch-in")
{
    GIVEN {
        ...
        OPPONENT(SPECIES_DIALGA) { Ability(ABILITY_PRESSURE); UniqueAbility(ABILITY_TEMPORAL_LOCK); Moves(MOVE_ROAR_OF_TIME); }
    } WHEN {
