...
SINGLE_BATTLE_TEST("Primal Molting cures status and uses Harden when Slither Wing is the only Paradox")
{
    ...
    GIVEN {
        ...
        {
            NONE_OF {
                ABILITY_POPUP(player, ABILITY_PRIMAL_MOLTING);
...
SINGLE_BATTLE_TEST("Primal Molting only triggers once each battle")
{
    GIVEN {
        PLAYER(SPECIES_SLITHER_WING) { HP(100); MaxHP(160); Speed(200); Status1(STATUS1_BURN); Moves(MOVE_CELEBRATE); }
        PLAYER(SPECIES_WOBBUFFET) { Speed(50); Moves(MOVE_CELEBRATE); }
