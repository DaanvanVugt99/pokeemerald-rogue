...
{
    GIVEN {
        PLAYER(SPECIES_MANAPHY) { Ability(ABILITY_HYDRATION); UniqueAbility(ABILITY_DRIFT_SONG); HP(300); MaxHP(400); Moves(MOVE_WATER_GUN); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE); }
    } WHEN {
        TURN { MOVE(player, MOVE_WATER_GUN); MOVE(opponent, MOVE_CELEBRATE); }
    } SCENE {
        ABILITY_POPUP(player, ABILITY_DRIFT_SONG);
        MESSAGE("Manaphy's Drift Song restored its HP a little!");
...
{
    GIVEN {
        PLAYER(SPECIES_MANAPHY) { Ability(ABILITY_HYDRATION); UniqueAbility(ABILITY_DRIFT_SONG); HP(300); MaxHP(400); Moves(MOVE_GROWL); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_MANAPHY) { Speed(100); Ability(ABILITY_HYDRATION); UniqueAbility(ABILITY_DRIFT_SONG); HP(300); MaxHP(400); Moves(MOVE_CELEBRATE, MOVE_WEATHER_BALL); }
...
{
    GIVEN {
        PLAYER(SPECIES_MANAPHY) { Ability(ABILITY_HYDRATION); UniqueAbility(ABILITY_DRIFT_SONG); HP(300); MaxHP(400); Moves(MOVE_TACKLE); }
        OPPONENT(SPECIES_WOBBUFFET) { Moves(MOVE_CELEBRATE); }
    } WHEN {
...
{
    GIVEN {
        PLAYER(SPECIES_MANAPHY) { Speed(100); Ability(ABILITY_TRUANT); UniqueAbility(ABILITY_DRIFT_SONG); HP(300); MaxHP(400); Moves(MOVE_WATER_GUN, MOVE_CELEBRATE); }
        ...
        MESSAGE("Manaphy's Drift Song restored its HP a little!");
        HP_BAR(player, damage: -50);
        MESSAGE("Manaphy is loafing around!");
