...
SINGLE_BATTLE_TEST("Cruel Premonition sets Future Sight after Freezing Glare")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { SpAttack(100); UniqueAbility(ABILITY_CRUEL_PREMONITION); Moves(MOVE_FREEZING_GLARE); }
        ...
        ABILITY_POPUP(player, ABILITY_CRUEL_PREMONITION);
...
SINGLE_BATTLE_TEST("Cruel Premonition does not replace a pending Future Sight")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { SpAttack(100); UniqueAbility(ABILITY_CRUEL_PREMONITION); Moves(MOVE_FUTURE_SIGHT, MOVE_FREEZING_GLARE); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1000); MaxHP(1000); SpDefense(100); Moves(MOVE_CELEBRATE); }
    } WHEN {
...
SINGLE_BATTLE_TEST("Cruel Premonition sets Future Sight after Freezing Glare knocks out its target")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { SpAttack(500); UniqueAbility(ABILITY_CRUEL_PREMONITION); Moves(MOVE_FREEZING_GLARE); }
...
SINGLE_BATTLE_TEST("Cruel Premonition does not trigger when Freezing Glare misses")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { UniqueAbility(ABILITY_CRUEL_PREMONITION); Moves(MOVE_CELEBRATE, MOVE_FREEZING_GLARE); }
...
SINGLE_BATTLE_TEST("Cruel Premonition does not trigger after knocking out the final opposing Pokemon")
{
    GIVEN {
        PLAYER(SPECIES_WOBBUFFET) { SpAttack(500); UniqueAbility(ABILITY_CRUEL_PREMONITION); Moves(MOVE_FREEZING_GLARE); }
        OPPONENT(SPECIES_WOBBUFFET) { HP(1); MaxHP(1); SpDefense(1); Moves(MOVE_CELEBRATE); }
