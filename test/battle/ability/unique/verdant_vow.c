...
SINGLE_BATTLE_TEST("Verdant Vow activates after an allied faint and boosts only the next damaging move", s16 firstDamage, s16 secondDamage)
{
    bool32 allyFainted;
    ...
    GIVEN {
        ...
        PLAYER(SPECIES_VIRIZION) { Ability(ABILITY_JUSTIFIED); UniqueAbility(ABILITY_VERDANT_VOW); Attack(100); Speed(100); Moves(MOVE_TACKLE); }
        ...
        if (allyFainted) {
            ABILITY_POPUP(player, ABILITY_VERDANT_VOW);
            ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_STATS_CHANGE, player);
            MESSAGE("Virizion's Verdant Vow raised its Sp. Def!");
        } else {
            NONE_OF {
                ABILITY_POPUP(player, ABILITY_VERDANT_VOW);
            }
        ...
        EXPECT_EQ(player->statStages[STAT_SPDEF], allyFainted ? DEFAULT_STAT_STAGE + 1 : DEFAULT_STAT_STAGE);
    } FINALLY {
