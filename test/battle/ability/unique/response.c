...
SINGLE_BATTLE_TEST("Response gives +1 priority to status moves if the party contains an Electric-type Pokemon")
{
    GIVEN {
        PLAYER(SPECIES_ILLUMISE) { Speed(50); Ability(ABILITY_OBLIVIOUS); UniqueAbility(ABILITY_RESPONSE); Moves(MOVE_GROWL); }
...
SINGLE_BATTLE_TEST("Response does not give +1 priority without an Electric-type Pokemon in the party")
{
    GIVEN {
        PLAYER(SPECIES_ILLUMISE) { Speed(50); Ability(ABILITY_OBLIVIOUS); UniqueAbility(ABILITY_RESPONSE); Moves(MOVE_GROWL); }
...
SINGLE_BATTLE_TEST("Response still affects Dark-type targets")
{
    GIVEN {
        PLAYER(SPECIES_ILLUMISE) { Speed(50); Ability(ABILITY_OBLIVIOUS); UniqueAbility(ABILITY_RESPONSE); Moves(MOVE_GROWL); }
        PLAYER(SPECIES_PIKACHU) { Speed(1); Ability(ABILITY_STATIC); }
