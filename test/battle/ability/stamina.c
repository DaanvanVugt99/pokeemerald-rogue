...
SINGLE_BATTLE_TEST("Stamina maximizes Defense on a critical hit")
{
  GIVEN
  {
    PLAYER(SPECIES_MUDSDALE)
    {
  ...
  WHEN
  {
    TURN
    {
      MOVE(opponent, MOVE_SLASH, criticalHit : TRUE);
    }
  }
  SCENE
  {
    MESSAGE("Mudsdale's Defense rose drastically!");
  }
}
...
DOUBLE_BATTLE_TEST("Stamina activates correctly for every battler with the ability when hit by a multi target move")
{
  ...
  PARAMETRIZE
  {
    abilityLeft = ABILITY_NONE, abilityRight = ABILITY_STAMINA;
  }
  PARAMETRIZE
  {
    abilityLeft = ABILITY_STAMINA, abilityRight = ABILITY_NONE;
  }
  PARAMETRIZE
  {
    abilityLeft = ABILITY_STAMINA, abilityRight = ABILITY_STAMINA;
  }
  ...
  SCENE
  {
    ...
    NOT HP_BAR(opponentLeft);  // We need to check the attacker itself does NOT get damaged. There was an issue when the
                               // targets would get overwritten by the Stamina's stat raise.
