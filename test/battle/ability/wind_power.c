...
ASSUMPTIONS
{
  ...
  ASSUME(gBattleMoves[MOVE_AIR_CUTTER].flags2 & FLAG_WIND_BASED);
  ASSUME(gBattleMoves[MOVE_PETAL_BLIZZARD].power != 0);
  ASSUME(gBattleMoves[MOVE_PETAL_BLIZZARD].target == MOVE_TARGET_FOES_AND_ALLY);
  ASSUME(gBattleMoves[MOVE_PETAL_BLIZZARD].flags2 & FLAG_WIND_BASED);
  ASSUME(!(gBattleMoves[MOVE_TACKLE].flags2 & FLAG_WIND_BASED));
}
...
SINGLE_BATTLE_TEST("Wind Power sets up Charge for player when hit by a wind move")
{
  ...
  GIVEN
  {
    ...
    OPPONENT(SPECIES_WOBBUFFET)
    {
      ...
      Speed(5);
    }  // Limber, so it doesn't get paralyzed.
  }
  WHEN
  {
    TURN
    {
      MOVE(player, MOVE_THUNDERBOLT), MOVE(opponent, move);
    }
    TURN
    {
      MOVE(player, MOVE_THUNDERBOLT), MOVE(opponent, move);
...
SINGLE_BATTLE_TEST("Wind Power sets up Charge for opponent when hit by a wind move")
{
  ...
  WHEN
  {
    TURN
    {
      MOVE(opponent, MOVE_THUNDERBOLT), MOVE(player, move);
    }
    TURN
    {
      MOVE(opponent, MOVE_THUNDERBOLT), MOVE(player, move);
...
DOUBLE_BATTLE_TEST("Wind Power activates correctly for every battler with the ability when hit by a 2/3 target move")
{
  ...
  PARAMETRIZE
  {
    abilityLeft = ABILITY_NONE, abilityRight = ABILITY_WIND_POWER;
  }
  PARAMETRIZE
  {
    abilityLeft = ABILITY_WIND_POWER, abilityRight = ABILITY_NONE;
  }
  PARAMETRIZE
  {
    abilityLeft = ABILITY_WIND_POWER, abilityRight = ABILITY_WIND_POWER;
  }
  ...
  WHEN
  {
    TURN
    {
      MOVE(opponentLeft, MOVE_AIR_CUTTER);
      MOVE(opponentRight, MOVE_AIR_CUTTER);
    }
  }
  SCENE
  {
...
DOUBLE_BATTLE_TEST("Wind Power activates correctly for every battler with the ability when hit by a 3 target move")
{
  ...
  WHEN
  {
    TURN
    {
      MOVE(opponentLeft, MOVE_PETAL_BLIZZARD);
    }
  }
  SCENE
  {
...
DOUBLE_BATTLE_TEST("Wind Power activates correctly when Tailwind is used")
{
  ...
  WHEN
  {
    TURN
    {
      MOVE((opponentSide == TRUE) ? opponentLeft : playerLeft, MOVE_TAILWIND);
    }
  }
  SCENE
  {
