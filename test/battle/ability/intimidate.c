...
SINGLE_BATTLE_TEST("Oblivious prevents Intimidate from lowering Attack", s16 damage)
{
    u32 playerAbility;
    PARAMETRIZE { playerAbility = ABILITY_OBLIVIOUS; }
    PARAMETRIZE { playerAbility = ABILITY_NONE; }
    ...
    WHEN
    {
        TURN { ; } // Trigger Intimidate on opponent switch-in
    }
    SCENE
    {
        ABILITY_POPUP(opponent, ABILITY_INTIMIDATE);
        if (playerAbility == ABILITY_OBLIVIOUS)
        {
