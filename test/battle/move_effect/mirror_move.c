...
SINGLE_BATTLE_TEST("Mirror Move's called powder move fails against Grass Types")
{
    GIVEN
    {
        ASSUME(gBattleMoves[MOVE_STUN_SPORE].flags & FLAG_POWDER_BASED);
    ...
    WHEN
    {
        TURN
        {
            MOVE(player, MOVE_STUN_SPORE);
            MOVE(opponent, MOVE_MIRROR_MOVE);
        }
    }
    SCENE
    {
