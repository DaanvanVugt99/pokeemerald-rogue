...
DOUBLE_BATTLE_TEST("Focus Punch activation is based on Speed")
{
  ...
  WHEN
  {
    TURN
    {
      MOVE(opponentRight, MOVE_FOCUS_PUNCH, target : playerLeft);
      MOVE(playerRight, MOVE_FOCUS_PUNCH, target : opponentLeft);
      MOVE(playerLeft, MOVE_FOCUS_PUNCH, target : opponentLeft);
      MOVE(opponentLeft, MOVE_FOCUS_PUNCH, target : playerLeft);
    }
  }
  SCENE
  {
...
SINGLE_BATTLE_TEST("Focus Punch with Inner Focus halves damage after being hit", s16 damage)
{
  u32 move;
  ...
  SCENE
  {
    ...
    ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_FOCUS_PUNCH_SETUP, player);
    MESSAGE("Lucario is tightening its focus!");
    ANIMATION(ANIM_TYPE_MOVE, MOVE_TACKLE, opponent);
    MESSAGE("Lucario used Focus Punch!");
    ANIMATION(ANIM_TYPE_MOVE, MOVE_FOCUS_PUNCH, player);
    HP_BAR(opponent);
  }
  FINALLY
