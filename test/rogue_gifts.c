...
TEST("Dynamic unique ability eligibility audit contains 572 abilities")
{
    ...
    for(ability = ABILITY_STRONG_WINDS; ability <= ABILITY_PREMONITION; ++ability)
    {
        if(RogueGift_IsDynamicUniqueAbilityEligible(ability))
            ++eligibleCount;
    }

    EXPECT_EQ(eligibleCount, 572);
    EXPECT_EQ(RogueGift_GetDynamicUniqueAbilityPoolCount(), 572);
